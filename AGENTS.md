...
5. **保留策略（保留期 / 上限 / 归档去向）**：**保留期** = 见第 4 条（探针批次收尾即删、交付批次消费完成即删；**通道无固定天数、无归档职能**）；**上限** = **在途批次 ≤ 1**（稳态 = 目录内仅 `README.md`），超限即违约，由巡检 / 宿主上报；**归档去向** = 需留存者落 `~/.nebflow/docs/<域>/`（人类交付 / 阶段文档，遵 `CONVENTIONS.md` §1/§2）或 `~/.nebflow/staging-archive-<YYYYMMDD_HHMMSS>/`（整树历史）。
