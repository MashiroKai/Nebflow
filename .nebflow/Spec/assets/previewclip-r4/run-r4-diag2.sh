...
# 跑 previewclip-r4-probe2.mjs 四层像素探针，然后 cleanup。
...
NEBFLOW_GATEWAY_PORT=$PORT java -cp "$CP" nebflow.Main \
  --home "$HOME_DIR" --port $PORT --no-browser start > "$HOME_DIR/instance-r4-diag2.log" 2>&1 &
INSTANCE_PID=$!
...
for i in $(seq 1 60); do
  ...
  kill -0 "$INSTANCE_PID" 2>/dev/null || { echo "FAIL: instance died early"; tail -20 "$HOME_DIR/instance-r4-diag2.log"; exit 3; }
  sleep 1
done
[ "$ok" = "1" ] || { echo "FAIL: gateway never became ready"; tail -20 "$HOME_DIR/instance-r4-diag2.log"; exit 3; }
...
node "$HOME_DIR/previewclip-r4-probe2.mjs" "$BASE" "diag2" > "$HOME_DIR/probe2-r4.json" 2> "$HOME_DIR/probe2-r4.err"
H=$?
if [ $H -ne 0 ]; then echo "PROBE FAIL (exit $H)"; tail -30 "$HOME_DIR/probe2-r4.err"; exit 7; fi
echo "== probe result: $HOME_DIR/probe2-r4.json =="
exit 0
