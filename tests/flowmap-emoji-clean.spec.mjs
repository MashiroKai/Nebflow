...
const emojiScan = (page) => page.evaluate(() => {
    ...
    while (walk.nextNode()) {
      ...
      for (const ch of txt) {
        if (EMOJI.test(ch) && !WHITELIST.has(ch)) {
          hits.push({ where: 'text', code: ch.codePointAt(0).toString(16), card: card.dataset.nodeId });
        }
      }
    }
    const titles = [card.getAttribute('title') || ''];
    card.querySelectorAll('[title]').forEach((el) => titles.push(el.getAttribute('title') || ''));
    for (const txt of titles) {
      for (const ch of txt) {
        if (EMOJI.test(ch) && !WHITELIST.has(ch)) {
          hits.push({ where: 'title', code: ch.codePointAt(0).toString(16), card: card.dataset.nodeId });
        }
      }
    }
  });
