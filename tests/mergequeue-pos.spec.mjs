...
//     路线「案 A：引擎条件键 + 前端渲染」）。
// 契约（前端只读不派生；真源 = 引擎条件键 `mergeQueue`，见 ProjectTypes.scala）：
//   head 行徽标  形态「排队中 · 前面还有 N 个」（N = 同键当前被挡的其他节点数含持有者）
//   等待脚注行   形态「被 XX 挡着」（XX = 持有者节点名，多持有者全列）
//   降级红线     键缺失 / 不可计算 / 同键多项目 ⇒ **不渲染数字**（至多裸「排队中」）
//
...
// ① 单持有者排队态：徽标「排队中 · 前面还有 1 个」+ 脚注「被 attach-merge 挡着」
const N_Q1 = {
...
// ② 多持有者排队态：徽标「前面还有 3 个」+ 脚注三名全列（含 running 与开态先到者）
const N_Q3 = {
...
// ④ 降级（键不可计算）：ahead 缺键 ⇒ 裸「排队中」，无数字（🔴 禁编造数字）
const N_QZ = {
  ...
  for (const vp of VIEWPORTS) {
    test.describe(`排队位次 — ${theme} / ${vp.label}(${vp.width}x${vp.height})`, () => {
      ...
      test(`徽标数字 + 持有者双显 / 降级不渲染数字 / 零漂移 / 卡内不溢出（fixture 载荷）`, async ({ page }) => {
        ...
        // ① 徽标（head 行）形态「排队中 · 前面还有 N 个」——N 取载荷 ahead 逐字渲染
        expect(r['n-q1'].badge).toBe('排队中 · 前面还有 1 个');
        expect(r['n-q1'].headCls).toContain('fm-head-queued');
        // ② 多持有者：数字 = 3，脚注三名全列（载荷序 join ' · '）
        expect(r['n-q3'].badge).toBe('排队中 · 前面还有 3 个');
