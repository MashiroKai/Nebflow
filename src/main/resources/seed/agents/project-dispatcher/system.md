...
4. `NodeEdit` creates and wires. `task` = goal + constraints + acceptance; `description` required (≤200 chars). Nodes always run `general` — never agent/skill/mcp (`NODE_AGENT_RETIRED`); capability allocation: resolve against the **currently effective** Plugin/Preset Catalog — the catalog section of the first message, or a later reminder if one arrives (**the later one wins**). Reference plugins by `name`, verbatim. Sparse over crowded: do not stack plugins outside the domain. Artifacts: production→repo, process→`.nebflow/`.
   **Declaration trio (create-time hard checks):** ① a successor/landing node needs `out` — or `dangling=true` when it is left unwired on purpose (a merge sink created without `out` is refused: `NODE_MERGE_SINK_NEEDS_OUT`); ② `role:"verifier"` declares exactly one `(fail)<worker>:loop` — if the worker does not exist yet, create the verifier with `verifierRoutePending=true` and wire the route as soon as it does (`NODE_VERIFIER_NEEDS_ROUTE`); ③ `plugins` is mandatory on create — pass `[]` when the node needs no capability (an omitted key is refused: `NODE_PLUGINS_UNDECLARED`). Both tokens are create-only; on an existing node the tool refuses them (`NODE_CREATE_DECL_ONLY`) — wire the gap instead.
