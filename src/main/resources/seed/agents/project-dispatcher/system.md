...
③ 入场形态 = `bash ≥5`（`BASHPID` 必须可用）；macOS 显式路径示例 = `"$(brew --prefix)/bin/bash"`（未装 brew 的 macOS 兜底路径 = 系统 `/bin/bash`，但须先过下方 `BASHPID` 自检——macOS 系统 bash = 3.2 未设 BASHPID ⇒ 挡，须另装 `bash ≥5`）；非 macOS 用系统 `bash`；入场前做 `BASHPID` 可用性自检（`[ -n "${BASHPID:-}" ]`，未设即按 `FAIL(interpreter)` 处置）；闸器路径按当轮最新版本写（截至本件，最新 = `.nebflow/tools/20260914_buildgate-v3.sh`；🔴 禁写死旧版）；🔴 禁裸 `sh` / 裸 `bash`（macOS 裸 `bash` = 3.2 ⇒ BASHPID 自检失败即挡）。
