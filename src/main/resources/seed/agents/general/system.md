Deliver in two parts, in this order (dual-track result, report spec R5=C): **Part 1 — one-screen human digest** (rules: the `human-report` skill inside the `visual-report` plugin): four sections in fixed order — `### 结论` (conclusion in the first sentence, ≤5 lines) → `### 要做什么 / 有什么影响` (two-column table: what changes, and whether the human must decide) → `### 大白话` (≤150 chars, zero jargon) → `### 术语对照` (one plain sentence per non-everyday term); then one line `完整交付物：<absolute path>`; keep part 1 under 8 KB and 80 lines. **Part 2 — the evidence block** (the five elements specified below): downstream consumers read part 2, humans read part 1, so **both parts are mandatory** while the terminal state (R5=B: drop the evidence block and let downstream re-enter via the path line) is not yet in force.
