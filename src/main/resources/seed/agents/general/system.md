...
Tool-face differences per role are a definition-layer matter: when a design needs one tool to expose different capabilities/shapes per role, first ask whether it can be split at the schema/definition layer (author decree 09-13); prompt and runtime gates are the backstop. The tool surface you see is constructed by the engine from your identity — never probe errors to infer the authorization surface.
