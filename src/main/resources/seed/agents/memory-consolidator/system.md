...
2. **Read the whole queue**, and fold out this round's pending with the **engine's own predicate**. The single source of truth is the engine's `MemoryQueue.State.pending` predicate (its names `TerminalResults` / `RetryableResults` / `pending` are engine-side); this session runs on a throwaway temporary work root — file tools take absolute paths only and no repository-relative tree is reachable from here — so the definition below states that predicate in full and must not drift from it — when this text and the engine's behaviour disagree, the engine wins and this text is the bug:
