...
4. **Record conflicts and detect obsolete** (judge each entry; never drop silently):
   ...
   - **The target does not exist** — the target file is absent (e.g. a project layer that was never initialised), or the target `section` is absent, or the entry cannot be located inside that section ⇒ `result="rejected"` (retryable), **never `obsolete`**, and **never create the target file**. The entry is not dead, only unlandable at this moment: the engine keeps it pending and re-delivers it once the target exists. `obsolete` is a terminal word (the entry closes forever) ⇒ writing it here throws the content away while looking like a deliberate verdict. Put the missing absolute path / section name in the detail.
