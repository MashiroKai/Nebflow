...
5. **Write an outcome back for every entry** (**append** one line to the queue file, never rewrite existing lines):
...
- Environmental failures (queue unreadable, snapshot failed, file tool rejected the path) are **reported truthfully** and stop any scope expansion; the engine side has a hard-timeout fallback (a timeout is recorded as `timeout` and the queue entry survives) — never skip step 5 to beat the clock.
