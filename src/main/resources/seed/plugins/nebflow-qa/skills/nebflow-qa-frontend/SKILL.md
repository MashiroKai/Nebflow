...
```bash
sbt "run --home <隔离 home> --port <隔离端口> --no-browser"
```

- 端口取宿主 gateway 端口之外的独占端口（逐实例递增），home 按任务名隔离。
- 等待就绪：`curl -s http://localhost:<隔离端口>/api/health` 返回 200 后才开始。
...
- `curl -o /dev/null -w '%{http_code}' http://localhost:<隔离端口>/<path>` 断言 200，或 Playwright network 断言。
...
- **环境表里的宿主 PID 绝对禁杀**（第一道防线）；**宿主 gateway 端口识别是第二道防线**：禁止裸 `sbt run`（默认 home + 默认端口会抢宿主）、禁止 kill/信号宿主实例。
- **非宿主进程可按需管理**：隔离实例（非宿主端口）、静态文件服务允许 kill——kill 前 PID 验身（`lsof -ti :<端口>` 定位 + cwd 确认，fork 子进程 cmdline 不含路径）。
- 唯一合法启动方式 = 带完整隔离参数（`--home <隔离 home> --port <隔离端口> --no-browser`）。
- 验收结论（PASS/FAIL + 证据）必须可复现：附命令与原始输出。

## 进程清理纪律

- 三步验证结束**必须清理自己 spawn 的进程**：隔离实例、静态文件服务、Playwright browser 全部关掉——不留给宿主或下一个会话手清。
