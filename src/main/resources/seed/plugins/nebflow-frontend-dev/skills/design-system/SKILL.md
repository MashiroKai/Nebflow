...
5. **踩坑记录**（三轮裁定，Evidence = 规格书版本标注 `message-search spec v2`）
...
6. **附录 · qa 测试基建三坑**（Playwright 隔离实例经验，Evidence = `msg-search-qa 证据集`）
...
**全链证据**：规格书 v2.1 `message-search spec v2`（v3.1 迭代版迁入 docs 后此处补链） · 实现 archive/scala @ 9e2b6d2a（4 commits）· qa 报告 `msg-search-qa 证据集`（report.json + run-a1-a10.mjs + shots/）· 视觉评审 PASS `msg-search visual review`（含交叉验证声明与 F-1 muted 对比度非阻断发现）
...
5. **踩坑记录**（v3.1 被打回「平的/禁 emoji/文字出卡」三点）
   ...
   - ⑤ **滚动可落在祖先级 + 离屏面板制造幻影溢出区**（2026-09-04 Galaxy v2.0 原型 19 断言收尾，归因=规格缺失）：③ 的升级版——(a) 原生 **focus scroll-into-view**（Tab 导航/`el.focus()`，无法 preventDefault，只有 `focus({preventScroll:true})` 可抑）会把视口外聚焦目标的滚动写进**任一** overflow:hidden 祖先，与相机 transform 叠加后「数学全对、落点全错」；(b) 关闭态面板 `translateX(100%+24px)` 的位移**计入可滚动溢出区**，使 `.fm-stage` 这类中间容器变成可编程滚动容器——面板开合过渡中 `dClose.focus()` 落在屏外坐标 → stage scrollLeft=483 → 整个舞台被推出窗，后续所有绝对坐标交互（含自动化 mouse.down）落空 → 教训：**归零防线必须挂满整条 positioned 祖先链**（stage+viewport，不能只挂 viewport），且验收断言前先 `elementFromPoint` 探活落点
