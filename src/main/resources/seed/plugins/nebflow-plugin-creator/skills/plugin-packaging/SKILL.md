...
1. **落盘即生效**（2026-09-13 起装载采用「在位即信任」，**无人工审批环节**）——
...
- 直写目标：`{{data_root}}/plugins/<name>/`（节点直写是既定流程能力；装载为
  「在位即信任」⇒ **落盘即生效**——先把自检做完再落盘，落盘后按纪律②冻结）。
...
## 落盘即生效（已受信；交付前自检 + 报告 digest 与来源）

**本包落盘即生效**：插件装载采用「在位即信任」——`{{data_root}}/plugins/<name>/`
下扫到的包直接被装载，**没有人工审批环节**（作者 2026-09-13 裁定）。所以终检
