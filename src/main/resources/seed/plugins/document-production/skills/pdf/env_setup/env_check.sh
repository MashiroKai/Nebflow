...
# Also resolves and exports SKILL_DIR and FONT_DIR.
...
# ── Resolve SKILL_DIR & FONT_DIR ──
_ENV_CHECK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
SKILL_DIR="$(cd "$_ENV_CHECK_DIR/.." && pwd)"
export SKILL_DIR
...
if [ -x "$SKILL_DIR/scripts/tectonic" ] || command -v tectonic &>/dev/null; then
...
$QUIET || echo "SKILL_DIR=$SKILL_DIR"
