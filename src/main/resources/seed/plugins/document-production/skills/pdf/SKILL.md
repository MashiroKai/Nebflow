---
name: pdf
description: "PDF 文档制作工作台：报告、创意视觉、学术 LaTeX、既有 PDF 处理四条产线，按文档类型自动路由，覆盖报告 / 海报 / 论文 / 简历 / 提取 / 合并 / 拆分 / 表单填写 / 格式转换。用于产出 PDF 交付件、把 HTML 或 LaTeX 渲染成 PDF、对既有 PDF 做加工等任务。"
---
...
python3 "${SKILL_DIR}/scripts/pdf.py" env.fix     # Auto-install missing CORE Python packages
```
...
ALL PDFs must have: Title, Author (default "Nebflow"), Creator, Subject.
...
meta.brand <pdf>             # Add Nebflow metadata
...
All paths are relative to `${SKILL_DIR}` — the single root variable for this skill. Resolve it once before calling any script:

```bash
SKILL_DIR="${SKILL_DIR}"   # ← parent directory of this SKILL.md

# Then all commands use ${SKILL_DIR}:
