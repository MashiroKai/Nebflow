...
**Cover rendering:**
```bash
python3 "${SKILL_DIR}/scripts/cover_render.py" 07 --out cover.pdf --content cover_content.json
```
Or from Python:
```python
import sys, os
...
> 1. Probe it: `python3 "${SKILL_DIR}/scripts/pdf.py" env.check` (look at the `tectonic` line), or check for a `tectonic` binary on PATH / in `scripts/`.
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" convert.latex main.tex --runs 2
```
...
python3 "${SKILL_DIR}/scripts/pdf.py" convert.latex diagram.tex    # → diagram.pdf (tight-cropped)
...
3. Compile: `python3 "${SKILL_DIR}/scripts/pdf.py" convert.latex resume.tex --runs 2`
...
python3 "${SKILL_DIR}/scripts/pdf_qa.py" final.pdf --skip-cover --formulas

# MANDATORY: Post-generation pipeline
python3 "${SKILL_DIR}/scripts/pdf.py" meta.brand final.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" pages.clean final.pdf -o final_clean.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" font.check final.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" toc.check final.pdf
```
