...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" palette.generate --title "<document title>" --mode minimal
```
...
python3 "${SKILL_DIR}/scripts/cover_render.py" 03 --out cover.pdf --content cover_content.json
```
...
def insert_cover(cover_pdf, body_pdf, output_pdf):
    ...
    writer.add_metadata({'/Title': 'Report Title', '/Author': 'Nebflow', '/Creator': 'Nebflow'})
...
```bash
python3 "${SKILL_DIR}/scripts/toc_validate.py" check-pdf output.pdf
```
...
| Fill PDF forms | Process brief | `python3 "${SKILL_DIR}/scripts/pdf.py" form.fill` or annotation workflow |
| Merge PDFs | Process brief | `python3 "${SKILL_DIR}/scripts/pdf.py" pages.merge` |
| Extract text | Process brief | `python3 "${SKILL_DIR}/scripts/pdf.py" extract.text` |
| Extract tables | Process brief | `python3 "${SKILL_DIR}/scripts/pdf.py" extract.table` |
...
4. Run `python3 "${SKILL_DIR}/scripts/toc_validate.py" output.pdf` to verify TOC links
...
- [ ] **Metadata**: Title matches filename, Author/Creator = "Nebflow"
