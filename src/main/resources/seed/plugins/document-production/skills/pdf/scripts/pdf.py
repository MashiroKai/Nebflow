...
def env_check(argv: list):
    ...
    if s_node[0] == "ok" and not shutil.which("node"):
        ...
        print(f'        "{_node_abs}" "${SKILL_DIR}/scripts/html2poster.js" poster.html -o poster.pdf')
...
def meta_brand(argv: list):
    """Add Nebflow branding metadata to PDF documents."""
    ...
    for input_path in argv:
        ...
        if not quiet:
            ...
            print(f"  Title: {title}")
            print("  Author: Nebflow")
            print("  Creator: Nebflow")
            print("  Producer: Nebflow")
            if out != input_path:
                print(f"  Output: {out}")
