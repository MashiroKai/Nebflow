...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" env.check
```

Reports availability but does **not** auto-install. Required: Python 3, pikepdf, pdfplumber.

Entry point: `python3 "${SKILL_DIR}/scripts/pdf.py" <group>.<action> [options]`
...
python3 "${SKILL_DIR}/scripts/pdf.py" extract.text report.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" extract.text report.pdf -p 1-3
python3 "${SKILL_DIR}/scripts/pdf.py" extract.text report.pdf -p 1,4,7

# Tables — returns structured JSON with page/rows/cols/data
python3 "${SKILL_DIR}/scripts/pdf.py" extract.table report.pdf

# Images — dumps embedded rasters to directory
python3 "${SKILL_DIR}/scripts/pdf.py" extract.image report.pdf -o ./images/
```
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" pages.merge a.pdf b.pdf -o combined.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" pages.split book.pdf -o ./chapters/
python3 "${SKILL_DIR}/scripts/pdf.py" pages.rotate doc.pdf 90 -o rotated.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" pages.rotate doc.pdf 180 -o rotated.pdf -p 1-3
python3 "${SKILL_DIR}/scripts/pdf.py" pages.crop doc.pdf 50,50,550,750 -o trimmed.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" pages.clean doc.pdf -o cleaned.pdf
```
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" meta.get doc.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" meta.set doc.pdf -o out.pdf -d '{"Title": "Report", "Author": "Jane"}'
python3 "${SKILL_DIR}/scripts/pdf.py" meta.brand doc.pdf -o branded.pdf
```
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" form.info input.pdf
```
...
python3 "${SKILL_DIR}/scripts/pdf.py" form.fill input.pdf -o filled.pdf \
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" form.detail input.pdf -o fields.json   # full field info (types, options, defaults)
python3 "${SKILL_DIR}/scripts/pdf.py" form.render input.pdf -o ./pages/       # render pages as PNG for visual check
```
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" form.render input.pdf -o ./pages/
```
...
python3 "${SKILL_DIR}/scripts/pdf.py" form.check-bbox fields.json

# Visual validation (red=target, blue=label)
python3 "${SKILL_DIR}/scripts/pdf.py" form.validate 1 fields.json page1.png validation.png
```
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" form.annotate input.pdf fields.json -o filled.pdf
```
...
1. ANALYZE  → Extract design DNA from template:
               - python3 "${SKILL_DIR}/scripts/pdf.py" meta.get template.pdf       (page size)
               - python3 "${SKILL_DIR}/scripts/pdf.py" extract.image template.pdf   (color samples)
               - python3 "${SKILL_DIR}/scripts/pdf.py" extract.text template.pdf    (text structure)
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" convert.office input.docx -o pass1.pdf
```

**Step 2 — Clean Blank Pages**:
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" pages.clean pass1.pdf -o pass1_clean.pdf
```
If `blank_pages_removed == 0`, use pass1.pdf directly.

**Step 3 — Fix TOC**:
```bash
python3 "${SKILL_DIR}/scripts/toc_validate.py" fix-docx input.docx -o fixed.docx
```
...
```bash
python3 "${SKILL_DIR}/scripts/toc_validate.py" fix-pages fixed.docx pass1_clean.pdf -o final.docx
```
...
```bash
python3 "${SKILL_DIR}/scripts/pdf.py" convert.office final.docx -o output.pdf
...
```bash
python3 "${SKILL_DIR}/scripts/toc_validate.py" check-conversion final.docx output_clean.pdf
```
