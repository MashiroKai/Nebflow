...
onMessage('askPermission', (msg, view) => {
  ...
  if (sid && state.bypassSessions.has(sid)) {
    if (view) {
      ...
      renderPermissionPrompt(msg.toolName, msg.summary, msg.input, msg.sessionId, msg.dangerLevel, msg.sourceAgent, msg.sourceSession, msg.requestId);
    } else {
      ...
      saveMsg({ type: 'askPermission', toolName: msg.toolName, summary: msg.summary, input: msg.input, dangerLevel: msg.dangerLevel, autoApproved: true, sourceAgent: msg.sourceAgent, sourceSession: msg.sourceSession, requestId: msg.requestId }, sid);
    }
    return;
  }
  if (sid) setSessionAttention(sid, true);
  ...
  if (view) {
    ...
    // Non-active session: persist so it can be restored on session switch
    saveMsg({ type: 'askPermission', toolName: msg.toolName, summary: msg.summary, input: msg.input, dangerLevel: msg.dangerLevel, sourceAgent: msg.sourceAgent, sourceSession: msg.sourceSession, requestId: msg.requestId }, sid);
  }
});
