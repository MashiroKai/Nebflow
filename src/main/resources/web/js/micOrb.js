...
const FS = [
  ...
  '  /* v8.2.7: bigger + softer background transition. A slower exp decay (4.5->2.8)',
  '     spreads the glow over a wider radius (border range grows), and a lower peak',
  '     amplitude (0.18->0.13) keeps the whole bloom dim so it fades as a gentle',
  '     gradient (no noticeable step at the body->glow transition). The alpha window',
  '     widens to match the longer skirt (1.30->1.46r0) so the glow dies smoothly',
  '     with the skirt instead of hitting a cutoff. Body material (<=0.92r0) and the',
  '     alpha-from-colNoRim coupling (v8.2.4) are untouched. */',
  '  float halo=exp(-max(len-r0*1.02,0.0)*2.8)*(1.0-bodyA);',
  '  float haloA=halo*0.13*(1.0-isLight)*(1.0-smoothstep(r0*1.02,r0*1.46,len));',
  ...
  '  /* v8.2.7: theme-aware feathered skirt — widened for a bigger, softer fade.',
  '     Dark: bloom extends to 1.44r0 (v8.2.6 was 1.28) so the fluorescent gradient',
  '     reaches further and fades over a longer run (no visible edge). Light: kept',
  '     at the v8.2.6 value 1.15r0 — light has NO halo bloom (its "glow" is body color',
  '     bleeding through the feathered alpha), so widening it would lift the glow',
  '     band past the body and blur the silhouette (A1 body-distinct regression).',
  '     The body edge (<=0.92r0) stays opaque (B9); the bloom is a dim, smooth',
  '     gradient - never a bright circular ring / white-base return. */',
  '  float skirtEnd=r0*mix(1.44,1.15,isLight);',
