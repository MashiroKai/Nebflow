...
import nebscala.core.{ReplUi, UserAbort}
import nebscala.shared.TerminalUtils.*
...
class NebscalaUI(store: UiStore):
...
object NebscalaUI:
  def create: IO[NebscalaUI] =
    UiStore.create.map(store => new NebscalaUI(store))
