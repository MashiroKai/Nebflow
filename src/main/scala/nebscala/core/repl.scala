...
import nebscala.shared.{Message, MessageRole, ContentBlock, LlmHandle, LlmRequest, StreamChunk, ToolCall, ToolDefinition}
import nebscala.core.tools.{ToolRegistry, ToolError}
...
object Repl:
  ...
  private val defaultPrompt = """你是 Nebscala 的命令行助手。用户通过终端与你对话，你可以读写项目文件、执行命令来帮用户完成工作。
