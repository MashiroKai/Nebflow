...
case class NebscalaServiceConfig(
  llm: ServiceLlmConfig,
  mcpServers: Option[Map[String, McpServerConfig]] = None,
  search: Option[SearchConfig] = None
)

object NebscalaServiceConfig:
  given Decoder[NebscalaServiceConfig] = deriveDecoder[NebscalaServiceConfig]

object Config:
  val NebscalaHome: os.Path = os.home / ".nebscala"
  val DefaultConfigPath: os.Path = NebscalaHome / "nebscala.json"
  ...
  def loadServiceConfig(configPath: Option[String] = None): NebscalaServiceConfig =
    ...
    resolvedJson.as[NebscalaServiceConfig] match
      case Right(cfg) => cfg
