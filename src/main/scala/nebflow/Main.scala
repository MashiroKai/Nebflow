...
import nebflow.cli.{Args, CliMode, MarkdownRenderer, NebflowUI, Splash, UiStore}
import nebflow.core.{AskUser, Repl, ReplUi, UserAbort}
import nebflow.core.mcp.{McpClient, StdioTransport, HttpTransport, createMcpToolWrapper}
import nebflow.core.tools.ToolRegistry
import nebflow.llm.{Config, LlmInterface, NebflowServiceConfig}
import nebflow.shared.{Message, MessageRole, TerminalUtils}
...
object Main extends IOApp:
  private val SESSION_PATH = os.home / ".nebflow" / "sessions" / "last.json"
        ...
        case input =>
          ...
          if trimmed == "quit" || trimmed == "exit" then
            saveSession(sessionMessages) *>
              IO.println(s"Session saved (${sessionMessages.length} messages). Use nebflow -c to resume.") *>
              IO.pure(ExitCode.Success)
