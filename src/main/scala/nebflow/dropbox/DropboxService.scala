...
final class DropboxService private (
  ...
  private def receiverTempPath(t: FileTransfer): os.Path =
    if t.tempPath.nonEmpty then os.Path(t.tempPath, os.pwd)
    else DropboxUtil.downloadsDir / s".${t.fileName}.dropbox-${t.transferId.take(8)}"
  ...
  private def receiveWholeBodyAsSingleChunk(
    ...
    for
      ...
      res <- transferOpt match
        ...
        case Some(t) =>
          val tempPath = receiverTempPath(t)
          val receiver = new ChunkReceiver(transferId, headers.totalBytes, headers.chunkSize, tempPath)
          for
            _ <- IO.blocking(os.makeDir.all(tempPath / os.up))
            ...
            _ <- applied match
              ...
              case Right(ack) =>
                transfersRef.update(m =>
                  m.get(transferId) match
                    case Some(cur) => m + (transferId -> cur.copy(bytesReceived = ack.bytesReceived, tempPath = tempPath.toString, receiverHash = ack.wholeSha256.getOrElse(cur.receiverHash)))
                    case None => m
                )
            _ <- persistTransfersThrottled
          yield applied match
    ...
    ensureTransfer(transferId, "in").flatMap {
      ...
      case Some(t) =>
        ...
        for
          ...
          _ <- transfersRef.update(m =>
            m.get(transferId) match
              case Some(cur) => m + (transferId -> cur.copy(bytesReceived = state.bytesReceived, tempPath = tempPath.toString))
              case None => m
          )
        yield Right(state)
    }
