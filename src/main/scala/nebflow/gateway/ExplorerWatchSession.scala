...
final class ExplorerWatchSession(send: Json => IO[Unit], log: NebflowLogger, live: Boolean = true):
  ...
  def subscribe(root: os.Path, echoRootPath: String, dirs: Option[List[String]]): IO[Unit] =
    ...
    else if !os.isDir(root) then
      IO.raiseError(new RuntimeException(s"watch root not found or not a directory: $root"))
    else
      for
        ...
        _      <- (registerDir(sub, "").adaptError {
                    case e => new RuntimeException(s"cannot watch root $root: ${e.getMessage}")
                  } *> IO.raiseWhen(declared.size > maxDirsPerRoot)(
                    new RuntimeException(s"too many watch dirs (max $maxDirsPerRoot)")
                  ) *> declared.traverse_(rel => validateRelDir(rel).flatMap(clean => registerDir(sub, clean))
                  )).onError(_ => closeSub(sub))
    ...
    IO.blocking {
      if !sub.closed && !sub.registered.containsKey(rel) then
        ...
        if os.isDir(abs) then
          ...
          ()
    }.void.handleErrorWith(e => log.debug(s"watch register skipped ($rel): ${e.getMessage}"))
  ...
  private def scheduleFlush(sub: ExplorerWatchSession.Sub): IO[Unit] =
    sub.flushScheduled.getAndSet(true).flatMap(wasScheduled =>
      if wasScheduled then IO.unit else (IO.sleep(debounce) *> flush(sub)).start.void
    )
