  ...
  private object RunTask extends CliSubcommand:
    ...
    def run(ctx: CliContext): IO[CliResult] =
      for
        ...
        result <-
          if task.trim.isEmpty then IO.pure(CliResult.Error("No task provided (use -p \"<task>\" or pipe stdin)", 2))
      ...
      for
        ...
        result <- sid match
          case Right(sessionId) =>
            client
              .post(
                s"/api/sessions/$sessionId/turn",
                Json.obj("content" -> task.asJson, "timeoutSec" -> timeoutSec.asJson)
              )
              .map(resp => render(resp, ctx.json))
              .handleErrorWith(e => IO.pure(CliResult.Error(s"turn request failed: ${e.getMessage}", 2)))
