...
private[agent] trait AgentCore:
  ...
  protected def logAgentEvent(
    agentDef: AgentDef, depth: Int, sessionId: Option[String],
    sessionName: Option[String], event: String, detail: String = ""
  ...
  protected def maybeAutoCompact(
    ...
    replyTo: Option[ActorRef[AgentEvent]], processing: ProcessingFn
    ...
    if state.pendingCompaction.isDefined then None
      ...
      val backoffOk =
        if state.compactionFailures == 0 then true
          ...
          if !ok then logAgentEvent(agentDef, depth, state.sessionId, state.sessionName,
      ...
      if !backoffOk then None
        ...
        logAgentEvent(agentDef, depth, state.sessionId, state.sessionName, "auto-compact-skipped",
        ...
        val shouldCompact = inputTokensOpt match
          ...
          case _ =>
            ...
            if estimated > threshold then Some(s"estimated=$estimated threshold=$threshold (provider did not report inputTokens)")
            else None
        shouldCompact match
          case Some(detail) =>
  ...
  protected def startDirectCompaction(
    agentDef: AgentDef, resources: SharedResources, depth: Int,
    parentRef: Option[ActorRef[AgentCommand]], state: AgentState,
    replyTo: Option[ActorRef[AgentEvent]], processing: ProcessingFn, mode: String
    ...
    for
      _ <- ctx.forkTurn(emitStreamIO(state.wsSend,
        AgentStreamEvent.CompactStart(mode, state.latestUsage.map(_.inputTokens),
    ...
    maybeAutoCompact(agentDef, resources, depth, parentRef, state, replyTo, processing) match
      ...
      case None =>
        ...
        val stateForLlm = microResult match
          case Some(compacted) =>
            logAgentEvent(agentDef, depth, state.sessionId, state.sessionName, "fast-micro-compact",
        ...
        val llmIo = for
          ...
          baseSystemStable = buildSystemPrompt(freshDef, resources, turnCtx.systemPrefix,
            turnCtx.projectRoot, turnCtx.rulesMd, state.session.chatWidth)
  ...
  protected def pipeToolExecutions(
    agentDef: AgentDef, resources: SharedResources, depth: Int,
    parentRef: Option[ActorRef[AgentCommand]], state: AgentState,
    result: ConsumeResult, replyTo: Option[ActorRef[AgentEvent]], processing: ProcessingFn
  )(using ctx: ActorContext[AgentCommand]): IO[Behavior[AgentCommand]] =
    ...
    if droppedCalls.nonEmpty then
      NebflowLogger.forName("nebflow.agent").warn(s"Tool calls filtered (not in allowed set): ${droppedCalls.map(_.name).distinct.mkString(", ")}")
    ...
    val io = for
      ...
      toolCtx = ToolContext(
        projectRoot = effectiveProjectRoot, llm = Some(resources.llm),
        sessionStore = Some(resources.sessionStore), agentActorRef = Some(ctx.self),
        contextWindow = state.contextWindow, sessionId = state.sessionId,
        sessionName = state.sessionName, taskStore = Some(resources.taskStore),
        wsSend = Some(state.wsSend), readTracker = state.readTracker,
        fileHistory = state.fileHistory, parentRef = parentRef, depth = depth,
        agentDef = Some(agentDef), agentLibrary = Some(resources.agentLibrary),
        ...
        hookContext = HookContext(sessionId = state.sessionId, projectRoot = effectiveProjectRoot, cwd = effectiveProjectRoot),
        folderId = state.folderId, mailboxAddress = state.session.sessionId,
        dreamSchedulerRef = resources.dreamSchedulerRef, sharedResources = Some(resources),
        actorSystem = Some(ctx.system), messages = state.messages)
      freshResults <- filteredCalls.parTraverse { call =>
        val skipStreaming = call.name == "AskUserQuestion"
        (if !skipStreaming then emitStreamIO(state.wsSend,
          AgentStreamEvent.ToolStart(nebflow.core.summarizeToolCall(call)), isSubagent, sessionIdOpt)
         else IO.unit) *>
          ...
          .map(r => (call, r)).attempt.map {
            ...
            if call.name != "AskUserQuestion" then
              ...
              emitStreamIO(state.wsSend,
                AgentStreamEvent.ToolEnd(nebflow.core.summarizeToolCall(call), summary, frontendContent, r.isError, input = Some(call.input)),
      ...
      _ <- ctx.self ! ToolsComplete(guardedBatch ++ droppedResults, result.text, replyTo, None, result.thinking, result.thinkingSignature)
    yield ()

    for
      _ <- ctx.forkTurn(io.handleErrorWith { e =>
        val msg = Option(e.getMessage).getOrElse(e.getClass.getSimpleName)
        NebflowLogger.forName("nebflow.agent").warn(
          s"pipeToolExecutions failed (this should be rare): $msg")
        ctx.self ! LlmFailed(new RuntimeException(s"Tool execution pipeline failed: $msg"), replyTo, state.currentTurnId)
      })
    yield
        ...
        (Some(deferred),
          (ctx.self ! AgentCommand.SetPermissionDeferred(deferred)).flatMap(_ =>
            IO {
              ...
              val dangerLevel =
                if call.name == "Bash" then call.input("command").flatMap(_.asString).map(nebflow.core.tools.BashTool.dangerLevel).getOrElse(0)
                else if call.name == "Curl" then call.input("method").flatMap(_.asString).map(_.toUpperCase) match
                  case Some(m) if !Set("GET", "HEAD", "OPTIONS").contains(m) => 2
                  case _ => 0
                else 1
              Json.obj("type" -> "askPermission".asJson, "sessionId" -> state.sessionId.asJson,
                "toolName" -> call.name.asJson, "summary" -> summary.asJson,
                "input" -> call.input.asJson, "dangerLevel" -> dangerLevel.asJson)
            }.flatMap { permJson =>
              for
                _ <- state.wsSend(permJson)
                approved <- deferred.get
                result <- if approved then executeTool(call, toolCtx)
    ...
    ToolRegistry.TOOL_MAP.get(call.name) match
      case Some(tool) =>
        ...
        IO.delay(System.nanoTime()).flatMap { start =>
          hookEngine.beforeTool(call.name, call.input, hookCtx).flatMap { preResult =>
                  ...
                  RemoteExecutor.current.get.execute(deviceName, call.name, remoteInput, Some(ctx)).flatMap {
                    case Right(result) =>
                      hookEngine.afterTool(call.name, finalInput, result, true, hookCtx).map { postResult =>
                        ...
                        ToolExecResult(result + (if hookSuffix.nonEmpty then s"\n\n$hookSuffix" else ""), frontendContent = Some(result))
                      }
                    case Left(err) =>
  ...
  protected def buildAllowedToolSet(agentDef: AgentDef, depth: Int = 0): Set[String] =
    ...
    val mcpTools =
      ...
      else agentDef.mcpServers.flatMap { serverId =>
  ...
  private def enrichDelegateTools(
    tools: Option[List[ToolDefinition]], models: List[(String, String, Option[String])]
  ): Option[List[ToolDefinition]] =
    if models.isEmpty then tools
    else tools.map(_.map { td =>
      if td.name == "Delegate" then
        val modelList = models.map { case (ref, _, desc) =>
          s"  - $ref" + desc.map(d => s": $d").getOrElse("")
        }.mkString("\n")
        td.copy(description = td.description + s"\n\nAvailable models (pass as the `model` parameter in \"provider/model\" format):\n$modelList")
      else td
    })

  protected def emitStream(
    wsSend: io.circe.Json => IO[Unit], event: AgentStreamEvent,
    isSubagent: Boolean = true, sessionId: Option[String] = None
  ...
  protected def streamEmitter(
    wsSend: io.circe.Json => IO[Unit], isSubagent: Boolean = true,
    sessionId: Option[String] = None, isAskMode: Boolean = false,
    ...
    stream => stream.evalTap {
      case StreamChunk.TextDelta(delta) if delta.nonEmpty && !isCompactTurn =>
        val json = if isAskMode then Json.obj("type" -> "askTextDelta".asJson, "sessionId" -> sessionId.asJson, "delta" -> delta.asJson)
      ...
      case StreamChunk.ThinkingDelta(delta) if delta.nonEmpty && !isCompactTurn =>
        val json = if isSubagent then AgentStreamEvent.Thinking.toJson(ctx.self.path.name, true, None)
      ...
      case StreamChunk.ToolCallStart(name) if name != "AskUserQuestion" && !isCompactTurn =>
        val json = if isSubagent then AgentStreamEvent.ToolCallDetected(name).toJson(ctx.self.path.name, true, None)
      ...
      case StreamChunk.ToolCallChunk(tc) if tc.name != "AskUserQuestion" && !isCompactTurn =>
        val json = if isSubagent then AgentStreamEvent.ToolStart(nebflow.core.summarizeToolCall(tc)).toJson(ctx.self.path.name, true, None)
        else Json.obj("type" -> "toolStart".asJson, "sessionId" -> sessionId.asJson, "label" -> nebflow.core.summarizeToolCall(tc).asJson)
        wsSend(json)
      case _ => IO.unit
    }
  ...
  protected def aggregateChunks(chunks: List[StreamChunk]): ConsumeResult =
    ...
    val usage = chunks.collectFirst { case StreamChunk.Done(_, Some(u), _, _) if u.inputTokens > 0 => u }
    ...
    val model = chunks.collectFirst { case StreamChunk.Done(_, _, Some(meta), _) => s"${meta.providerId}/${meta.model}" }
    val contextWindow = chunks.collectFirst { case StreamChunk.Done(_, _, _, cw) => cw }.flatten
    ConsumeResult(text, toolCalls, Nil, stopReason, usage, Option.when(thinking.nonEmpty)(thinking), thinkingSignature, model, contextWindow)

  protected def buildSystemPrompt(
    agentDef: AgentDef, resources: SharedResources, systemPrefix: String,
    sessionProjectRoot: Option[String] = None, sessionRulesMd: Option[String] = None, chatWidth: Int = 0
  ): String =
    ...
    if now - lastUpdate < 30000 then cached
      ...
      val refreshed = RemoteExecutor.current.flatMap(_.meshServiceOpt).flatMap { ms =>
        try
          ...
          }.map(_.mkString(" ")) else Nil
          val allDevices = (localStr :: peerStrs).mkString("; ")
          val deviceHint = if loggedIn && peersList.nonEmpty then
            "\nEach tool accepts a `device` parameter. Select the appropriate device for each task." else ""
