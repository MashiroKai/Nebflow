...
object TaskStuckWatcher:
    ...
    if inflight > 0 then
      ...
      StuckClassification(
        ...
        s"tool '${a.toolName.getOrElse("?")}' " +
          (if overAuthorised then s"over its authorised window ($toolNote > $authorisedNote) but still advancing"
  ...
  def assessDetailed(
    ...
    if agentStale && toolOverdue then
      Some(
        StuckAssessment(
          math.max(agentIdleMs, toolPhaseMs) / 1000,
      ...
      Some(
        StuckAssessment(
          agentIdleMs / 1000,
          s"agent idle ${agentIdleMs / 1000}s (no LLM/tool event)",
          BranchAgentStale,
          agentIdleMs,
          toolPhaseMs,
          rec.currentToolName
      ...
      Some(
        StuckAssessment(
          toolPhaseMs / 1000,
          s"tool '$toolLabel' running ${toolPhaseMs / 1000}s$deadlineNote in an unfinished turn",
          BranchToolOverdue,
          agentIdleMs,
          toolPhaseMs,
          rec.currentToolName
  ...
  def recoveryGate(rec: AgentRecord, led: RecoveryLedger, now: Long): Option[RecoveryGate] =
    if rec.status == AgentStatus.Frozen || rec.frozenReason.contains(LoopFreezeReasonWire) then
      ...
      Some(RecoveryGate(GateRecoveryLoopDetected, true, "互斥点 2 已命中（恢复后循环指纹再次命中）—— 恢复链已停，不再消耗预算"))
    else if led.lastRecoveryAt > 0L &&
      (now - led.lastRecoveryAt) < nebflow.shared.Defaults.StuckRecoveryCooldownMs
    then
      ...
      (now - led.lastAttemptAt) < nebflow.shared.Defaults.stuckRecoveryBackoffMs(led.chainAttempts + 1)
    then
  ...
  private def gateActions(
    ...
    val tried: List[String] =
      if led.chainAttempts > 0 then
        List(
          s"suspend → hard-resume leg ×${led.chainAttempts} (each logged as a stuck-recovery-attempt event; " +
             ...
             stopCounts.get.flatMap { counts =>
               ledger.update(m => m.updated(rec.sessionId, led.copy(reportedAt = System.currentTimeMillis()))) *>
                 reportExhausted(
                   ...
                   assessment,
                   cls,
                   counts.getOrElse(rec.sessionId, 0),
                   tried,
                  ...
                  .flatMap { n =>
                    logger.warn(
                      ...
                      (if n > 0 && cls.destructiveAllowed then
                         nebflow.core.tools.BgTaskRegistry
                           .reclaimSession(Some(rec.sessionId), wsHub.broadcast, rec.rootSessionId)
                           .handleErrorWith(e =>
                             logger
                               .warn(s"TaskStuckWatcher: session reclaim for ${rec.sessionId} failed: ${e.getMessage}")
                           )
                       else
            ...
            if !hard then
                      ...
                      act(
                        ...
                        then
                          nebflow.core.node.NodeRunner
                            ...
                            .handleErrorWith(e =>
                              logger.warn(
                                s"TaskStuckWatcher: panel done frame for ${rec.sessionId} failed: ${e.getMessage}"
                              )
                            )
                        else IO.unit
                      ) *>
              ...
              attempts match
                case 1 =>
                  ...
                  ) *> recordFire("L1", attempts) *> act(broadcastStuck(wsHub, rec, idleSecs, "halt", reason)) *> act(
                    hardCancel()
                  )
                case 2 =>
                  ...
                  ) *> recordFire("L2", attempts) *> act(
                    broadcastStuck(wsHub, rec, idleSecs, "hard-abort", reason)
                  ) *> act(transportAbort())
                ...
                case 3 =>
                  ...
                  logger.warn(
                  ...
                  ) *> classifyNote *> recordFire("L3", attempts) *> act(
                    ...
                    act(
                      ...
                      then
                        nebflow.core.node.NodeRunner
                          ...
                          .handleErrorWith(e =>
                            logger
                              .warn(s"TaskStuckWatcher: panel done frame for ${rec.sessionId} failed: ${e.getMessage}")
                          )
                      else IO.unit
                    ) *>
                    ...
                    pendingL3.update(
                      _.filterNot(_.sessionId == rec.sessionId) :+
                        PendingL3(
                          rec.sessionId,
                          rec.rootSessionId,
                          System.currentTimeMillis(),
                          assessment.branch,
                          assessment.toolPhaseMs,
                          assessment.agentIdleMs,
                          attempts
                        )
                    ) *>
                          ...
                          logger.warn(
                            ...
                            (if n > 0 && cls.destructiveAllowed then
                               nebflow.core.tools.BgTaskRegistry
                                 ...
                                 .handleErrorWith(e =>
                                   logger.warn(
                                     s"TaskStuckWatcher: session reclaim for ${rec.sessionId} failed: ${e.getMessage}"
                                   )
                                 )
                             else
                               logger.warn(
                      ...
                      case Some(sup) =>
                        logger.warn(
                          ...
                          (sup ! AgentEvent.Cancelled(
                            ...
                            logger.warn(
                              s"TaskStuckWatcher: supervisor Cancelled for ${rec.sessionId} failed: ${e.getMessage}"
                            )
                          ) *>
                ...
                classifyNote *>
                  ...
                  act(
                    (rec.ref ! AgentCommand.Stop(s"stuck-task-${rec.sessionId}"))
                      .handleErrorWith(e =>
                        logger
                          .warn(s"TaskStuckWatcher: Stop to stuck sub-agent ${rec.sessionId} failed: ${e.getMessage}")
                      )
                  )
              }
        case None =>
                  ...
                  case 1 =>
                    (
                      ...
                      logger.warn(
                        ...
                        .void
                        .handleErrorWith(e =>
                          logger.warn(s"TaskStuckWatcher: L1 halt for ${rec.sessionId} failed: ${e.getMessage}")
                        )
                    )
                  case 2 =>
                    (
                      ...
                      logger.warn(
                        ...
                        (if !nebflow.core.project.NodeEngine.sessionLevelPrimitiveAllowed(rec) then
                           ...
                           nebflow.llm.LlmInterface
                             ...
                             .flatMap { n =>
                               logger.warn(
                                 s"TaskStuckWatcher: L2 aborted $n in-flight LLM request(s) of ${rec.sessionId}"
                               ) *>
                                 ...
                                 (if n > 0 && cls.destructiveAllowed then
                                    nebflow.core.tools.BgTaskRegistry
                                      ...
                                      .handleErrorWith(e =>
                                        logger.warn(
                                          s"TaskStuckWatcher: L2 reclaim for ${rec.sessionId} failed: ${e.getMessage}"
                                        )
                                      )
                                  else
                                    logger.warn(
                             ...
                             .handleErrorWith(e =>
                               logger.warn(s"TaskStuckWatcher: L2 abort for ${rec.sessionId} failed: ${e.getMessage}")
                             ))
                    )
                  case 3 =>
                    (
  ...
  private def verifyL3Outcomes(
    ...
    pendingL3
      ...
      .handleErrorWith(e =>
        logger.warn(
          s"TaskStuckWatcher: L3 outcome self-check failed (audit-only): ${Option(e.getMessage).getOrElse(e.toString)}"
        )
      )
  ...
  private def checkL3Outcome(resources: SharedResources, p: PendingL3, l3VerifyDelayMs: Long): IO[Unit] =
    ...
    runtimeOwning(p.sessionId)
          ...
          for
            ...
            _ <- verdict match
              case Some((evidence, nodeId)) =>
                logger.error(
                  s"TaskStuckWatcher: L3 INEFFECTIVE for ${p.sessionId} (evidence=$evidence, node=${
                      if nodeId.isEmpty then "-" else nodeId
                    }) — " +
      ...
      .handleErrorWith(e =>
        logger.warn(
          s"TaskStuckWatcher: L3 self-check for ${p.sessionId} failed: ${Option(e.getMessage).getOrElse(e.toString)}"
        )
      )
      ...
      if genExhausted || chainExhausted then
        val which =
          if chainExhausted then
            ...
            s"per-generation budget exhausted (${led0.genAttempts}/${nebflow.shared.Defaults.StuckRecoveryMaxPerGen})"
        logger.error(
        ...
        val led1 =
          led0.copy(genAttempts = led0.genAttempts + 1, chainAttempts = led0.chainAttempts + 1, lastAttemptAt = now)
                        ...
                        rt.engine.hardResumeNode(rec.sessionId, Some(anchor), onResumed).flatMap {
                          ...
                          case None =>
                            ...
                            reportExhausted(
                              ...
                              cls,
                              attempts,
                              List("suspend → hard-resume CAS/transcript leg"),
  ...
  private def awaitSessionDrained(resources: SharedResources, sessionId: String, timeoutMs: Long): IO[Boolean] =
    ...
    go.handleErrorWith(e =>
      logger
        .warn(
          s"TaskStuckWatcher: suspend confirmation poll for $sessionId failed: ${Option(e.getMessage).getOrElse(e.toString)}"
        )
        .as(false)
    )

  end awaitSessionDrained
