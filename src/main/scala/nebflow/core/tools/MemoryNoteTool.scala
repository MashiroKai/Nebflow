 ...
 * 零落盘」不符；旧名字面在本仓已零残留，更名史实见对应 commit 与批次报告）——
 * 记忆变更的**记账**工具：四 action 语义不变，落点从「记忆文件」改为 append-only
 * 队列 `<dataRoot>/memory/queue.jsonl`（[[MemoryQueue]]）。写盘时机 = **下一次上下文
 * 压缩**的记忆整理轨（[[nebflow.agent.MemoryTrack]] 起的 `memory-consolidator` 会话），
 * 本工具**零落盘**（2026-09-12 改造批「保名换语义」落地，spec §5 R2 O-A）。
 *
 * 2026-09-17 更名**只换字面**：四 action / 授权与剥离面 / 队列与预算语义零变化；
 * 授能剥离集（`AgentCore.NebulaExclusiveTools` / `DreamAdmittedTools` / registry 映射 /
 * 本工具内身份串）逐处同步改名，**集成员与语义逐字保留**；`MEMORYEDIT_*` 错误码域不随更名。
 *
 ...
 */
object MemoryNoteTool extends Tool:
...
## Queue
- Ledger: ${PathUtil.dataRootRenderValue}/memory/queue.jsonl (append-only JSONL; `note` = queued request, `outcome` = a verdict on a note: applied / modified / rejected / obsolete / deduped / timeout / notrun / blocked / applied-by-reconcile). Change history: ${PathUtil.dataRootRenderValue}/memory/history.jsonl.
- Verdict classes: `applied`/`modified`/`obsolete`/`deduped`/`applied-by-reconcile` are TERMINAL (the note is closed). `rejected`/`timeout`/`notrun`/`blocked` are RETRYABLE (the note stays pending and is retried on the next compaction) — `rejected` means "the consumer ran and could not land it now", `notrun`/`blocked` mean "the engine never got to run" (infra, never written as `rejected`), and `applied-by-reconcile` is the engine's own terminal verdict on a run the hard timeout truncated (judged against the files as they stand after the write).
...
- **A second ALERT covers a different silent case**: a pending note whose target layer has no memory file (a project layer that was never initialised) cannot be located, so nothing is created for it and no terminal verdict is written — it keeps its retryable status and stays pending until that layer exists. The injected queue summary names the layers that are missing; registering the project / creating the layer is what unblocks those notes.
...
- Injection is NEVER truncated (ruling 2026-09-05 §3.3): the write-side gate is the only enforcement, so an over-budget memory taxes every future session."""
  ...
  private def queuedText(
    ...
    val head =
      ...
      else s"""MemoryNote queued ${res.id} (applied at next compaction)."""
    val lines = List(
      head,
