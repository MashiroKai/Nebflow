...
(1) role=task (execution node) — (a) FINISH: declare completion explicitly (OPTIONAL: completing is the default behaviour, so a normal finish needs no report; use it when you want the completion to be explicit); (b) BLOCKED: your task CANNOT be completed (upstream deliverables not ready, underspecified task, capability mismatch, missing external condition) — pass a specific blocked category when you know it, or the generic "blocked"; the engine marks the node blocked and re-engages the dispatcher. A task node CANNOT report pass/fail ('pass' / 'fail' are illegal in this role: NODE_REPORT_CATEGORY_ROLE): an execution failure is decided by the engine, not declared by you.
(2) role=verifier (verification node) — (a) PASS: you formally declare the target verified; (b) FAIL: you formally declare the target REJECTED — this is a VERDICT about the object you judged, NOT node failure: the engine keeps THIS node completed and routes the fail verdict along the "(fail)<target>:loop" edge (the target re-runs). (c) BLOCKED works exactly as above. 'finish' is illegal in this role (NODE_REPORT_CATEGORY_ROLE): a verifier declares a verdict about the object it judged, never its own completion.
Either role: BLOCKED semantics (visible terminal state + dispatcher re-entry + blockCount) are unchanged. Unreported means never terminalized: the node stays running, its result is not delivered downstream, and it is only reminded on a ladder — never auto-failed. Never report "failed" — engine-side execution failure has no declaration channel.
