...
object NodeEditTool extends Tool:
  ...
  /** 观测面上下文经济学批（20260907 裁定⑤b）：描述 7,438 → 3,742 字符（-50%，
    * 压措辞不改语义——参数面/动作语义/错误码/校验/重激活规则全保留；该描述随
    * tools 数组进分发器每次请求。长度上限由 NodeSchemaSlimSpec 断言钉住）。 */
  val description =
...
- abandon (optional, default false): terminal (blocked/completed/failed/cancelled), wiring/pending, or STALE running (dead session) node → cancelled + display TTL. The give-up exit for blocked. LIVE running refused — use NodeCancel.
- notifyDispatcher (optional, default false): on terminal state trigger a dispatcher session with this node's result reference (independent channel, no out-edge cost). Settable while wiring/pending/running; dispatcher-created nodes default false.
...
- Validation (0 spawn except worktree): description rules; referenced nodes exist; DAG cycle check; running target → input frozen. abandon → cancelled + display TTL (result kept in archive). Result = agent's final output, auto-saved per-node, delivered along out ("Nebula" → root session). Full result: NodeList(detail=<nodeId>)."""
