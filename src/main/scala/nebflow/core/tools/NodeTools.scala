                      ...
                      rt.store.archiveSnapshot.flatMap { arch =>
                        arch.nodes.values.find(_.name == nodename) match
                          ...
                          case None =>
                            ...
                            else createNode(rt, nodename, task, description, worktree.flatMap(_.asBoolean), preset, maxRetries, pluginsForCall, inJson, depsJson, outJson, hold)
                      }
                  }
              }
          }
                    ...
                    case Some(true) =>
                      ...
                      if !isGitRoot then
                        ...
                        IO.blocking(createWorktreeFor(ws, nodename)).flatMap {
                          ...
                          case Right(bare) => proceed(rt, nodename, agentName, task, description, Some(bare), preset, maxRetries, plugins, ins, deps, out, hold)
                        }
                    case _ => proceed(rt, nodename, agentName, task, description, None, preset, maxRetries, plugins, ins, deps, out, hold)
              }
