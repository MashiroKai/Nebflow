...
- IMPORTANT: You MUST include a "Sources:" section listing all relevant URLs"""
    ...
    if results.isEmpty then
      ...
      linkRegex.findAllMatchIn(html).foreach { m =>
        if count < 8 then
          ...
          if text.length > 15 && href.startsWith("http") && !href.contains("bing") && !href.contains("baidu") && !href
              .contains("google")
          then
            results += s"**$text**\n$href\n"
            count += 1
  ...
  private def searchOne(query: String, engine: SearchEngine): Either[String, String] =
    try
      ...
      try
        ...
        if !response.code.isSuccess then Left(s"HTTP ${response.code}")
        else
          val html = response.body
          val text = extractSearchResults(html, engine.name)
          if text.trim.isEmpty || text.trim.length < 50 then Left("No meaningful results extracted")
          else Right(text)
  ...
  def call(input: JsonObject, ctx: ToolContext): IO[Either[ToolError, String]] = IO.blocking {
    ...
    val found = enginesToTry.iterator
      .map { engine =>
    ...
    found match
      case Some((engine, text)) =>
        ...
        if blocked.nonEmpty then result = result.split("\n").filter(l => !blocked.exists(l.contains)).mkString("\n")
        if allowed.nonEmpty then result = result.split("\n").filter(l => allowed.exists(l.contains)).mkString("\n")
