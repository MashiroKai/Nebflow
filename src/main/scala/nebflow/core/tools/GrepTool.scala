...
- Use -i for case insensitive search"""
  ...
  def call(input: JsonObject, ctx: ToolContext): IO[Either[ToolError, String]] = IO.blocking {
    ...
    // 都不得把缺省根打回 user.dir（design §4.4 S2 验收）。无会话根（非项目会话 /
    // enabled=false 回退态）时保持旧行为 = user.dir。
    ...
    FileSandbox.checkReadRoot(ctx, os.Path(rawSearchRoot)) match
      ...
      case Right(canonicalRoot) =>
        ...
        runGrep(pattern, canonicalRoot.toString, mode, offset, effectiveLimit, workDir, workDirPath, input)
  }
