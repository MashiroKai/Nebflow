...
class UpdateOrchestrator(
  ...
  def request(
    ...
    if !req.confirm then
      IO.pure(UpdateAdmission.Refused(
  ...
  private def runChain(
    ...
    for
      ...
      frozen <- (hotRestart match
        ...
        case None     => IO.pure(Left(HotRestart.DrainFailure.Busy("hot-restart orchestrator is unavailable in this instance")): Either[HotRestart.DrainFailure, Unit]))
...
object UpdateOrchestrator:
  ...
  def defaultInstall(channel: UpdateChannel): IO[Either[String, String]] =
    val overrideCmd =
      sys.props.get("nebflow.hotupdate.installCommand").map(_.trim).filter(_.nonEmpty)
    ...
    overrideCmd match
      ...
      case Some(cmd) =>
        ...
        IO.blocking {
          ...
          IO.pure(
            Left(
              s"verification install command error: ${Option(e.getMessage).getOrElse(e.toString)}")))
      ...
      case UpdateAdmission.AlreadyInFlight(key, phase) =>
        Some(
          Json.obj(
            ...
            "detail" -> "an update with the same idempotency key is already in flight (merged, not queued)"
              .asJson
          ))
