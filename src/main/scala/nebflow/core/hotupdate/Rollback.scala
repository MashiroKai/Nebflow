...
import nebflow.core.hotrestart.{HotRestart, HotRestartIntent, InstallPointer, SuccessorGate}
...
final class Rollback(
  ...
  spawnPrevious: (HotRestartIntent, String) => IO[Either[String, HotRestart.SpawnedProcess]] =
    (intent, jar) => HotRestart.defaultSpawnWithJar(intent, Some(jar)),
  host: String = "0.0.0.0",
  port: Int = 8080
):
  ...
  def run(reason: String): IO[Either[String, Rollback.Outcome]] =
    InstallPointer.readCurrent(installDir).flatMap { current =>
      InstallPointer.readPrevious(installDir).flatMap { previous =>
        (current, previous) match
          ...
          case (_, None) =>
            IO.pure(Left(
              s"no previous-version pointer under ${installDir.toString} — no retained previous version"))
      ...
      case Right(jar) =>
        ...
        for
          ...
          _ <- spawned match
            ...
            case Right(p) =>
              logger.warn(
                s"[hotupdate] ROLLBACK relaunch: previous version $target spawning (pid ${p.pid}, jar $jar)")
        yield spawned match
