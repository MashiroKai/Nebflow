...
final class ProbeEndpoint private (server: com.sun.net.httpserver.HttpServer, val port: Int):
  ...
  def stop: IO[Unit] =
    IO.blocking(server.stop(0)).void.handleErrorWith(e =>
      IO(ProbeEndpoint.logger.warn(s"[hot-restart] probe endpoint stop failed: ${msg(e)}")))
...
object ProbeEndpoint:
  ...
  def start(monitor: nebflow.llm.ProviderHealthMonitor): IO[Either[String, ProbeEndpoint]] =
    IO.blocking {
      ...
      IO.pure(Left(s"probe endpoint unavailable: ${Option(e.getMessage).getOrElse(e.toString)}")))
...
object HealthCheck:
        ...
        def loop(deadline: Long): IO[HealthTierResult] =
          probe(port).flatMap {
            case false =>
              val now = System.currentTimeMillis()
              if now >= deadline then
                IO.pure(finish(HealthOutcome.Fail, s"nothing accepted on the announced probe port $port within ${deadlineMs}ms"))
              else IO.sleep(pollMs.millis) *> loop(deadline)
            case true =>
              HealthHttp.fetch("127.0.0.1", port, math.max(500L, deadlineMs - (System.currentTimeMillis() - started)).toInt).flatMap {
                case Right(payload) =>
                  IO.pure(finish(HealthOutcome.Pass, s"probe port $port served HTTP 200 ${ProbeEndpoint.Path} (version=${HealthPayload.versionOf(payload).getOrElse("<absent>")})"))
                case Left(err) =>
                  val now = System.currentTimeMillis()
                  if now >= deadline then
                    IO.pure(finish(HealthOutcome.Fail, s"probe port $port accepted but did not serve ${ProbeEndpoint.Path} within ${deadlineMs}ms: $err"))
                  else IO.sleep(pollMs.millis) *> loop(deadline)
              }
          }
        for
          r <- loop(System.currentTimeMillis() + deadlineMs)
            .handleErrorWith(e => IO.pure(finish(HealthOutcome.Fail, s"port probe raised: ${Option(e.getMessage).getOrElse(e.toString)}")))
        yield r
  ...
  private def tierVersionMatch(
    ...
    if expected.isEmpty then
      ...
      IO.pure(finish(
        HealthOutcome.Unverified,
        "no usable loopback probe endpoint — version comparison not performed"))
    else
      def loop(deadline: Long, payload: Option[Json]): IO[HealthTierResult] =
        ...
        observed match
          ...
          case None =>
            val now = System.currentTimeMillis()
            if now >= deadline then
              IO.pure(finish(HealthOutcome.Fail, s"no version-bearing health payload from the successor within ${deadlineMs}ms"))
            else
  ...
  def atDoor(
    ...
    for
      ...
      r2 <- if r1.outcome == HealthOutcome.Fail then IO.pure(HealthTierResult(HealthTier.DoorReceipt, HealthOutcome.Unverified, "skipped: first tier failed", 0L))
            else tierReceipt(readIntent, t2DeadlineMs, pollMs)
      intent <- readIntent
      probePort = intent.flatMap(_.probePort)
      r3 <- if r1.outcome == HealthOutcome.Fail || r2.outcome == HealthOutcome.Fail then
              IO.pure(HealthTierResult(HealthTier.PortServing, HealthOutcome.Unverified, "skipped: an earlier tier failed", 0L))
            else
              tierPortServing(probePort, connectProbe, t3DeadlineMs, pollMs, isAlive)
      cached <- probePort.filter(_ => r3.outcome == HealthOutcome.Pass).traverse(p => HealthHttp.fetch("127.0.0.1", p, 1500)).map(_.flatMap(_.toOption))
      r4 <- if r1.outcome == HealthOutcome.Fail || r2.outcome == HealthOutcome.Fail || r3.outcome == HealthOutcome.Fail then
              IO.pure(HealthTierResult(HealthTier.VersionMatch, HealthOutcome.Unverified, "skipped: an earlier tier failed", 0L))
            else
              tierVersionMatch(probePort, connectProbe, expected, t4DeadlineMs, pollMs, cached)
        ...
        payload.flatMap(HealthPayload.versionOf) match
          ...
          case None =>
            IO.pure(HealthTierResult(
              ...
              0L))
    for
      r1 <- IO.pure(HealthTierResult(HealthTier.SuccessorAlive, HealthOutcome.Pass, "this process is serving (self)", 0L))
      r2 <- IO.pure(HealthTierResult(HealthTier.DoorReceipt, HealthOutcome.Pass, "handover completed (self)", 0L))
      r3 <- serving
      cached <- (if r3.outcome == HealthOutcome.Pass then HealthHttp.fetch("127.0.0.1", port, 1500) else IO.pure(Left("skipped")))
                  .map(_.toOption)
