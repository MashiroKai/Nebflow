...
class HotRestart(
  ...
  private def signal(outcome: Option[HotRestart.HandoverSignal], result: Either[HotRestart.HandoverFailure, Unit]): IO[Unit] =
  ...
  private def requestRestartInternal(
    ...
    if HotRestart.isDisabled then
      IO.pure(Left(HotRestart.HandoverFailure(
                  ...
                  if q.isIdle then begin(source, outcome).start.as(Right(()))
                    ...
                    mode match
                      case RestartMode.RejectIfBusy =>
                        // 放弃占位 + 五域明细立即回报（验收 2）
                        HotRestart.inProgressRef.set(false) *>
                          IO.pure(Left(HotRestart.HandoverFailure(HotRestart.FailureKind.Busy, s"busy — ${q.detail}")))
                      ...
                      case Right(proc) =>
                        ...
                        IO.sleep(timing.c1GraceMs.millis) *>
                          proc.isAlive.flatMap { alive =>
                            if !alive then
                              abort(Some(intent), s"C1: successor (pid ${proc.pid}) died within ${timing.c1GraceMs}ms of spawn", outcome)
                                    ...
                                    healthGate(proc, intent).flatMap { report =>
                                      if !report.isHealthy then
                                        ...
                                        proc.destroy.attempt.void *>
                                          ...
                                          abort(Some(intent), reason, outcome, HotRestart.FailureKind.HealthCheckFailed).as(Left(reason))
                                      else
  ...
  private def pointerRollbackOnHealthFailure(reason: String): IO[Unit] =
    installDir match
      ...
      case Some(dir) =>
        InstallPointer.readCurrent(dir).flatMap {
