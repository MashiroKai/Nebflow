...
object SuccessorGate:
  ...
  def readIntent(path: os.Path): IO[Option[HotRestartIntent]] =
    IO.blocking(if os.exists(path) then Some(os.read(path)) else None).flatMap {
      ...
      case Some(raw) =>
        IO.fromOption(io.circe.parser.decode[HotRestartIntent](raw).toOption)(new Exception("undecodable"))
          .map(Some(_))
  ...
  def markPhase(
    ...
    readIntent(path).flatMap {
      case None => IO.unit
      case Some(intent) =>
        writeIntent(path, intent.copy(phase = phase, failure = failure, probePort = probePort.orElse(intent.probePort)))
    }
  ...
  def validate(
    ...
    else if ctx.host != expectedHost then Left(s"intent host '${ctx.host}' != resolved gateway host '$expectedHost'")
