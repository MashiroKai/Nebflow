...
object InstallPointer:
  ...
  def defaultInstallDir(env: Map[String, String] = sys.env, home: String = sys.props.getOrElse("user.home", "")): os.Path =
  ...
  private def readTrimmed(path: os.Path): IO[Option[String]] =
    IO.blocking(if os.exists(path) then Some(os.read(path).trim) else None)
      ...
      .handleErrorWith(e =>
        IO(logger.warn(s"[hotupdate] pointer read failed ${path.toString}: ${msg(e)}")).as(None))
    ...
    readCurrent(dir).flatMap { cur =>
      readPrevious(dir).flatMap { prev =>
        (cur, prev) match
          ...
          case (_, None) =>
            IO.pure(Left(s"no previous-version pointer at ${previousPath(dir).toString} — no rollback target on disk"))
      }
  ...
  def jarIn(dir: os.Path, version: String): IO[Either[String, String]] =
    IO.blocking {
      ...
      case Some(p) => Right(p)
      case None =>
        Left(s"no package for version '$version' under ${versionDir(dir, version).toString} (version retention missing?)")
  ...
  def layout(dir: os.Path = InstallPointer.defaultInstallDir()): IO[Layout] =
    (readCurrent(dir), readPrevious(dir),
