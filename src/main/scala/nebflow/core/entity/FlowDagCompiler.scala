...
object FlowDagCompiler:
  ...
  def validate(flow: FlowDagDef, agentNames: Set[String]): CompileResult =
    ...
    if flow.nodes.isEmpty then
      err(
        ...
        None,
        "nodes",
        "must be a JSON object with ≥2 node entries",
    ...
    flow.nodes.keys.filterNot(id => idRe.matches(id)).toList.sorted.foreach { id =>
      err(
        ...
        Some(id),
        "id",
        s"node id must match [a-zA-Z0-9_-]+ (got '$id')",
    ...
    flow.nodes.toList.sortBy(_._1).foreach { (id, node) =>
      if node.agent.trim.isEmpty then
        err("003", Some(id), "agent", "must be a non-empty string naming a global agent", "set \"agent\": \"<name>\"")
      if node.maxRetries < 0 then
        err(
          "010",
          Some(id),
          "maxRetries",
          s"must be a non-negative integer (got ${node.maxRetries})",
          "number of restart attempts"
        )
    }
    if flow.entry.trim.isEmpty then
      err("006", None, "entry", "must name a node in nodes", "set \"entry\": \"<node-id>\"")
    if flow.maxFanout <= 0 || flow.maxFanout > MaxFanoutHardLimit then
      err(
        "007",
        None,
        "maxFanout",
        s"must be a positive integer ≤ $MaxFanoutHardLimit (got ${flow.maxFanout})",
    ...
    if flow.maxLoop < 1 then
      err(
        ...
        None,
        "maxLoop",
        s"must be a positive integer (got ${flow.maxLoop})",
        "loop protection bound for redo cycles"
      )
    if flow.name.trim.isEmpty then
      err("017", None, "name", "a non-empty display name is required for WS/UI events", "pass \"name\" to FlowExecute")
      ...
      node.onComplete match
        ...
        case Parallel(Nil, _) =>
          err(
            ...
            Some(id),
            "onComplete.parallel",
            "fan must list at least one target",
      ...
      FlowStructure.routeTargets(node.onComplete).foreach { (target, _) =>
        if target != FlowStructure.ReturnNode && !nodeIds.contains(target) then
          err(
            ...
            Some(id),
            "onComplete",
            s"routes to unknown node '$target'",
          ...
          expr match
            case switchExprRe(snode, _) if !nodeIds.contains(snode) =>
              err(
                ...
                Some(id),
                "onComplete.switch",
                s"expression '$expr' references unknown node '$snode'",
            ...
            case _ =>
              err(
                ...
                Some(id),
                "onComplete.switch",
                s"expression '$expr' is not of the form $$<node>.$$<field>",
                "point the switch at a declared node's verdict (e.g. \"$$agg.verdict\")"
              )
        case _ => ()
    }
    ...
    FlowStructure
      ...
      .foreach { (t, owners) =>
        err(
          ...
          Some(t),
          "",
          s"referenced as dynamic template by ${owners.size} fans (${owners.mkString(", ")})",
    ...
    flow.nodes.toList.sortBy(_._1).foreach { (id, node) =>
      ...
      allSlotsRe.findAllMatchIn(node.input).foreach { m =>
        ...
        if !templateNodes.contains(t) then
          ...
          val known =
            if declaredOutputs(t).isEmpty then "(none)" else declaredOutputs(t).keys.toList.sorted.mkString(", ")
          err(
            "303",
            Some(id),
            "input",
      ...
      outRe.findAllMatchIn(node.input).foreach { m =>
        if !nodeIds.contains(m.group(1)) then
          err(
            ...
            Some(id),
            "input",
            s"output reference '$$${m.group(1)}.output' — node '${m.group(1)}' does not exist",
      ...
      paramsRe.findAllMatchIn(node.input).foreach { m =>
        if !flow.params.contains(m.group(1)) then
          ...
          err(
            ...
            Some(id),
            "input",
            s"parameter reference '$$params.${m.group(1)}' — flow declares no param '${m.group(1)}' (declared: $known)",
      ...
      node.onComplete match
        case pd: ParallelDynamic =>
          pd.slotField match
            case slotsRe(src, f) =>
              checkSlotRef(id, "onComplete.parallel.slots", src, f, requireArray = true, allowSelf = true)
            case literal =>
              checkSlotRef(id, "onComplete.parallel.slots", id, literal, requireArray = true, allowSelf = true)
        case _ => ()
    }
    ...
    if flow.entry.nonEmpty && !nodeIds.contains(flow.entry) then
      err("203.1", Some(flow.entry), "", s"entry node '${flow.entry}' not in nodes", "point entry at a declared node")
    ...
    if flow.entry.nonEmpty && nodeIds.contains(flow.entry) then
      ...
      (nodeIds -- reached).toList.sorted.foreach { orphan =>
        warn(
          ...
          Some(orphan),
          "",
          s"unreachable from entry '${flow.entry}' (orphan)",
    ...
    unconditionalCycle(flow).foreach { cycle =>
      ...
      err(
        ...
        Some(cycle.last),
        "",
        s"participates in an unconditional cycle ($path) with no switch condition to exit",
    ...
    FlowStructure.validate(flow).foreach { msg =>
      (msg, flow.nodes.size) match
        case (m, n) if m.startsWith("flow must have at least 2 nodes") && n > 0 =>
          err("212", None, "", s"needs at least 2 nodes (got $n)", "a single agent is an agent + skill, not a flow")
        ...
        case (m, _) if m.startsWith("flow has no termination path") =>
          err(
            ...
            None,
            "",
            "no termination path — no node routes to $return",
        ...
        case (m, _) if m.startsWith("join ") && m.contains("mixes parallel and serial") =>
          ...
          err(
            ...
            "",
            m,
            "a serial arrival corrupts the barrier count — route the serial in-edge through the fan's downstream"
          )
    ...
    flow.nodes.toList.sortBy(_._1).foreach { (id, node) =>
      if node.agent.nonEmpty && !agentNames.contains(node.agent) then
        err(
          ...
          Some(id),
          "agent",
          s"agent '${node.agent}' not found in the global library",
  ...
  private def unconditionalCycle(flow: FlowDagDef): Option[List[String]] =
    ...
    flow.nodes.keys.toList.sorted.iterator
      .flatMap { n =>
