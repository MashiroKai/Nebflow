...
object ActivityRecord:
  given Encoder[ActivityRecord] = Encoder.instance(r =>
    io.circe.Json.obj("timestamp" -> r.timestamp.asJson, "sessionId" -> r.sessionId.asJson, "eventType" -> r.eventType.asJson))
  given Decoder[ActivityRecord] = Decoder.instance(c =>
    for ts <- c.downField("timestamp").as[Long]; sid <- c.downField("sessionId").as[String]; et <- c.downField("eventType").as[String]
...
object TimeWindow:
  given Encoder[TimeWindow] = Encoder.instance(w =>
    io.circe.Json.obj("dayOfWeek" -> w.dayOfWeek.asJson, "startHour" -> w.startHour.asJson, "durationHours" -> w.durationHours.asJson))
  given Decoder[TimeWindow] = Decoder.instance(c =>
    for d <- c.downField("dayOfWeek").as[Int]; h <- c.downField("startHour").as[Int]; dur <- c.downField("durationHours").as[Int]
...
object UsagePattern:
  given Encoder[UsagePattern] = Encoder.instance(p =>
    io.circe.Json.obj(
      "hourlyActivity" -> p.hourlyActivity.asJson, "dailyActivity" -> p.dailyActivity.asJson,
      "idleWindows" -> p.idleWindows.asJson, "totalRecords" -> p.totalRecords.asJson, "lastUpdated" -> p.lastUpdated.asJson))
...
object UsageTracker:
  ...
  def analyzePattern(): IO[UsagePattern] = IO.blocking {
