...
class NodeEngine(
  ...
  private val running: Ref[IO, Map[String, Deferred[IO, Unit]]] =
   ...
   */
  private def noteNonTerminalConsumption(
    nodeId: String,
    sessionId: String,
    declared: Option[BlockedFeedback],
    branch: String
   ...
   */
  private def completeNodeR(nodeId: String, resultText: String, declared: Option[BlockedFeedback] = None): IO[Boolean] =
    declared match
   ...
   */
  private def blockedNodeR(nodeId: String, feedback: BlockedFeedback, finalText: Option[String] = None): IO[Boolean] =
    for
      now <- IO(System.currentTimeMillis())
      s <- store.mutate { st =>
        st.nodes.get(nodeId) match
   ...
   */
  private def blockedNode(nodeId: String, feedback: BlockedFeedback, finalText: Option[String] = None): IO[Unit] =
