...
object WakeSensor:
  ...
  private def announce(w: SleepWindow, prev: Sample, cur: Sample): IO[Unit] =
    ...
    val nanoDeltaMs = (cur.nanoMs - prev.nanoMs) / 1000000L
    ProjectStore.list().flatMap { pds =>
      pds.traverse_ { pd =>
        powerEntries(w, bootId, pd.name).traverse_(e =>
          BootDispatcherWake.appendPowerMarker(BootDispatcherWake.markerPath(pd), pd.name, e)) *>
          FlowMapEventLog
            .append(pd.workspace, pd.name, bootId, FlowMapEventLog.HostWakeType,
              FlowMapEventLog.hostWakeSummary(bootId, w.sleepAtMs, w.wakeAtMs, w.durationMs,
                wallDeltaMs, nanoDeltaMs, Defaults.WakeSenseSlopMs))
            .handleErrorWith(e =>
  ...
  private[project] def powerEntries(
    ...
    List(
      BootDispatcherWake.MarkerEntry(
        bootId = bootId, project = project, at = w.sleepAtMs, result = "noted",
        reason = s"host sleep began (inferred at wake; frozenMs=${w.durationMs})",
        blocking = false, nodes = 0, items = Nil, kind = BootDispatcherWake.KindSleep,
        sleepAt = Some(w.sleepAtMs), wakeAt = None),
      BootDispatcherWake.MarkerEntry(
        bootId = bootId, project = project, at = w.wakeAtMs, result = "noted",
        reason = s"host wake (frozenMs=${w.durationMs})",
        blocking = false, nodes = 0, items = Nil, kind = BootDispatcherWake.KindWake,
        sleepAt = Some(w.sleepAtMs), wakeAt = Some(w.wakeAtMs)))
