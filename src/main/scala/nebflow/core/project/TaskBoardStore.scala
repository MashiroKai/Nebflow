...
class TaskBoardStore private (val project: String, workspace: String):
  ...
  private def notFound(store: Store, id: String): ToolError =
    ...
    ToolError(s"""TaskBoard: no entry #$id. Open entries: $hint — pick an id from action=list. (${Codes.NotFound})""")
  ...
  private def pruneNoteOf(pruned: List[Entry], quarantined: Option[String]): String =
    (pruned.size, quarantined) match
      ...
      case (n, Some(q)) =>
        s" NOTE: previous task-board.json was corrupted — quarantined as $q. Pruned $n stale done entries."
        ...
        yield
          ...
          val histExtra = pruneHistory(pruned) +
            appendHistory(
              TaskBoardEvent(
                at = now,
                kind = TaskBoardHistory.Kinds.Create,
                id = Some(entry.id),
                actor = actor,
                links = links,
                detail = Some(s"title=${truncate(entry.title, 120)}")
              )
            )
          s"""[OK] TaskBoard created #$nextId [open @${entry.assignee
              .getOrElse("?")}] ${truncate(entry.title, 60)}$depsNote$pruneNote$quarantineHist$histExtra
            ...
            invalid match
              ...
              case None =>
                ...
                yield
                  ...
                  val qNote = quarantined
                    .map(q => s" NOTE: previous task-board.json was corrupted — quarantined as $q.")
                    .getOrElse("")
                  val quarantineHist = quarantineHistory(quarantined)
        ...
        checkTextLimit("note", note, NoteWriteMaxChars, NoteLimitFix) match
          ...
          case None =>
            ...
            yield
              ...
              val quarantineHist = quarantineHistory(quarantined)
              val hist =
                appendHistory(closeEvent(id, now, actor, existing, updated, note.map(_.trim).filter(_.nonEmpty)))
        ...
        store.tasks.find(_.id == id) match
          ...
          case Some(existing) =>
            history.appendSync(
              TaskBoardEvent(
                at = nowStr,
                kind = TaskBoardHistory.Kinds.Log,
                id = Some(id),
                actor = actor,
                text = Some(body),
                links = links,
                detail = Some(s"chars=${body.length} title=${truncate(existing.title, 60)}")
              )
            ) match
              case Some(reason) =>
                Left(
              ...
              case None =>
                ...
                Right(
                  s"""[OK] TaskBoard logged #$id (${body.length} chars appended; note untouched, board unchanged)$linkNote.${histNoteFor(
                      id
                    )}"""
                )
        end match
  ...
  def showSync(
    ...
    store.tasks.find(_.id == id) match
      ...
      case None =>
        if history.countLinesFor(id) > 0 then Right(TaskBoardRenderer.renderArchived(id, store.tasks, notes, states))
...
object TaskBoardStore:
  ...
  private[project] def checkTextLimit(field: String, value: Option[String], max: Int, fix: String): Option[ToolError] =
    value.map(_.trim).filter(_.nonEmpty).filter(_.length > max).map { v =>
      ToolError(s"TaskBoard: `$field` is ${v.length} chars — the write-side limit is $max chars. $fix (${Codes.Param})")
    }
   ...
   * （不写史，避免噪声）。
   */
  private[project] def updateEvents(
    id: String,
    at: String,
    actor: String,
    before: Entry,
    ...
    def ev(field: String, o: Option[String], n: Option[String]): Unit =
      buf += TaskBoardEvent(
        at = at,
        kind = TaskBoardHistory.Kinds.Update,
        id = Some(id),
        actor = actor,
        field = Some(field),
        prev = o,
    ...
    if after.status != before.status then
      buf += TaskBoardEvent(
        ...
        actor = actor,
        field = Some("status"),
        from = Some(before.status),
        to = Some(after.status)
      )
    if after.note != before.note then
      buf += TaskBoardEvent(
        ...
        actor = actor,
        field = Some("note"),
        prev = before.note.filter(_.nonEmpty),
        next = after.note.filter(_.nonEmpty),
  ...
  private[project] def closeEvent(
    ...
    if after.note != before.note then
      TaskBoardEvent(
        at = at,
        kind = TaskBoardHistory.Kinds.Close,
        id = Some(id),
        actor = actor,
        field = Some("note"),
        from = Some(before.status),
  ...
  private[project] def checkDepsClosed(
    store: Store,
    blocks: List[String],
    action: String,
    id: String
