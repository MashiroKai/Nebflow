...
object NodeReportRegistry:
  ...
  def register(workspace: String, project: String, nodeId: String, sessionId: String,
      feedback: BlockedFeedback): IO[Unit] =
    if workspace.trim.isEmpty then
      ...
      ensureLoaded(workspace) *>
        appendLine(workspace, project, nodeId,
          Json.obj(
            "op" -> "declare".asJson,
        ...
        signals.update { m =>
          ...
          m.updated(sessionId, Entry(sessionId, nodeId, project, workspace, feedback,
            recovered = prior.exists(_.recovered)))
        }
    ...
    ensureLoadedMounted *>
      signals.modify(m => (m - sessionId, m.get(sessionId))).flatMap {
        case Some(e) if e.recovered =>
          FlowMapEventLog.append(e.workspace, e.project, e.nodeId, NodeEngine.ReportUnconsumedEventType,
            s"kind=recovered-discard node_report NOT consumed — a declaration recovered from the journal " +
              ...
              s"suggestion=${e.feedback.suggestion}").handleErrorWith(t =>
              logger.warn(s"${WriteFailedLogToken} could not append the recovered-discard audit line " +
                s"for session=$sessionId (${Option(t.getMessage).getOrElse(t.toString)})")) *>
            logger.warn(s"Node '${e.nodeId}' declaration (${e.feedback.category}) recovered from the journal " +
  ...
  private def mountedProjects: IO[List[(String, String)]] =
    ProjectRuntimeRegistry.all
      ...
      .handleErrorWith(t =>
        logger.warn(s"node-report registry: mounted-project enumeration failed " +
          s"(${Option(t.getMessage).getOrElse(t.toString)}) — journal load skipped this round") .as(Nil))
  ...
  private def loadJournal(workspace: String): IO[LoadOutcome] =
    ...
    IO.blocking {
      ...
      case Left(t) =>
        degraded(workspace, path, s"journal unreadable (${t.getClass.getSimpleName}: " +
  ...
  private def replay(workspace: String, path: os.Path, bytes: Array[Byte]): IO[LoadOutcome] =
                ...
                case Some("declare") =>
                  (str("session"), str("category")) match
                    case (Some(sid), Some(cat)) if sid.nonEmpty && cat.nonEmpty =>
                      acc.updated(sid, Entry(
                        sessionId = sid,
                        nodeId = str("node").getOrElse(""),
                        project = str("project").getOrElse(""),
                        workspace = workspace,
                        feedback = BlockedFeedback(cat, str("detail").getOrElse(""),
                          str("suggestion").getOrElse("")),
                        recovered = true))
                    case _ =>
                      badLines += 1
                      acc
    ...
    val announce = replayed.toList.sortBy(_._1).traverse_ { case (sid, e) =>
      FlowMapEventLog.append(workspace, e.project, e.nodeId, StoreEventType,
        ...
        .handleErrorWith(t =>
          logger.warn(s"${WriteFailedLogToken} could not append the recovered-declaration audit line " +
            s"for session=$sid (${Option(t.getMessage).getOrElse(t.toString)})"))
    }
    val degradedBy: Option[String] =
      if tornTail.nonEmpty then
        Some(s"torn tail line at offset ${text.length - tornTail.length} " +
          s"(${tornTail.length} byte(s) kept on disk, never truncated)")
      else if badLines > 0 then
        Some(s"$badLines journal line(s) unparseable or carrying an unknown/incomplete shape " +
          "(kept on disk, never truncated)")
      else None
    // 内存工作集合并：内存优先（同进程内新申报胜过日志里的旧副本）
    signals.update { m => replayed.foldLeft(m) { case (acc, (sid, e)) => if acc.contains(sid) then acc else acc.updated(sid, e) } } *>
      (degradedBy match
  ...
  private def degraded(workspace: String, path: os.Path, reason: String, recovered: Int = 0): IO[LoadOutcome] =
    FlowMapEventLog.append(workspace, "", "", StoreEventType,
      ...
      .handleErrorWith(t =>
        logger.warn(s"${WriteFailedLogToken} could not append the degraded-load audit line " +
          s"(${Option(t.getMessage).getOrElse(t.toString)})")) *>
      logger.warn(s"node-report journal DEGRADED for workspace '$workspace' ($path): $reason — " +
  ...
  private def consumeLine(e: Entry, by: String): IO[Unit] =
    ...
    else appendLine(e.workspace, e.project, e.nodeId,
      Json.obj(
        "op" -> "consume".asJson,
        "ts" -> Json.fromLong(System.currentTimeMillis()),
      ...
      IO.blocking(os.write.append(journalPath(workspace), line.noSpaces + "\n", createFolders = true))
        ...
        .flatMap {
          ...
          case Left(t) =>
            ...
            logger.warn(s"${WriteFailedLogToken} node-report journal append failed for workspace " +
              ...
              FlowMapEventLog.append(workspace, project, nodeId, StoreEventType,
                ...
                .handleErrorWith(t2 =>
                  logger.warn(s"${WriteFailedLogToken} could not append the write-failed audit line either " +
  ...
  private def unboundVisible(project: String, nodeId: String, sessionId: String): IO[Unit] =
    logger.warn(s"${UnboundLogToken} node_report declaration for session=$sessionId node=$nodeId " +
