...
object CompletionGate:
  ...
  val defaultRunner: GitRunner = (cwd, args) =>
    IO.blocking {
  ...
  def check(
      ...
      if disabled then IO.pure(Pass("kill-switch (nebflow.completionGate.disabled=true)"))
        ...
        worktree match
          ...
          case Some(wt) =>
            ...
            if root == workspace then IO.pure(Pass(s"worktree '$wt' unresolvable → workspace fallback, skip"))
            else if !Files.exists(Paths.get(root)) then IO.pure(Pass(s"worktree dir missing: $root, skip"))
            else
              dirtyFiles(root, runner).flatMap {
                ...
                case Right(dirty) if dirty.isEmpty =>
                  aheadOf(root, runner).map { a =>
                    if !a.failed && a.count > 0 then Pass(s"committed: ahead main..HEAD=${a.count}, worktree clean")
                    else
                      Pass(s"zero-change: ahead=${a.count}${
                          if a.failed then " (rev-list failed → 按 0)" else ""
                        }, worktree clean")
                  }
                case Right(dirty) =>
    ...
    guarded
      .timeout(GateTimeout)
      .handleErrorWith(t => failOpen(s"gate error: ${Option(t.getMessage).getOrElse(t.toString)}"))
