...
class FlowMapStore private (
      ...
      state.get.map { s =>
        ...
        val depsReverse: Map[String, List[String]] =
          s.nodes.values.foldLeft(Map.empty[String, List[String]]) { (acc, n) =>
            FlowMapStore
              .resolveDepTargets(n.deps, s.nodes)
              .ids
  ...
  def sweepCompletedChainsDetailed(now: Long): IO[List[FlowMapStore.SweptChain]] =
    for
      ...
      chains = FlowMapStore
        .topologicalChains(s.nodes.values ++ a.nodes.values)
      ...
      _ <-
        if doneChains.isEmpty then IO.unit
        else
          ...
          for
            ...
            metas: Map[String, ArchiveBatchMeta] = doneChains.map { case (c, members) =>
              ...
              c.id -> ArchiveBatchMeta(
                c.id,
                now,
                members.map(_.id).toSet ++ merged,
                summarySentAt = None,
                summaryLedgerOn = true
              )
            }.toMap
            ...
            _ <- IO(
              logger.infoSync(
                s"FlowMap[$project] chain sweep: ${doneChains.map(_._1.id).mkString(", ")} (${ids.size} node(s)) → archive"
              )
            )
          yield ()
          end for
  ...
  def chainSummaryBatch(maxIndividual: Int): IO[(List[FlowMapStore.ChainSummary], List[FlowMapStore.ChainSummary])] =
    for
      ...
      due = bt.values
        .filter(m =>
          m.summaryLedgerOn && m.summarySentAt.isEmpty && m.nodeIds.size >= FlowMapStore.MinChainMembersForSummary
        )
        .toList
  ...
  def markChainSummarySent(chainId: String, at: Long): IO[Unit] =
    for
      ...
      updated = before.get(chainId) match
        ...
        case None => before
      _ <-
        if before.get(chainId).isDefined then
  ...
  end restoreChainsFromArchiveDetailed
      ...
      if batchIds.nonEmpty then
        ...
        batchIds.foreach { bid =>
          bt.get(bid).foreach { meta =>
            ...
            if nodes.isEmpty && meta.nodeIds.nonEmpty then os.remove.all(batchPath(bid)) // 批成员全移除 → 批文件删除（当前无调用方，防呆）
                  ...
                  if orphans.isEmpty then None
                    ...
                    Some(
                      c.id -> bt
                        ...
                        .map(p =>
                          base.copy(
                            nodeIds = base.nodeIds ++ p.nodeIds,
                            archivedAt = math.max(base.archivedAt, p.archivedAt)
                          )
                        )
                        .getOrElse(base)
                    )
          ...
          case Some(r) if r.trim.nonEmpty =>
            ...
            Json.fromJsonObject(
              io.circe.JsonObject.fromMap(
                nobj.toMap
                  .updated("result", FlowMapStore.summarizeResult(r).asJson)
        ...
        nobj("task").flatMap(_.asString) match
          case Some(t) if t.trim.nonEmpty =>
            ...
            Json.fromJsonObject(
              io.circe.JsonObject.fromMap(
                nobj.toMap
                  .updated("task", FlowMapStore.summarizeTask(t).asJson)
   ...
   */
  private def hydrateAndMigrate(
    nodes: Map[String, NodeDef],
    sourcePath: os.Path,
    migrateTasks: Boolean
  ): (Map[String, NodeDef], Boolean) =
                ...
                Some(
                  (
                    ArchiveBatchMeta(
                      ...
                      summaryLedgerOn = b.summaryLedgerOn.contains(true)
                    ),
                    hydrated,
                    migrated
                  )
          ...
          jsonParse(os.read(archivePath)).flatMap(_.as[FlowMapArchive]) match
            case Right(a0) =>
              ...
              if hydrated.nonEmpty then
                logger.warnSync(
                  s"flow-map '$project': legacy single-file archive migrated to per-batch files (${migratedBatches.size} batch file(s), ${toMigrate.size} node(s)) — original renamed to ${dest.last}"
                )
              migratedBatches
            case Left(e) =>
...
object FlowMapStore:
  ...
  def renderChainSummary(
    ...
    if members.size < MinChainMembersForSummary then None
      ...
      val head =
        s"[Chain '$title' · ${sorted.size} nodes · $completed completed · $failed failed · $cancelled cancelled" +
      ...
      val rows =
        if sorted.size > ChainSummaryFoldMembers then
          ...
          (if completed > 0 then List(s"· completed ×$completed（折叠：成员数 ${sorted.size} > $ChainSummaryFoldMembers）")
           else Nil) ++
      ...
      val text =
        if raw.length <= ChainSummaryMaxChars then raw
          ...
          (kept.toList :+ s"… (摘要按 ${ChainSummaryMaxChars} 字符上限截断，共 ${sorted.size} 成员；全文见 NodeList(detail=\"<id>\") 与 REST results 端点)")
            .mkString("\n")
  ...
  def renderChainSummaryOverflow(rest: List[ChainSummary]): String =
    ...
    val head =
      s"[Chain] 本回合另有 ${rest.size} 条链完成（超过每回合 ${ChainSummaryMaxPerRound} 条的独立摘要上限，合并为一条；逐链明细见 NodeList 与 REST results 端点）"
    (head :: lines).mkString("\n")
    ...
    nodeMap.values.foreach { n =>
      ...
      n.out.foreach { e =>
        if e.to != OutEdge.RootTarget then
          ...
          OutEdge
            .resolveTargetId(nodeMap, e.to)
            .foreach(t => link(n.id, t, if OutEdge.isLoopEdge(e) then "loop" else "out", memberEdge = true))
      }
  ...
  def chainTitle(members: Iterable[NodeDef], chainId: String): String =
    ...
    val taskPreview = sorted.headOption
      ...
      .map { t =>
        ...
        if firstLine.length > NodePayload.TaskPreviewMaxChars then firstLine.take(NodePayload.TaskPreviewMaxChars) + "…"
        else firstLine
      }
  ...
  def open(project: String, workspace: String): IO[FlowMapStore] =
    ...
    for
      ...
      store = new FlowMapStore(
        project,
        statePath,
        archivePath,
        Ref.unsafe[IO, FlowMapState](FlowMapState(project = project, updatedAt = 0L)),
        Ref.unsafe[IO, FlowMapArchive](FlowMapArchive(project = project)),
        Ref.unsafe[IO, Map[String, ArchiveBatchMeta]](Map.empty),
        ledgerStore
      )
      ...
      _ <-
        if dupIds.isEmpty then IO.unit
          ...
          for
            ...
            _ <- store.batches.update(m =>
              m.filterNot { case (_, meta) =>
