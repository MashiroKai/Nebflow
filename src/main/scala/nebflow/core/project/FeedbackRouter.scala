...
final class FeedbackRouter(
  ...
  def decide(node: NodeDef): IO[(FeedbackDecision, Boolean)] =
    guard.modify { g =>
      ...
      if inCooldown then (g.copy(blockedAt = recent), (FeedbackDecision.Suppress: FeedbackDecision, false))
      else if feedbackMode == FeedbackRouter.ModeEscalateOnly then
        (
          g.copy(blockedAt = recent),
          (FeedbackDecision.Escalate(FeedbackDecision.EscalateReason.Mode): FeedbackDecision, false)
        )
      else if node.blockCount > NodeEngine.MaxBlockRoundsPerNode then
        (
          g.copy(blockedAt = recent),
          (FeedbackDecision.Escalate(FeedbackDecision.EscalateReason.LoopCap): FeedbackDecision, false)
        )
      ...
      else (g.copy(blockedAt = recent), (FeedbackDecision.Reenter: FeedbackDecision, false))
      end if
    }
    ...
    for
      ...
      _ <- verdict match
        ...
        case (FeedbackDecision.Suppress, _) =>
          logger.warn(
            s"Project '$projectName' node '${node.name}' blocked during cooldown — suppressed (merged into single escalation)"
          )
    yield ()
  ...
  private def doEscalate(
    node: NodeDef,
    feedback: BlockedFeedback,
    reason: FeedbackDecision.EscalateReason,
    cooldownJustOn: Boolean
  ): IO[Unit] =
    for
      ...
      _ <- logger.warn(
        s"Project '$projectName' node '${node.name}' blocked escalated to Nebula (reason=$reason, round ${node.blockCount})"
      )
    yield ()
    ...
    val histSection =
      if hist.length <= 1 then ""
        ...
        "\n历史轮次反馈（本进程内记录 " + hist.length + " 轮）：\n" +
          hist.zipWithIndex
            .map((f, i) => s"  ${i + 1}. [${f.category}] ${f.detail} — 建议: ${f.suggestion}")
            .mkString("\n")
    reason match
