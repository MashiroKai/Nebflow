  ...
  def ackAttachmentReceived(attachmentId: String, evidence: AttachmentAck.Evidence): IO[AttachmentAck.Result] =
    AttachmentAck.decide(evidence) match
      case AttachmentAck.Decision.Skip(reason) =>
        logger
          .info(
            s"attachment ack skipped ($reason) for $attachmentId — server blob untouched (24h TTL is the safety net)"
          )
      ...
      case AttachmentAck.Decision.Fire(digest) =>
        withClient(_.confirmAttachmentReceived(attachmentId, digest))
          ...
          .handleErrorWith(e =>
            logger
              .warn(s"attachment ack for $attachmentId raised: ${truncate(msg(e))}")
              .as(AttachmentAck.Result.Failed("raised"))
          )
...
final class FriendMessagingGuard(
  ...
  def trySend(friendId: String, now: Long): IO[Either[String, Unit]] =
    state.modify { s =>
      ...
      if pf.size >= perFriendPerHour then (s, Left(s"rate limit: $perFriendPerHour msgs/hour to this friend"))
      else if gl.size >= globalPerHour then (s, Left(s"rate limit: $globalPerHour msgs/hour total"))
      else
  ...
  def setRead(conversationId: String, lastReadMessageId: Long): IO[Unit] =
    state.update { s =>
      ...
      s.copy(cursors =
        s.cursors.updated(
          conversationId,
          cur.copy(lastReadMessageId = Math.max(cur.lastReadMessageId, lastReadMessageId), unreadCount = 0)
        )
      )
    }
