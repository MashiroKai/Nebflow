 ...
 * 移除，恰十四件；general/BaseTools 六件默认注入不变，Nebula 是唯一例外）。
 ...
 *   交付面双层）∧ Bash/Write/Edit ∉ Nebula 集 + 恰十四件计数——本文件即
 *   变异验红锚点：机制集加回写手任一件（或计数漂移）即红。
...
class NebulaSixBaseToolsSpec extends FunSuite:
  ...
  test("① 读三件 ⊆ Nebula 机制集（静态集+交付面双层）∧ Bash/Write/Edit 均不在且恰十四件——加回任一写手即红"):
    ...
    // 收走写手后恰十四件（编排四件 + 通信 + 读三件 + Card + 用户面二件 +
    // 平台二件 + MemoryEdit）
    assertEquals(fixed.size, 14, "Nebula 机制集恰十四件（2026-09-05 23:34 裁定：回归纯编排）")
