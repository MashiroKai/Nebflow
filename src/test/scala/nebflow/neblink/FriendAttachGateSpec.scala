...
class FriendAttachGateSpec extends FunSuite:
      ...
      s.createContext(
        ...
        (ex: HttpExchange) =>
          ...
          requests.synchronized {
            requests += s"${ex.getRequestMethod} $uri | auth=${
                if auth.startsWith("Bearer ") then "Bearer <token>" else auth
              } | " +
          ...
          val (code, resp) =
            if path.startsWith("/api/device/") then
              ...
              (
                200,
                """{"friends":[{"userId":"u1","username":"alice","display_name":"Alice"}],"incoming":[],"outgoing":[]}"""
              )
            else if path.endsWith("/attachments") && body.contains("\"size\":0") then
              (422, """{"error":"INVALID_ARGUMENT"}""")
            else if path.endsWith("/attachments") then
              (201, """{"attachmentId":"att-1","receivedBytes":0,"state":"uploading"}""")
  ...
  test("A-5 judge: 404 ⇒ Unsupported；422/429/403 ⇒ Supported；5xx/传输失败 ⇒ Undetermined") {
    assert(
      AttachmentCapability
        .judge(Left("""HTTP 404: {"error":"not found"}"""))
        .isInstanceOf[AttachmentCapability.Unsupported]
    )
    assert(
      AttachmentCapability
        .judge(Left("""HTTP 422: {"error":"INVALID_ARGUMENT"}"""))
        .isInstanceOf[AttachmentCapability.Supported]
    )
    assert(
      AttachmentCapability
        .judge(Left("""HTTP 429: {"error":"rate_limited"}"""))
        .isInstanceOf[AttachmentCapability.Supported]
    )
    assert(
      AttachmentCapability
        .judge(Left("""HTTP 403: {"error":"not_friends"}"""))
        .isInstanceOf[AttachmentCapability.Supported]
    )
  ...
  test("A-5 零自报字段（①）：body 里有没有「能力声明」都不改判词 —— 判据只在状态码") {
    ...
    assert(
      AttachmentCapability
        .judge(Left("""HTTP 404: {"supportsAttachments":true}"""))
        .isInstanceOf[AttachmentCapability.Unsupported]
    )
  }
  ...
  test("A-5 态1 对齐：探测 = Supported，且上传链真的把 sha256/分块/附件 id 送上 wire") {
    withStub("new") { stub =>
      ...
      val probeLine = log
        .find(l => l.contains("/api/friends/u1/attachments") && l.contains("\"size\":0"))
        .getOrElse(fail(s"no probe request: $log"))
      ...
      assert(
        log.exists(l => l.contains("POST /api/friends/u1/attachments") && l.contains(FixtureSha)),
        s"E1 must carry the real whole-file sha256: $log"
      )
      assert(
        log.exists(l => l.contains("/chunks?offset=0") && l.contains("X-Chunk-Sha256=" + FixtureSha)),
        s"E2 must carry the chunk sha header: $log"
      )
      assert(
        log.exists(l => l.contains("POST /api/friends/u1/messages") && l.contains("\"attachments\":[\"att-1\"]")),
        s"send payload must carry attachment ids: $log"
      )
      evidence(
        "态1 对齐",
        s"probe=$cap",
  ...
  test("A-5 态2 不对齐：404 ⇒ Unsupported，工具面明确拒绝且**零上传**（不白传字节）") {
    withStub("old") { stub =>
      ...
      evidence(
        "态2 不对齐",
        s"probe=${cli.probeAttachmentCapability("u1").unsafeRunSync()}",
        s"refusal=$msg",
        s"非探测请求=${stub.nonProbeRequests}"
      )
    }
  }
