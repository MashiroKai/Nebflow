...
class LogtoAuthCodeSpec extends FunSuite:
  ...
  test("authorizeUrl carries client_id, S256 challenge, state, offline_access and the loopback redirect") {
    ...
    assertEquals(url, "https://auth.example/oidc/auth?client_id=pkce-app&redirect_uri=http%3A%2F%2F127.0.0.1%3A8080%2Fauth%2Fcallback&response_type=code&scope=openid+offline_access&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256&state=st-123")
  }
