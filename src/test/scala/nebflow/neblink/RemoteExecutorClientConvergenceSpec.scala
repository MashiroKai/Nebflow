        ...
        .use { dispatcher =>
          ...
          yield
            ...
            assertEquals(
              probes,
              1,
              s"must be exactly ONE read-only profile probe on the relay path (bodies=${fix.relayExecBodies.asScala.toList})"
            )
            assertEquals(
              business,
              1,
              s"and exactly ONE business dispatch (probe/business split read from the bodies, not from the count)"
            )
        }
