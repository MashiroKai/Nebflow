...
class WatchdogReplaySegmentationSpec extends CatsEffectSuite:
  ...
  test("对照 fixture: 注入的 fp→true 翻转必被不变量列表判红（读数点名该行、条数 = 1），正常窗口后行不误红") {
    IO.blocking {
      ...
      println("[对照 fixture] 窗口起点=" + r.window.start.getOrElse(-1L) +
      ...
      assertEquals(flip.map(_.status), Vector(Status.Fail),
        "注入的 fp→true 翻转必须让不变量判定判红")
      assertEquals(r.fpToTrue, Vector("node-fixture-fp2true@1789217002500"),
        "翻转列表必须精确点名注入行（条数 = 1，无其它）")
      assert(flip.head.detail.contains("1 条") && flip.head.detail.contains("node-fixture-fp2true"),
        "判红读数必须给条数与命中会话，实得: " + flip.head.detail)
      assertEquals(leak.map(_.status), Vector(Status.Ok),
        "注入的翻转不是「放走真卡死」⇒ 该列表不得非空")
      assertEquals(post.map(_.status), Vector(Status.Fail),
        "注入行同时使「窗口后段: 新判据重放复现日志 class」判红（该行日志 class 与其读数矛盾）")
      assertEquals(r.post.newMismatch.size, 1, "窗口后段只应有注入的 1 条不一致（正常行零假阳）")
      assertEquals(pre.map(_.status), Vector(Status.Ok, Status.Ok, Status.Ok),
        "窗口前段（旧构建出处行）不受注入影响 ⇒ 三段判定全绿")
    }
  }
  ...
  test("反向对照: 旧形态（全语料一律断言旧判据保真）在同一 fixture 上判红 4 条，而新形态窗口前段 = 0 条") {
    IO.blocking {
      ...
      println("[反向对照] 新形态（窗口后段新判据保真）: " +
        WatchdogReplayCore.fidelityVerdicts(r).filter(_.name.startsWith("窗口后段")).head.detail)

      assertEquals(oldForm.map(_.status), Vector(Status.Fail),
        "旧形态在含窗口后行的语料上必然判红（这正是 R1′：落地即红）")
      assertEquals(r.allLegacyMismatch.size, 4,
        "旧形态读数 = 4 条（第 i 刀 2 + 第 ii 刀 1 + 注入 1）")
      assertEquals(newPre.map(_.status), Vector(Status.Ok),
        "同一 fixture 上新形态的窗口前段保真度 = 绿（分段把「修复构建写的行」移出该断言）")
  ...
  test("窗口前段断言非空洞: 在窗口前段行上注入正信号不一致 ⇒ 该段判红 1 条（点名行），后段/不变量不受影响") {
    IO.blocking {
      ...
      val mutated = rows.map { r =>
        if r.ts == targetTs then
          r.copy(note = r.note.replace("progress signal 24s ago", "progress signal 300s ago"))
        else r
      }
      ...
      println("[前段注入] 窗口前段旧判据不一致: " + r.pre.legacyMismatch.size +
