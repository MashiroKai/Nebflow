...
class WakeSensorSpec extends CatsEffectSuite:
  ...
  test("J1 DarkWake 突刺回放：34s 墙缝内 19s 清醒 ⇒ frozen 15s < 45s ⇒ 无窗") {
    ...
    val cur = WakeSensor.Sample(
      wallMs = prev.wallMs + 34_000L,
      nanoMs = prev.nanoMs + 19_000_000_000L)
  ...
  test("E2 append-only 共存：boot blocking 条目不被电源条目顶掉（duplicate-boot 幂等不受影响）") {
    ...
    val bootEntry = BootDispatcherWake.MarkerEntry(
      bootId = "boot-1", project = "pw", at = 1L, result = "woken", reason = "",
      blocking = true, nodes = 0, items = Nil)
    // 预置合法 v2 台账文件（含 boot blocking 条目）
    os.makeDir.all(marker / os.up)
    os.write.over(marker, BootDispatcherWake.MarkerFile(
      project = "pw", lastBootId = "boot-1", entries = List(bootEntry)).asJson.noSpaces)
    // 追加一对电源条目（WakeSensor 同款写点）
    val w = SleepWindow(1_000L, 2_000L)
    WakeSensor.powerEntries(w, "boot-1", "pw").traverse_(e =>
      BootDispatcherWake.appendPowerMarker(marker, "pw", e)).unsafeRunSync()
  ...
  test("E3 bootShutdownCause：无 marker 无 unclean ⇒ None；marker 在 ⇒ graceful；本 boot unclean ⇒ unclean") {
    ...
    os.write.over(BootDispatcherWake.shutdownMarkerPath,
      BootDispatcherWake.ShutdownMarker(v = 1, kind = "graceful", at = 42L,
        bootId = "boot-0", cause = "SIGINT/SIGTERM").asJson.noSpaces)
    ...
    val marker = BootDispatcherWake.markerPath(pd)
    os.makeDir.all(marker / os.up)
    os.write.over(marker, BootDispatcherWake.MarkerFile(project = "pc", lastBootId = "boot-1",
      entries = List(BootDispatcherWake.MarkerEntry(
        bootId = "boot-1", project = "pc", at = 9L, result = "noted",
        reason = "unclean", blocking = false, nodes = 3, items = Nil,
        kind = BootDispatcherWake.KindUnclean))).asJson.noSpaces)
  ...
  test("E4 annotateShutdownCause：零残留零条目；有残留无 marker ⇒ unclean；有残留有 marker ⇒ graceful") {
    ...
    os.write.over(BootDispatcherWake.shutdownMarkerPath,
      BootDispatcherWake.ShutdownMarker(v = 1, kind = "graceful", at = 42L,
        bootId = "b0", cause = "SIGINT").asJson.noSpaces)
