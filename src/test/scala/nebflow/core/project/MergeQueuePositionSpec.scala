...
class MergeQueuePositionSpec extends CatsEffectSuite:
  ...
  private def queueReadings(p: Json): List[(Int, String, Long, Long)] =
    p.hcursor.downField("nodes").as[List[Json]].getOrElse(Nil).flatMap { nj =>
      for
  ...
  private def dumpPayload(p: Json): Unit =
    val path = sys.env.get("NEBFLOW_QUEUEPOS_DUMP").filter(_.trim.nonEmpty)
      .getOrElse((os.pwd / "target" / "queuepos-payload.json").toString)
    val f = new java.io.File(path)
    Option(f.getParentFile).foreach(_.mkdirs())
    java.nio.file.Files.write(f.toPath,
      p.spaces2.getBytes(java.nio.charset.StandardCharsets.UTF_8))
  ...
  test("① author scenario: with one merge RUNNING and three waiters whose verdict gate is unmet, the OLD key's `ahead` is identical for all three (1/1/1 — the reported symptom) while `mergeQueuePos.position` is unique per node (2/3/4) and matches the SEM-2 rank order") {
    ...
    for
      ...
      verifier = NodeDef(id = "n-ver", name = "verify-r1", agent = "general", task = Some("verify"),
        status = NodeLifecycle.Completed, role = NodeRoles.Verifier, lastVerdict = Some(rt.engine.VerdictFail),
        result = Some("fail verdict"), out = Nil, createdAt = now - 20_000L, completedAt = Some(now - 8_500L))
      _ <- seed(rt,
    ...
    yield
      ...
      assertEquals(aheads, List("n-q1" -> Some(1), "n-q2" -> Some(1), "n-q3" -> Some(1)),
        s"pre-change reading (RED): every waiter reports the SAME `ahead` — that is the author's symptom: $p")
      assertEquals(aheads.flatMap(_._2).distinct.size, 1,
      ...
      assertEquals(positions.flatMap(_._2).map(_._1).distinct.size, positions.size,
        "post-change reading: the positions MUST be pairwise distinct (位次逐节点唯一)")
      // 队列身份 + 次序键元数据（前端只读不派生）
      assertEquals(posStr(p, "n-q1", "queue"), Some(MergeMutexPolicy.QueueName),
        "the slot names the queue it belongs to (engine-side token, frontend only translates)")
      assertEquals(posField(p, "n-q1").flatMap(_.hcursor.downField("rank").get[String]("primary").toOption),
        Some(MergeMutexPolicy.RankPrimary), "the rank basis is published next to the position")
      // 已到达（上游全终态 ⇒ readyAt = max(completedAt)）：arrived = true，且 readyAt 可复算
      assertEquals(posBool(p, "n-q1", "arrived"), Some(true))
      assertEquals(posLong(p, "n-q1", "readyAt"), Some(now - 8_500L),
  ...
  test("② no-display pole: three waiters with NO running holder and mutually verdict-blocked predecessors carry NO `mergeQueue` at all (0/3 on the old face) while `mergeQueuePos` still gives each one a unique rank (1/2/3 of 3)") {
    ...
    for
      ...
      verifier = NodeDef(id = "n-ver", name = "verify-h", agent = "general", task = Some("verify"),
        status = NodeLifecycle.Completed, role = NodeRoles.Verifier, lastVerdict = None,
        out = Nil, createdAt = now - 20_000L, completedAt = Some(now - 8_500L))
      _ <- seed(rt,
        verifier,
    ...
    yield
      ...
      assertEquals(oldFace.map(_._2), List(None, None, None),
        s"pre-change reading (RED): not one waiter carries the old key — nothing is displayed: $p")
      // 改后绿读数（新面）：位次照带，互异
      assertEquals(List("n-h1", "n-h2", "n-h3").map(id => id -> posPair(p, id)),
  ...
  test("③ arrival flag: a waiter whose upstream is not terminal is still ranked (the queue stays fully readable) but is flagged arrived=false and sorts LAST (rank primary = MaxValue)") {
    ...
    for
      ...
      _ <- seed(rt,
        ...
        NodeDef(id = "n-up", name = "upstream-worker", agent = "coder", task = Some("work"),
          status = NodeLifecycle.Running, out = List(OutEdge("n-late")), createdAt = now - 30_000L),
    ...
    yield
      ...
      assertEquals(posBool(p, "n-late", "arrived"), Some(false),
        "arrived=false = its upstreams are not terminal yet (the frontend/reader must not read it as 'about to run')")
      assertEquals(posLong(p, "n-late", "readyAt"), Some(Long.MaxValue),
        "readyAt = MaxValue is the rank primary key of a not-yet-arrived node (让位，不阻断)")
  }

  // ── ④ 条件键纪律（零漂移：没有队列就没有键）──────────────────────────

  test("④ conditional-key discipline: non-merge nodes, the running holder, terminal merges and blocked merges carry NO mergeQueuePos key; a SOLO merge (fewer than 2 contenders in its whole project) carries none either — while a waiter behind a running holder does") {
    ...
    for
      ...
      _ <- seed(rt,
        NodeDef(id = "n-plain", name = "plain", agent = "coder", task = Some("work"),
          status = NodeLifecycle.Pending, out = List(OutEdge.nebula), createdAt = now - 3_000L),
        runningHolder("n-hold", "hold-merge", now - 9_000L),
    ...
    yield
      for id <- List("n-plain", "n-hold", "n-done", "n-blocked") do
        assertEquals(posField(p, id), None,
  ...
  test("④b two-node queue: with two open merges and nothing running, the head carries rank 1 and the later one rank 2 — the queue exists WITHOUT any holder") {
    ...
    yield
      ...
      assertEquals(mqField(p, "n-a"), None,
  ...
  test("⑤ zero drift on the frozen key: `mergeQueue` keeps EXACTLY {ahead,inSection,rank,holders} and its literal values, and now coexists with `mergeQueuePos` on the same node") {
    ...
    for
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield
      assertEquals(mqKeys(p, "n-wait") - "sameKeyProjects", Set("ahead", "inSection", "rank", "holders"),
      ...
      assertEquals(posKeys(p, "n-wait") - "sameKeyProjects",
  ...
  test("⑥ derived only: `mergeQueuePos` is never persisted on NodeDef (it does not enter flow-map.json / the archive batch)") {
    ...
    yield
      ...
      assertEquals(persisted.hcursor.downField("mergeQueuePos").focus, None,
  ...
  test("⑦ same-key multi-project (O-1): the position slot carries sameKeyProjects too, so the frontend degrades to a bare 'queued' with no number") {
    ...
    if !gitAvailable then IO.unit
      ...
      for
        ...
        _ <- seed(rtA, runningHolder("a-holder", "merge-a", now - 8_000L),
      ...
      yield
        ...
        assertEquals(posField(pB, "b-wait").flatMap(_.hcursor.downField("sameKeyProjects").as[List[String]].toOption),
        ...
        assertEquals(posField(pA, "a-wait").flatMap(_.hcursor.downField("sameKeyProjects").as[List[String]].toOption),
  ...
  test("⑧ truth-source consistency: the published positions are pairwise distinct and their order is EXACTLY the rank order recomputed from the payload's own (readyAt, createdAt, id) triples (a running holder holds a rank without a slot — hence the gap)") {
    ...
    for
      ...
      _ <- seed(rt,
        runningHolder("n-hold", "hold-merge", now - 9_000L),
        NodeDef(id = "n-ver", name = "verify-c", agent = "general", task = Some("verify"),
          status = NodeLifecycle.Completed, role = NodeRoles.Verifier, lastVerdict = Some(rt.engine.VerdictFail),
          out = Nil, createdAt = now - 20_000L, completedAt = Some(now - 4_500L)),
    ...
    yield
      ...
      assertEquals(readings.map(r => (r._1, r._2)), List((1, "n-c3"), (3, "n-c1"), (4, "n-c2")),
      ...
      assertEquals(posPair(p, "n-hold"), None,
