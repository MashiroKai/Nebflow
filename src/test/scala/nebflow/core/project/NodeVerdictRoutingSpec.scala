...
class NodeVerdictRoutingSpec extends CatsEffectSuite:
  ...
  test("B4 syntax: '(fail)X:loop' parses to on={fail} + mode=loop; gate/mode errors tell the two gates apart") {
    ...
    assertEquals(
      ok.map(es => es.map(e => (e.to, e.on, e.mode))),
      Right(List(("n-w", Set("fail"), "loop"))),
      "fail gate + loop mode parse"
    )
    val bad = NodeTools.parseOut(Some(Json.fromString("(nope)n-w")))
    assert(bad.isLeft, "unknown gate must be rejected")
    assert(
      bad.left.exists(m => m.contains("'failed'") && m.contains("'fail'") && m.contains("VERDICT")),
    ...
    assert(
      badMode.left.exists(m => m.contains("loop") && m.contains("signal") && m.contains("result")),
    ...
    assert(
      OutEdge.isVerdictGate(OutEdge("n-w", Set(OutEdge.Fail), OutEdge.Loop)),
      "fail+loop is the verdict control edge"
    )
  ...
  test("B4 validation family: each verdict-routing criterion rejects with its own code (0 spawn)") {
    ...
    yield
      ...
      assert(
        !code(r10).contains("Cycle detected"),
        s"the loop edge must stay out of the DAG cycle check, got: ${code(r10)}"
      )
      // 0 spawn：被拒的创建不落库（v-self 是唯一合法建立的 verifier）
      val names = snap.nodes.values.map(_.name).toSet
      val rejected =
        List("t-fail", "v-failed", "v-failonly", "v-looponly", "v-noroute", "v-neb", "v-miss", "v-multi", "v-collide")
  ...
  test("B4 validation family (direct): the verdict gate itself rejects a dangling fail target (NODE_LOOP_EDGE_ROLE)") {
    ...
    for
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield assert(
      dangling.exists(e => e.contains("NODE_LOOP_EDGE_ROLE") && e.contains("n-ghost")),
  ...
  test("B4 validation family (edit face): retry on a fail-route target is rejected (NODE_RETRY_LOOP_CONFLICT)") {
    ...
    for
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield assert(
      conflict.left.exists(_.contains("NODE_RETRY_LOOP_CONFLICT")),
  ...
  test("B4 red line ①: a ':loop' control edge is never mirrored into the target's in ledger (pass edges still are)") {
    ...
    yield
      ...
      assert(
        ...
        "the control edge stays in the out ledger"
      )
    end for
  }
  ...
  test("B5 circuit-break (wall clock): sweep terminalizes the verifier failed, logs loop-budget, clears the timer") {
    ...
    try
      ...
      yield
        ...
        assert(
          ver.result.exists(_.contains("rounds=")) && ver.result.exists(_.contains("wallClock")),
  ...
  test("B5 circuit-break idempotence: a second scan of the same sighting does nothing (timer already cleared)") {
    ...
    try
      ...
      yield
        ...
        assertEquals(
          second.count(_._1 == "loop-budget"),
          1,
          "the second scan is a no-op (timer cleared ⇒ no longer due)"
        )
    finally
      LoopProps.foreach(System.clearProperty)
    end try
  ...
  test(
    "B5 circuit-break (orphan timer): no live fail-route driver ⇒ timer cleared with an orphan event, nothing terminalized"
  ) {
    ...
    try
      ...
      yield
        ...
        assert(
          audit.exists((t, id, s) => t == "loop-budget" && id == "n-orph" && s.contains("orphan")),
  ...
  test("B5 predicate single point LoopBudget.decide: round cap / wall clock / independent dims / <=0 disables a dim") {
    assertEquals(
      LoopBudget.decide(rounds = 3, maxRounds = 3, startedAt = None, now = 1000L, maxWallMs = 0L),
      Some("rounds=3/3")
    )
    assertEquals(
      LoopBudget.decide(rounds = 0, maxRounds = 3, startedAt = Some(0L), now = 1000L, maxWallMs = 500L),
      Some("wallClock=1000ms/500ms")
    )
    assert(
      LoopBudget.decide(rounds = 0, maxRounds = 3, startedAt = Some(0L), now = 100L, maxWallMs = 500L).isEmpty,
      "inside budget ⇒ None"
    )
    assert(
      LoopBudget.decide(rounds = 9, maxRounds = 0, startedAt = Some(0L), now = 1L, maxWallMs = 0L).isEmpty,
      "thresholds <= 0 turn the dimension off (knob semantics)"
    )
    assert(
      LoopBudget
        .decide(rounds = 5, maxRounds = 3, startedAt = Some(0L), now = 900L, maxWallMs = 500L)
    ...
    for
      ...
      badRole <- nodeEdit(
        nodeInput(
          "vrole",
          "r-bad",
          "description" -> Json.fromString("bad role"),
          "task" -> Json.fromString("t"),
          "role" -> Json.fromString("controller")
        ),
        ctx
      )
      _ <- seed(
  ...
  test("B8/C5: reactivateCompleted is the explicit authorization — rejected off a completed node, honoured on one") {
    ...
    for
      ...
      createAuth <- nodeEdit(
        nodeInput(
          "vreact",
          "brand-new",
          "description" -> Json.fromString("new node"),
          "task" -> Json.fromString("t"),
      ...
      runAuth <- nodeEdit(
        nodeInput(
          ...
          "run-node",
          "reactivateCompleted" -> Json.fromBoolean(true),
          "task" -> Json.fromString("run-node task v2")
        ),
        ctx
      )
      doneAuth <- nodeEdit(
        nodeInput(
          ...
          "done-node",
          "reactivateCompleted" -> Json.fromBoolean(true),
          "description" -> Json.fromString("done-node v2")
        ),
        ctx
      )
    ...
    yield
      assert(
        createAuth.left.exists(_.contains("NODE_COMPLETED_REACTIVATION")),
        s"the flag is edit-only (no node exists on the create path), got: $createAuth"
      )
      assert(
        runAuth.left.exists(_.contains("NODE_COMPLETED_REACTIVATION")),
      ...
      assert(
        doneAuth.exists(_.contains("reactivated from completed")),
      ...
      assert(
        audit.exists((t, id, s) => t == "reactivated" && id == "n-done" && s.contains("NODE_COMPLETED_REACTIVATION")),
        s"the reactivation must be audited with the authorization name, got: $audit"
      )
    end for
  }
end NodeVerdictRoutingSpec
