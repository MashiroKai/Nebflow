...
class NodeMergeSpec extends CatsEffectSuite:
        ...
        Stream.eval(inputs.update(_ :+ text)).flatMap { _ =>
          if toolResults.nonEmpty then
            Stream.eval(IO.sleep(50.millis)) >>
              Stream(
                StreamChunk.TextDelta(s"merge done (${toolResults.mkString.take(120)})"),
            ...
            Stream(
              StreamChunk.ToolCallChunk(
                ToolCall("merge-landing-cmd", "Bash", JsonObject("command" -> cmd.asJson, "timeout" -> 120000.asJson))
              ),
  ...
  private def nodeInput(project: String, nodename: String, extra: (String, CJson)*): CJson =
    CJson.obj(
      ("project" -> CJson.fromString(project)) :: ("nodename" -> CJson
        .fromString(nodename)) :: ("plugins" -> CJson.arr()) :: extra.toList*
    )
  ...
  test(
    "T0: merge=true refuses worktree; zero-upstream merge refused (NODE_MERGE_REQUIRES_UPSTREAM); merge persists with in (legal order); archived node refuses merge"
  ) {
    ...
    for
      ...
      n1 <- nodeEdit(
        nodeInput(
          "merge-t0",
          "m1",
          "task" -> CJson.fromString("merge"),
          "description" -> CJson.fromString("merge node refuse worktree"),
          "out" -> CJson.fromString("Nebula"),
          "merge" -> CJson.fromBoolean(true),
          "worktree" -> CJson.fromBoolean(true)
        ),
        ctx
      )
      ...
      n0 <- nodeEdit(
        nodeInput(
          ...
          "m0",
          "task" -> CJson.fromString("merge"),
          "description" -> CJson.fromString("merge zero upstream"),
          "out" -> CJson.fromString("Nebula"),
          "merge" -> CJson.fromBoolean(true)
        ),
        ctx
      ...
      _ <- nodeEdit(
        nodeInput(
          "merge-t0",
          "up-t0",
          "task" -> CJson.fromString("produce t0 artifact"),
          "description" -> CJson.fromString("t0 upstream"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
      ...
      n2 <- nodeEdit(
        nodeInput(
          "merge-t0",
          "m2",
          "task" -> CJson.fromString("merge-task"),
          "description" -> CJson.fromString("merge landing sink"),
          "out" -> CJson.fromString("Nebula"),
          "in" -> CJson.fromString(upId),
          "merge" -> CJson.fromBoolean(true)
        ),
        ctx
      )
      ...
      _ <- rt.store.mutateArchive(a =>
        a.copy(nodes =
          a.nodes + ("n-arch-x" -> NodeDef(
            id = "n-arch-x",
            name = "arch-x",
            agent = "test-agent",
    ...
    yield
      assert(
        n1.isLeft && n1.left.exists(_.contains("merge=true")),
        s"N1 merge+worktree must refuse with actionable text, got $n1"
      )
      assert(
        n0.isLeft && n0.left.exists(_.contains("NODE_MERGE_REQUIRES_UPSTREAM")),
      ...
      assert(n3.isLeft && n3.left.exists(_.contains("create-only")), s"N3 merge on archived node must refuse, got $n3")
    end for
  }
  ...
  test("S1: two upstreams completed → merge node triggers, lands --no-ff merges, zero worktree/branch residue") {
    ...
    for
      ...
      b <- byName(rt, "task-b")
      mergeInput <- llm.inputs.get.map(
        _.find(t => t.contains("=== Node task-a ===") && t.contains("=== Node task-b ==="))
      )
    ...
    yield
      ...
      assert(
        log.contains("merge: task-a") && log.contains("merge: task-b"),
      ...
      assert(!wts.exists(p => p.endsWith("task-a") || p.endsWith("task-b")), s"no batch worktree may remain, got $wts")
      assertEquals(wts.count(_.endsWith("ws-s1")), 1, "only the main worktree remains")
      assert(
        !branches.contains("feat/task-a") && !branches.contains("feat/task-b"),
  ...
  test("S2: one upstream BLOCKED → merge node NOT triggered (stays pending), escalation visible, nothing merged") {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "merge-s2",
          "task-a",
          "task" -> CJson.fromString("blocked-please"),
          "description" -> CJson.fromString("s2 upstream a"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          "merge-s2",
          "task-b",
          "task" -> CJson.fromString("produce artifact b"),
          "description" -> CJson.fromString("s2 upstream b"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
    ...
    yield
      ...
      assert(
        imms.exists(_.eventType.contains("blocked")),
  ...
  test(
    "S3: one upstream FAILED → merge node converts to blocked (visible, not hanging), nothing merged, Nebula notified"
  ) {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "merge-s3",
          "task-a",
          "task" -> CJson.fromString("boom-please"),
          "description" -> CJson.fromString("s3 upstream a"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "task-b",
          "task" -> CJson.fromString("produce artifact b"),
          "description" -> CJson.fromString("s3 upstream b"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
    ...
    yield
      ...
      assert(
        merge.blockedFeedback.exists(_.detail.contains("task-a")),
      ...
      assert(
        imms.exists(m => m.eventType.contains("blocked") && m.text.contains("merge-z")),
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "merge-s4",
          "task-a",
          "task" -> CJson.fromString("produce artifact a"),
          "description" -> CJson.fromString("s4 upstream a"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "task-b",
          "task" -> CJson.fromString("produce artifact b"),
          "description" -> CJson.fromString("s4 upstream b"),
          "out" -> CJson.fromString("Nebula")
        ),
        ctx
      )
