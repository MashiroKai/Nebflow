...
class MailChainBoundarySpec extends FunSuite:
  ...
  test("T1 mailChainIdSet: 声明链 ∪ 兜底派生链 ∪ 链级依赖目标 ∪ 台账登记面（含旧号别名）；未登记号仍不在集合内") {
    val nodes = List(
      n("u-1", 1),
      n("u-2", 2, in = List("u-1")), // 兜底分量（≥2 成员）⇒ chain-u-1
    ...
    val ledger = ChainLedger.State(
      entries = Map(
        "chain-ledger-canon" -> ChainLedger
          .Entry(chainId = "chain-ledger-canon", anchor = "chain-ledger-canon", bornAt = 1L)
      ),
      aliases = Map(
        "chain-legacy-old" -> ChainLedger.AliasRow(
          alias = "chain-legacy-old",
          canonical = "chain-ledger-canon",
          createdAt = 1L
        )
      )
    )
  ...
  test("T2 payloadChains: 声明轨（不论成员数）∪ 兜底轨（≥2 成员）+ 含活动成员约束") {
    val nodes = List(
      ...
      n("u-1", 2),
      n("u-2", 3, in = List("u-1")), // 兜底分量 ⇒ chain-u-1
    ...
    assert(!FlowMapStore.payloadChains(combined, Set("d-1")).map(_.id).contains("chain-u-1"), "无活动成员的分量不进主图旁挂（含活动成员约束）")

    // 🔴 变异承重（复核位复跑点）：把 chains[] 组装改回「成员 ≥2」纯兜底口径 ⇒ 下句失败
    val pureFallback = FlowMapStore
      .topologicalChains(nodes)
      .filter(c => c.memberIds.size >= 2 && c.memberIds.exists(active.contains))
      .map(_.id)
    assert(!pureFallback.contains("chain-declared-fx"), "纯兜底口径下声明单成员链缺席 ⇒ 判据 4 的承重红面（若本句失败，说明 chains[] 已不含声明轨）")
  }
  ...
  test("T3 mergeChainAttrs: 两个值（所属声明链 / 本次汇聚上游链）+ 旧键 chainIds 逐字保留") {
    ...
    assertEquals(
      FlowMapStore.mergeChainIds(combined, cs, "m"),
      Some(List("chain-e1", "chain-e2")),
      "旧键投影与 MergeChainAttrs.chainIds 同源（改造前语义逐字保留）"
    )
    assertEquals(
      FlowMapStore.mergeUpstreamChains(combined, cs, "m"),
      Some(List("chain-e1", "chain-e2")),
      "新键投影 = upstreamChains"
    )
  ...
  test("T3b mergeChainAttrs 门控: 仅 1 条入口可达成员链 ⇒ None（禁「主链 + 1 条」凑够 2）") {
    val nodes = List(
      n("e1", 1),
      n("m", 2, in = List("e1"), merge = true, status = NodeLifecycle.Wiring)
    )
    val combined = mapOf(nodes*)
    val cs = FlowMapStore.topologicalChains(nodes)
  ...
  test("T4 buildNodeJson: mergeUpstreamChains 条件键（注入才带；缺省键集零漂移）") {
    ...
    val withKey = NodePayload.buildNodeJson(
      m,
      0L,
      chainId = Some("chain-e1"),
      chainIds = Some(List("chain-e1", "chain-e2")),
      mergeUpstreamChains = Some(List("chain-e1", "chain-e2"))
    )
    val withoutKey =
      NodePayload.buildNodeJson(m, 0L, chainId = Some("chain-e1"), chainIds = Some(List("chain-e1", "chain-e2")))
    assertEquals(
      withKey.hcursor.downField("mergeUpstreamChains").as[List[String]].toOption,
      Some(List("chain-e1", "chain-e2"))
    )
    assertEquals(
      withoutKey.asObject.get.keys.toSet,
      withKey.asObject.get.keys.toSet - "mergeUpstreamChains",
