...
class NotifyDispatcherSpec extends CatsEffectSuite:
  ...
  test(
    "① notify-enabled node completion triggers dispatcher session (name/terminal/reason code in task text; result persisted + read hint)"
  ) {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "task" -> Json.fromString("notify-one"),
          "out" -> Json.fromString("(pass,failed)Nebula"),
          "notifyDispatcher" -> Json.fromBoolean(true)
        ),
        ctx
      )
    ...
    yield
      ...
      assert(
        audit.exists((t, id) => t == "dispatch-notify" && id == nodeId),
        s"dispatch-notify audit line must exist, got: $audit"
      )
    end for
  }
  ...
  test("② non-flagged node: no dispatcher notify; out=Nebula delivery unchanged (zero regression)") {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "plain-one",
          "description" -> Json.fromString("test node purpose"),
          "task" -> Json.fromString("plain-one"),
    ...
    yield
      ...
      val delivered =
        deliveries.filter((t, ev) => t.contains("[Node 'plain-one' completed]") && ev.contains("completed"))
  ...
  test("③ budget: exhaustion keeps node Completed + single supervisor notice (notice, no fake blocked) + no rescan") {
    ...
    yield
      ...
      assertEquals(
        ...
        "budget-exhausted node must STAY completed (no fake blocked)"
      )
      ...
      assert(
        !esc.head.contains("[Node 'budget-three' blocked]"),
        "notice must NOT use blocked semantics (kept completed)"
      )
  ...
  test(
    "④ single-entry reason gate: blocked stays reserved no-op; failed wired (status-gated, flag-independent, no double trigger with FeedbackRouter)"
  ) {
    for
    ...
    yield
      ...
      assert(
        calls.head.contains("gate-failed") && calls.head.contains("reason=failed"),
        s"failed trigger text, got: ${calls.headOption}"
      )
  ...
  test("⑤ redeliver: unmarked completion gets re-triggered; marked (persisted) never re-triggers across restart") {
    for
      ...
      _ <- store
        .mutate(s =>
          s.copy(nodes = s.nodes.updated("n-r3", s.nodes("n-r3").copy(notifySentAt = Some(System.currentTimeMillis()))))
        )
        .void
      ...
      dn2 = new DispatchNotify(
        ...
        emitUpdated = (_: NodeDef) => IO.unit,
        trigger = _ => IO.unit,
        budgetMax = DispatchNotify.DefaultBudget,
        windowMs = 0L
      )
      _ <- dn2.redeliver()
  ...
  test(
    "⑥ NodeEdit flag: create on / edit withdraw / domain guard (terminal refused, running allowed) / payload field / default off"
  ) {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "unflagged",
          "description" -> Json.fromString("test node purpose"),
          "task" -> Json.fromString("unflagged"),
          "out" -> Json.fromString("(pass,failed)Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "slow-runner",
          "description" -> Json.fromString("test node purpose"),
          "task" -> Json.fromString("slow-runner"),
          "out" -> Json.fromString("(pass,failed)Nebula"),
          "notifyDispatcher" -> Json.fromBoolean(true)
        ),
        ctx
    ...
    yield
      ...
      assertEquals(
        payload.hcursor.get[Boolean]("notifyDispatcher").toOption,
        Some(true),
        "conditional payload field when on"
      )
      ...
      assert(
        completedRefused.left.exists(_.contains("wiring/pending/running")),
        s"refusal must name the domain, got: $completedRefused"
      )
    end for
  }
  ...
  test("⑦ failed dedup: same node notifies exactly once (in-flight claim + persisted marker; flag-independent)") {
    ...
    yield
      assertEquals(calls.size, 1, "second notify for same node must be deduped")
      assert(
        calls.head.contains("fdup-node") && calls.head.contains("reason=failed"),
        s"failed trigger text, got: ${calls.headOption}"
      )
      assertEquals(after.notifySentAt.isDefined, true, "marker persisted after first trigger")
  }
  ...
  test("⑧ failed ledger split: completion budget exhaustion does NOT consume failed budget (independent Refs)") {
    ...
    yield
      ...
      assertEquals(
        calls.count(_.contains("reason=failed")),
        1,
        "failed budget must be independent of completion exhaustion"
      )
  ...
  test(
    "⑨ failed budget exhaustion: 3rd failed (budget=2) not triggered, kept failed, single notice (notice semantics), no rescan"
  ) {
    for
    ...
    yield
      ...
      assertEquals(
        ...
        "budget-exhausted node must STAY failed (terminal not flipped)"
      )
      assertEquals(b3After.notifySentAt.isDefined, true, "notifySentAt set → exits redeliver candidate set")
      ...
      assert(
        audit.exists((t, id) => t == "dispatch-notify" && id == "n-fb3"),
        s"failed budget-exhausted audit for fb3, got: $audit"
      )
      assertEquals(callsAfterScan.size, 2, "no rescan: budget-exhausted node not re-notified")
  }

  test(
    "⑩ failed window circuit breaker: threshold → merged escalation + cooldown; suppressed/cooldown-on unmarked; cooldown end → redeliver delivers (not lost)"
  ) {
    for
      (store, dn, triggered, escalated, _) <- mkUnit(
        "failed-window",
        failedWindowThreshold = 3,
        failedWindowMs = 250,
        failedCooldownMs = 300
      )
  ...
  test("⑪ failed redeliver backlog: unmarked failed re-triggered by scan; marked never re-triggered (across restart)") {
    for
      ...
      _ <- store
        .mutate(s =>
          s.copy(nodes =
            s.nodes.updated("n-fr2", s.nodes("n-fr2").copy(notifySentAt = Some(System.currentTimeMillis())))
          )
        )
        .void
      scan1 <- dn.redeliver()
      ...
      dn2 = new DispatchNotify(
        ...
        escalate = (_, _) => IO.unit,
        emitUpdated = (_: NodeDef) => IO.unit,
        trigger = t => triggered2.update(_ :+ t),
        budgetMax = DispatchNotify.DefaultBudget,
        windowMs = 0L
      )
      _ <- dn2.redeliver()
  ...
  test(
    "⑫ failed re-notify after reactivation: in-flight released + notifySentAt cleared → second real failure notifies again (retry loop, not cycle)"
  ) {
    for
      ...
      _ <- store.mutate(s => s.copy(nodes = s.nodes.updated("n-rn", s.nodes("n-rn").copy(notifySentAt = None)))).void
    ...
    yield
      assertEquals(
        calls.size,
        2,
        "reactivated node's second real failure must notify again — inFlight must not block forever"
      )
  ...
  test(
    "⑬ failed (out=Nebula) double delivery: Nebula gets eventType=failed AND dispatcher triggered; notify text/audit/marker asserts"
  ) {
    ...
    for
      ...
      _ <- waitUntil(20.seconds)(
        nebula.get.map(_.exists((t, ev) => t.contains("[Node 'fail-nebula' failed]") && ev.contains("failed")))
      )
      // 分发器触发（spawn prompt 捕获）
      _ <- waitUntil(20.seconds)(
        llm.inputs.get.map(_.exists(p => p.contains("[dispatch-notify]") && p.contains("fail-nebula")))
      )
      prompts <- llm.inputs.get
    ...
    yield
      val prompt = prompts
        .find(p => p.contains("[dispatch-notify]") && p.contains("fail-nebula"))
      ...
      assert(
        prompt.contains("NodeList(detail=") && prompt.contains(nodeId),
        "prompt must carry result read hint (NodeList detail)"
      )
      ...
      assert(
        audit.exists((t, id) => t == "dispatch-notify" && id == nodeId),
        s"dispatch-notify failed audit must exist, got: $audit"
      )
      assert(
        deliveries.exists((t, ev) => t.contains("[Node 'fail-nebula' failed]") && ev.contains("failed")),
  ...
  test("⑭ failed (out=None): dispatcher still triggered (no out-wiring dependency); no Nebula delivery for the node") {
    ...
    for
      ...
      _ <- rt.engine.settleStaleRunningNodes()
      _ <- waitUntil(20.seconds)(
        llm.inputs.get.map(_.exists(p => p.contains("[dispatch-notify]") && p.contains("fail-dangling")))
      )
    ...
    yield
      ...
      assert(
        !deliveries.exists((t, ev) => t.contains("[Node 'fail-dangling'") || ev.contains("failed")),
  ...
  test("⑮ failed (out=node): downstream stays waiting (D5 zero-settlement) AND dispatcher triggered with waiter list") {
    ...
    for
      ...
      _ <- waitUntil(20.seconds)(
        llm.inputs.get.map(_.exists(p => p.contains("[dispatch-notify]") && p.contains("fail-up")))
      )
    ...
    yield
      ...
      assertEquals(
        up.notifySentAt.isDefined,
        true,
        "dispatcher notified for failed upstream (settle-then-notify order)"
      )
      // D5 翻转（原 collect 断言）：下游停等零结算——不启动、无 result、无占位键
      assertEquals(
        dn.status,
        NodeLifecycle.Wiring,
        "downstream stays wiring (D5 zero-settlement — collect placeholder abolished)"
      )
      ...
      assert(
        prompt.exists(p => p.contains("下游等待者") && p.contains("down-node")),
