...
import nebflow.shared.{Defaults, LlmHandle, LlmRequest, LlmResponse, Message, MessageRole, StreamChunk}
...
class InterruptRecoverySpec extends CatsEffectSuite:
  ...
  test("R1a: graceful shutdown flips every Running node to interrupted (bgWait cleared, sessionRef/deliveredTo kept, result untouched) and leaves other statuses alone") {
    ...
    for
      ...
      _ <- seedNode(rt, "n-r1a-1", "run-a", "task a", sessionRef = Some("node-r1a1"),
        bgWait = Some("1 background task(s): 'compile'"), deliveredTo = List("n-up"))
    ...
    yield
      ...
      assert(!NodeLifecycle.Terminal.contains(NodeLifecycle.Interrupted),
      ...
      assert(events.exists(e => e.contains("\"interrupted\"")),
        s"interrupted audit trail expected, got: ${events.mkString("|").take(400)}")
      assert(frames.exists(f => f.hcursor.get[String]("type").toOption.contains("nodeUpdated")),
        s"nodeUpdated WS frame expected, got: ${frames.map(_.noSpaces.take(80)).mkString("|").take(300)}")
      assert(frames.exists(f => f.hcursor.downField("node").get[String]("status").toOption.contains(NodeLifecycle.Interrupted)),
        "WS payload must carry status=interrupted (NodePayload passthrough)")
  }

  test("R1a-guard: while draining, both failure writers refuse (failNode via class-c claim on Running AND on interrupted, autoFailDeadRunning via watchdog) and never deliverFailed") {
    ...
    yield
      ...
      assertEquals(g2.status, NodeLifecycle.Running, "watchdog refuses the dead-session failed convergence while draining")
      ...
      assert(events.exists(e => e.contains("\"interrupted\"") && e.contains("dead-session failed write suppressed while draining")),
  ...
  test("R1a-guard-control: with draining NOT set the same paths DO converge to failed (proves the guard is load-bearing)") {
  ...
  test("R1b: interrupted node is claimed by the boot sweep and resumes from the SAME session checkpoint (not a fresh rerun from the task)") {
    ...
    for
      ...
      _ = seedTranscript(sid, List(
        msg(MessageRole.User, "原始任务：产出报告 R"),
        msg(MessageRole.Assistant, "INTERRUPTED-HALFWAY-CONTEXT")))
      _ <- seedNode(rt, "n-r1b", "mid-a", "产出报告 R",
        status = NodeLifecycle.Interrupted, sessionRef = Some(sid))
    ...
    yield
      ...
      assert(reqs.exists(_.contains("INTERRUPTED-HALFWAY-CONTEXT")),
        "the LLM request must carry the hydrated transcript (progress not lost)")
      assert(turns.exists(_.contains("crashed during your last turn and restarted")),
        s"resume prompt expected, got: ${turns.headOption.map(_.take(300))}")
      assert(events.exists(e => e.contains("boot-recovery") && e.contains("n-r1b") && e.contains("claimed")),
  ...
  test("R1b-class-c: interrupted node with a lost transcript fails through the normal chain (failed notification is legitimate)") {
    ...
    for
      ...
      _ <- seedNode(rt, "n-r1bc", "lost-a", "lost task",
        status = NodeLifecycle.Interrupted, sessionRef = Some("node-r1bcnofile"))
      (trig, trigger) <- triggerRecorder
      actions <- ProjectCrashRecovery.recoverProject(rt, trigger = trigger)
      n <- byName(rt, "lost-a")
      events <- readEvents(ws)
    ...
    yield
      ...
      assert(n.result.exists(_.contains("crash recovery: session transcript lost/corrupt")),
        s"class-c reason expected, got ${n.result}")
      assert(imms.exists(_.text.contains("[Node 'lost-a' failed]")),
  ...
  test("R1c: degraded mode (crashRecovery disabled) — interrupted node stays put across ≥2 watchdog ticks and every other sweep (zero convergence, zero notify, zero events)") {
    ...
    for
      ...
      _ <- seedNode(rt, "n-r1c", "degraded-a", "degraded task",
        status = NodeLifecycle.Interrupted, sessionRef = Some("node-r1cx"))
  ...
  test("R1d: rollback switch off — no flip, no draining (guards inert), eligibility set excludes interrupted, failure path unchanged") {
    ...
    yield
      ...
      assert(!events.exists(_.contains("\"interrupted\"")), s"zero interrupted events, got: ${events.mkString("|").take(300)}")
      assertEquals(actions, 1, "Running residue is still claimed (pre-batch eligibility)")
      assertEquals(a2.status, NodeLifecycle.Failed, "failure path unchanged (guard inert — same shape as R1a-guard-control)")
      assert(a2.result.exists(_.contains("crash recovery: session transcript lost")),
        s"class-c reason unchanged, got ${a2.result}")
      assertEquals(left.status, NodeLifecycle.Interrupted,
      ...
      assert(events2.forall(e => !(e.contains("\"interrupted\"") && e.contains("left-b"))),
        "no event ever touches the interrupted residue")
  }

  // ══ R1e 超时降级路径（可注入超时）════════════════════════════════════

  test("R1e: injectable timeout — deadline gives up on the remaining nodes, which fall back to the kill -9 style boot sweep") {
    ...
    for
      ...
      _ = seedTranscript(sidA, List(
        msg(MessageRole.User, "tick task a"),
        msg(MessageRole.Assistant, "TICK-HALF-CONTEXT")))
    ...
    yield
      ...
      assertEquals(actions, 2, "BOTH the interrupted node and the residual Running node are eligible for the ordinary sweep")
      ...
      assertEquals(b2.status, NodeLifecycle.Failed, "residual Running node is disposed by the ordinary sweep (class c here)")
      assert(b2.result.exists(_.contains("crash recovery: session transcript lost/corrupt")),
        s"class-c reason expected, got ${b2.result}")
  }

  test("R1e-zero: timeout 0 degrades deterministically (zero flips → the kill -9 shape, still recovered by the boot sweep)") {
  ...
  test("R2: NodeMessage to an interrupted node goes the undelivered path (recorded on task — never refused as terminal)") {
    ...
    for
      ...
      _ <- seedNode(rt, "n-r2", "sleep-a", "sleep task",
        status = NodeLifecycle.Interrupted, sessionRef = Some("node-r2gone"))
    ...
    yield
      ...
      assert(n.task.exists(_.contains("补充指示：改用方案 B")),
      ...
      assert(events.exists(e => e.contains("node-message") && e.contains("not-delivered")),
  ...
  test("R2-live: NodeMessage to an interrupted node with a live session is still injected (no message loss in the hook window)") {
    ...
    for
      ...
      _ <- seedNode(rt, "n-r2l", "live-a", "live task",
        status = NodeLifecycle.Interrupted, sessionRef = Some(sid))
  ...
  test("R2-reactivate: NodeEdit on an interrupted node reactivates it (fresh rerun, NEW session) and it runs to completion") {
    ...
    for
      ...
      _ <- seedNode(rt, "n-r2r", "manual-a", "manual task",
        status = NodeLifecycle.Interrupted, sessionRef = Some("node-r2rold"))
      _ <- {
        val ctx = ToolContext(
