...
class DeferredWiringSpec extends CatsEffectSuite:
  ...
  test("B① 缺口③ 编辑期：running 目标的新控制边入待接线队列（不整单拒）；目标离开 running 后自动接线 + 落痕") {
    ...
    for
      ...
      _ <- rt.store.mutate(s => s.copy(nodes = s.nodes.updated("n-w2",
        s.nodes("n-w2").copy(status = NodeLifecycle.Completed, result = Some("w2 done")))))
    ...
    yield
      ...
      assert(evs1.exists((t, id, _) => t == FlowMapEventLog.WiringDeferredType && id == v1.id),
        s"a ${FlowMapEventLog.WiringDeferredType} audit line is the mechanism's visibility condition, got: $evs1")
      assert(OutEdge.canonical(v2.out).contains(LoopW2), s"leaving running must auto-wire the queued edge, got: ${v2.out}")
      assert(v2.pendingOut.isEmpty, s"the queue must drain on wiring, got: ${v2.pendingOut}")
      assert(evs2.exists((t, id, _) => t == FlowMapEventLog.WiringAppliedType && id == v2.id),
  ...
  test("B② 缺口③ 边界不得放宽：指向 running 目标的**新增非控制边**照旧拒，且带错误码") {
    ...
    for
      ...
      r0 <- nodeEdit(nodeInput("defer-b", "t-plain",
        "description" -> Json.fromString("plain task node"),
        "task" -> Json.fromString("t-plain task")), ctx)
      r <- nodeEdit(nodeInput("defer-b", "t-plain",
        "description" -> Json.fromString("plain task node"),
        "out" -> Json.fromString("n-run")), ctx)
      _ <- stop(system)
    yield
      assert(r0.isRight, s"creating a dangling node must be accepted, got: $r0")
      val msg = r.fold(identity, ok => fail(s"a real input edge into a running target must still be rejected, got: $ok"))
  ...
  test("B③ 缺口③ 过宽面收窄：**已接线**的控制边在目标 running 期间编辑 ⇒ 放行且仍在 out（不摘进队列）") {
    ...
    for
      ...
      v0 <- nodeByName(rt, "v")
      _ <- rt.store.mutate(s => s.copy(nodes = s.nodes.updated("n-w", s.nodes("n-w").copy(status = NodeLifecycle.Running))))
    ...
    yield
      ...
      assert(OutEdge.canonical(v1.out).contains(LoopW),
  ...
  test("B④ 对偶两面同判（硬）：编辑期与运行期对 Running 目标同判「不在此刻动＋留痕」；仅改一面 ⇒ 本用例必红") {
    ...
    yield
      ...
      assertEquals(w2.status, NodeLifecycle.Running,
      ...
      assert(evs.exists((t, _, _) => t == FlowMapEventLog.WiringDeferredType),
        "edit face must leave a trace (the queue's visibility condition)")
      // ── 面②：运行期（NodeEngine:4324-4328 回边腿）─────────────────────
      assert(NodeEngine.loopReworkAdmission(NodeLifecycle.Running).isLeft,
        "runtime face: the same judgement — a running target is excluded from the loop re-run reset")
      assert(NodeEngine.loopReworkAdmission(NodeLifecycle.Running).left.exists(_.contains("running")),
        "the exclusion reason must name the frozen state")
      assertEquals(NodeEngine.loopReworkAdmission(NodeLifecycle.Completed), Right(()),
      ...
      assertEquals("NodeEngine\\.loopReworkAdmission\\(".r.findAllIn(src).size, 1,
      ...
      assert(reloopIdx > 0 && callIdx > reloopIdx && bodyEnd > callIdx,
  ...
  test("B⑤ 零回归负控：无待接线项时扫描腿零写零事件（幂等）") {
    ...
    yield
      ...
      assert(!evs.exists((t, _, _) =>
        t == FlowMapEventLog.WiringAppliedType || t == FlowMapEventLog.WiringDeferredType),
