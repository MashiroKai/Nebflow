...
class NodeEdgeGatingSpec extends CatsEffectSuite:
  ...
  test("④a fan-out '(pass)B, (failed)C': completion reaches B only; C (failed gate) keeps waiting") {
    ...
    val llm = new GateLlm(
      replyOf = t => if t.contains("seed-fan-a") then "a-result-token" else "ok",
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-fanout",
          "fan-a",
          "description" -> Json.fromString("fan source"),
          "task" -> Json.fromString("seed-fan-a"),
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-fanout",
          "fan-b",
          "description" -> Json.fromString("pass consumer"),
          "task" -> Json.fromString("b-consumes-a"),
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-fanout",
          "fan-c",
          "description" -> Json.fromString("failed fallback"),
          "task" -> Json.fromString("c-own-task"),
          "in" -> Json.fromString(aId),
          "out" -> Json.fromString("Nebula")
        ),
        ctx
      ...
      rew <- nodeEdit(nodeInput("gating-fanout", "fan-a", "out" -> Json.fromString(s"(pass)$bId, (failed)$cId")), ctx)
    ...
    yield
      ...
      assertEquals(
        aAfter.out,
        List(OutEdge(bId), OutEdge(cId, Set("failed"))),
      ...
      assert(
        bInput.exists(_.contains("=== Node fan-a ===")) && bInput.exists(_.contains("a-result-token")),
  ...
  test("④b '(failed)C:signal': upstream failure starts C via signal (own task input, no error payload)") {
    ...
    val llm = new GateLlm(
      failWhen = t => t.contains("seed-sig-fail"),
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-signal",
          "sig-a",
          "description" -> Json.fromString("doomed source"),
          "task" -> Json.fromString("seed-sig-fail"),
          "out" -> Json.fromString("Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-signal",
          "sig-b",
          "description" -> Json.fromString("signal fallback"),
          "task" -> Json.fromString("b-own-task"),
          "in" -> Json.fromString(aId),
          "out" -> Json.fromString("Nebula")
        ),
        ctx
      ...
      bId <- idOf(rt, "sig-b")
      // 改接 A：失败纯信号边（P1 新表达力）
      rew <- nodeEdit(nodeInput("gating-signal", "sig-a", "out" -> Json.fromString(s"(failed)$bId:signal")), ctx)
    ...
    yield
      ...
      assertEquals(
        aAfter.out,
        List(OutEdge(bId, Set("failed"), "signal")),
      ...
      assert(
        bAfter.deliveredTo.contains(aId),
        s"signal edge must settle the barrier bookkeeping, got ${bAfter.deliveredTo}"
      )
      // signal 模式：不投载荷——B 的输入 = 自身 task（无 A 错误文本、无上游结果段）
      assert(bInput.isDefined, "B must have run with its own task input")
      assert(
        !bInput.exists(_.contains("boom-exploded")),
        s"B's input must NOT carry A's error text, got: ${bInput.map(_.take(300))}"
      )
      assert(
        !bInput.exists(_.contains("=== Node ")),
        s"B's input must NOT carry any upstream payload section, got: ${bInput.map(_.take(300))}"
      )
    end for
  }
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-mergegate",
          "mg-up",
          "description" -> Json.fromString("upstream"),
          "task" -> Json.fromString("mg-up-task"),
          "out" -> Json.fromString("Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "out" -> Json.fromString("Nebula"),
          "task" -> Json.fromString("landing commands")
        ),
        ctx
      )
      mId <- idOf(rt, "mg-sink")
      ...
      createRes <- nodeEdit(
        nodeInput(
          "gating-mergegate",
          "mg-attacker",
          "description" -> Json.fromString("attacker"),
          "task" -> Json.fromString("attacker task"),
          "out" -> Json.fromString(s"(failed)$mId")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-mergegate",
          "mg-editor",
          "description" -> Json.fromString("editor"),
          "task" -> Json.fromString("editor task"),
          "out" -> Json.fromString("Nebula")
        ),
        ctx
      )
      editRes <- nodeEdit(nodeInput("gating-mergegate", "mg-editor", "out" -> Json.fromString(s"(failed)$mId")), ctx)
  ...
  test(
    "⑤b WARNING: downstream holding in-edge from a FAILED upstream without on-failed edge gets a non-blocking warning"
  ) {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-warn",
          "warn-up",
          "description" -> Json.fromString("doomed upstream"),
          "task" -> Json.fromString("seed-warn-fail"),
          "out" -> Json.fromString("Nebula")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          "gating-warn",
          "warn-ok-up",
          "description" -> Json.fromString("healthy upstream"),
          "task" -> Json.fromString("ok-up-task"),
          "out" -> Json.fromString("Nebula"),
          "notify" -> Json.fromString("dispatcher")
        ),
        ctx
      )
      ...
      quiet <- nodeEdit(
        nodeInput(
          "gating-warn",
          "warn-dn2",
          "description" -> Json.fromString("quiet downstream"),
          "task" -> Json.fromString("dn2-task"),
          "in" -> Json.fromString(okUpId),
          "out" -> Json.fromString("Nebula"),
          "notify" -> Json.fromString("dispatcher")
        ),
        ctx
    ...
    yield
      ...
      // 对照：健康上游零噪音
      assert(
        quiet.isRight && !quiet.toOption.getOrElse("").contains("⚠"),
  ...
  test("codec dual-read: legacy string / Nebula / absent out decode per G1; persisted shape stays edge-array") {
    ...
    yield
      ...
      assertEquals(
        aOut.map(_.noSpaces.replace(" ", "")),
        Some("""[{"to":"n-b","on":["pass"],"mode":"result"}]"""),
        s"persisted shape must be the edge array, got: $aOut"
      )
      assert(
        cOut.map(_.noSpaces).exists(_.contains("\"pass\"")) && cOut.map(_.noSpaces).exists(_.contains("\"failed\"")),
