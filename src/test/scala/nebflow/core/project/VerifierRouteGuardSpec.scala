...
class VerifierRouteGuardSpec extends CatsEffectSuite:
  ...
  private def verifierFixture(store: FlowMapStore, now: Long, workerStatus: String = NodeLifecycle.Completed): IO[Unit] =
    for
      _ <- seed(store, NodeDef(id = "n-work", name = "WORK", agent = "general", status = workerStatus,
        task = Some("work"), result = Some("round-1 output"),
        completedAt = Some(now - 30_000), createdAt = now - 90_000))
      _ <- seed(store, NodeDef(id = "n-ver", name = "VER", agent = "general", status = NodeLifecycle.Pending,
      ...
      _ <- seed(store, NodeDef(id = "n-land", name = "LAND", agent = "general", status = NodeLifecycle.Pending,
        task = Some("land"), in = List("n-ver"), createdAt = now - 50_000))
    yield ()
  ...
  private def assertRejectionState(
    ...
    for
      ...
      ver <- node(rt, verId)
      _ <- IO(println(s"[spec] $leg — $verId verifierRoute=$key ; out=${ver.out} ; " +
        s"verifier-route-lost=${lost.map(l => s"[nodeId=${l._2}] ${l._3}")} ; payload keys=${keys.mkString(",")}"))
    yield
      assertEquals(key, Some("lost"),
        s"$leg: the victim verifier must carry the derived rejection-state key (verifierRoute=lost)")
      assertEquals(lost.map(_._2).distinct, List(verId),
        s"$leg: the audit subject must be the VICTIM VERIFIER, not the retired node (got ${lost.map(_._2)})")
      lostTarget.foreach { tgt =>
        assert(lost.exists(_._3.contains(tgt)),
          s"$leg: the line must name the detached fail target '$tgt', got ${lost.map(_._3)}")
      }
      assert(lost.exists(s => s._3.contains("(fail)") || s._3.contains("NODE_VERIFIER_NEEDS_ROUTE")),
        s"$leg: the line must carry the actionable restore form, got ${lost.map(_._3)}")

  // ── R1 / G1：工具腿（abandon）────────────────────────────────────

  test("R1/G1 abandon leg: the detach semantics stay byte-for-byte, but the victim verifier lands in the REJECTION STATE in the same frame (payload verifierRoute=lost + verifier-route-lost with the verifier as subject)") {
    ...
    for
      ...
      _ <- IO(println(s"[spec] R1 BEFORE — verifier.out=${ver.out} ; verifierRoute=$keyBefore " +
    ...
    yield
      ...
      assertEquals(work.out, List(OutEdge.nebula), "the retiring node's own out still collapses to the Nebula exit marker")
      // ② 摘边照摘（本批不改这个退场语义）：受害 verifier 的 fail 边被摘除，pass 边保留
      assertEquals(verAfter.out, List(OutEdge("n-land")),
      ...
      assert(audit.exists((t, id, s) => t == "abandoned" && id == "n-work" && s.contains("out-refs=1")),
  ...
  test("R3/A4 verdict leg: a verifier that reports fail with no usable fail route leaves a rejection-state trace instead of a benign-degradation line") {
    ...
    for
      ...
      _ <- seed(rt.store, NodeDef(id = "n-work", name = "WORK", agent = "general", status = NodeLifecycle.Completed,
        task = Some("work"), result = Some("round-1 output"), completedAt = Some(now - 30_000), createdAt = now - 90_000))
      _ <- seed(rt.store, NodeDef(id = "n-ver", name = "VER", agent = "general", status = NodeLifecycle.Wiring,
        task = Some("judge"), in = List("n-work"), deliveredTo = List("n-work"), role = NodeRoles.Verifier,
        out = List(OutEdge("n-land")), createdAt = now - 60_000))
      _ <- seed(rt.store, NodeDef(id = "n-land", name = "LAND", agent = "general", status = NodeLifecycle.Pending,
        task = Some("land"), in = List("n-ver"), createdAt = now - 50_000))
      keyBefore <- routeKeyOf(rt, "n-ver")
      _ <- IO(println(s"[spec] R3 BEFORE — route-less verifier payload key=$keyBefore " +
      ...
      _ <- waitUntil(40.seconds) {
        rt.store.getNode("n-ver").map(_.exists(n => n.status == NodeLifecycle.Completed || n.status == NodeLifecycle.Failed))
      }
      ver <- node(rt, "n-ver")
    ...
    yield
      ...
      assertEquals(ver.lastVerdict, Some("fail"), "the fail verdict is still recorded (verdict ≠ node status, unchanged)")
      ...
      assert(audit.exists((t, id, _) => t == "verifier-route-lost" && id == "n-ver"),
        s"the rejection state must be audited with the verifier as subject, got ${audit.map(_._1)}")
  }

  // ── G2：自动解禁（纯派生键，补回即消失）─────────────────────────

  test("G2 auto-release: re-declaring '(fail)<worker2>:loop' clears the key with zero extra unblock action (purely derived, no migration)") {
    ...
    for
      ...
      _ <- verifierFixture(rt.store, now)
      _ <- seed(rt.store, NodeDef(id = "n-work2", name = "WORK2", agent = "general", status = NodeLifecycle.Pending,
        task = Some("work-2"), createdAt = now - 20_000))
      ctx = mkCtx(res, system, ws.toString)
      ...
      r <- nodeEdit(nodeInput("vrg-g2", "VER",
        "out" -> Json.fromString("(pass)LAND, (fail)WORK2:loop")), ctx)
      released <- routeKeyOf(rt, "n-ver")
      ver <- node(rt, "n-ver")
      _ <- IO(println(s"[spec] G2 step 2 — after re-wiring the key=$released ; out=${ver.out} ; pendingOut=${ver.pendingOut}"))
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield
      ...
      assertEquals(released, None,
        "the key MUST vanish by itself (derived ⇒ zero extra unblock action, zero migration, zero backfill)")
      assert(ver.out.exists(e => e.on.contains(OutEdge.Fail) && OutEdge.isLoopEdge(e)),
        s"the restored route must be wired into out, got ${ver.out}")
  }

  // ── G3：pendingOut 计入「已声明」（防假阳性）─────────────────────

  test("G3 pendingOut counts as declared: a fail route queued for a RUNNING target is NOT a rejection state (no false positive inside the real auto-wiring window)") {
    ...
    for
      ...
      _ <- seed(rt.store, NodeDef(id = "n-work3", name = "WORK3", agent = "general", status = NodeLifecycle.Running,
        task = Some("work-3"), startedAt = Some(now - 10_000), createdAt = now - 20_000))
      ctx = mkCtx(res, system, ws.toString)
      ...
      r <- nodeEdit(nodeInput("vrg-g3", "VER",
        "out" -> Json.fromString("(pass)LAND, (fail)WORK3:loop")), ctx)
      verQueued <- node(rt, "n-ver")
      keyQueued <- routeKeyOf(rt, "n-ver")
      auditQueued <- readAudit(ws)
      _ <- IO(println(s"[spec] G3 step 2 (target running) — key=$keyQueued ; out=${verQueued.out} ; " +
      ...
      _ <- rt.store.mutate(s => s.copy(nodes = s.nodes.updated("n-work3",
        s.nodes("n-work3").copy(status = NodeLifecycle.Pending)))).void
      _ <- rt.engine.applyDeferredWiring()
      verWired <- node(rt, "n-ver")
      keyWired <- routeKeyOf(rt, "n-ver")
      auditWired <- readAudit(ws)
      _ <- IO(println(s"[spec] G3 step 3 (target left running) — key=$keyWired ; out=${verWired.out} ; " +
    ...
    yield
      ...
      assert(verQueued.pendingOut.exists(e => e.on.contains(OutEdge.Fail) && OutEdge.isLoopEdge(e)),
      ...
      assert(verWired.out.exists(e => e.on.contains(OutEdge.Fail) && OutEdge.isLoopEdge(e)),
  ...
  test("boundary matrix: empty-out verifier is legal (two-phase token form); a DANGLING fail target counts as invalid (same yardstick as the edit-time gate); a healthy route and a non-verifier are never flagged") {
    ...
    for
      ...
      _ <- seed(rt.store, NodeDef(id = "n-work", name = "WORK", agent = "general", status = NodeLifecycle.Pending,
        task = Some("work"), createdAt = now - 90_000))
      _ <- seed(rt.store, NodeDef(id = "n-land", name = "LAND", agent = "general", status = NodeLifecycle.Pending,
        task = Some("land"), createdAt = now - 80_000))
      // (a) 健康路由 ⇒ 缺键
      _ <- seed(rt.store, NodeDef(id = "n-healthy", name = "HEALTHY", agent = "general", status = NodeLifecycle.Wiring,
        task = Some("judge"), role = NodeRoles.Verifier,
        out = List(OutEdge("n-land"), OutEdge("n-work", Set(OutEdge.Fail), OutEdge.Loop)),
      ...
      _ <- seed(rt.store, NodeDef(id = "n-emptyout", name = "EMPTYOUT", agent = "general", status = NodeLifecycle.Pending,
        task = Some("judge"), role = NodeRoles.Verifier, out = Nil, createdAt = now - 60_000))
      // (c) 边界 (ii)：fail 边目标悬空（n-ghost 不在活动区）⇒ **计非法**
      _ <- seed(rt.store, NodeDef(id = "n-dangling", name = "DANGLING", agent = "general", status = NodeLifecycle.Wiring,
        task = Some("judge"), role = NodeRoles.Verifier,
        out = List(OutEdge("n-land"), OutEdge("n-ghost", Set(OutEdge.Fail), OutEdge.Loop)),
        createdAt = now - 50_000))
      // (d) 非 verifier（task 带 :loop 的畸形存量）⇒ 恒不报（角色门）
      _ <- seed(rt.store, NodeDef(id = "n-taskloop", name = "TASKLOOP", agent = "general", status = NodeLifecycle.Wiring,
        task = Some("work"), role = NodeRoles.Task,
        out = List(OutEdge("n-land"), OutEdge("n-ghost", Set(OutEdge.Fail), OutEdge.Loop)),
      ...
      work <- node(rt, "n-work")
      _ <- IO(println(s"[spec] boundary — healthy=$healthy emptyout=$emptyout dangling=$dangling taskloop=$taskloop ; " +
  ...
  test("G4 negative control: the guard adds exactly one conditional key — healthy nodes keep their payload key set byte-for-byte") {
    ...
    for
      ...
      _ <- IO(println(s"[spec] G4 payload key sets — verifier=${verKeys.mkString(",")} ; " +
        s"land=${landKeys.mkString(",")} ; work=${workKeys.mkString(",")}"))
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield
      // 健康 verifier：role 键在（非 task 携带），verifierRoute 键**不在**
      assert(verKeys.contains("role") && verKeys.contains("out") && !verKeys.contains("verifierRoute"),
      ...
      for (id, ks) <- List("n-land" -> landKeys, "n-work" -> workKeys) do
        assert(!ks.contains("role") && !ks.contains("verifierRoute"),
