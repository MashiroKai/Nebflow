...
class CancelDeadlockFixSpec extends CatsEffectSuite:
  ...
  private def mountProject(
    ...
    for
      ...
      engine = new NodeEngine(
        ...
        // 腿 2 默认开行为由 NodeReportReminderSpec 覆盖）。本 spec 的取消/终态断言不得
        // 被「未申报 hold」截断。
  ...
  test(
    "R1-A1: cancelled terminal triggers the dispatcher — text carries cancelled-only guidance (承接/改接/abandon), NOT the failed四步 reactivate recipe; flag not consulted; second call no-op"
  ) {
    for
    ...
    yield
      ...
      assert(
        cText.contains("承接") && cText.contains("改接") && cText.contains("abandon"),
      ...
      assert(!cText.contains("NodeEdit 编辑该节点任意实际变更"), "cancelled text must not copy the failed step-1 recipe")
      assert(cText.contains("请勿照用"), "cancelled text must explicitly forbid reusing the failed recipe")
      // failed 侧逐字保留（本批 failed 文本零变化）
      assert(
        fText.contains("NodeEdit 编辑该节点任意实际变更") && fText.contains("原节点复活"),
  ...
  test(
    "R1-A2: cancelled budget exhaustion keeps the node cancelled + single-flight notice; accounting is INDEPENDENT of the failed budget"
  ) {
    for
    ...
    yield
      ...
      assertEquals(
        n2After.notifySentAt.isDefined,
        true,
        "exhausted node must be marked (leaves the redeliver candidate set)"
      )
      ...
      assert(
        fired.exists(_.contains("fb1")) && fired.exists(_.contains("reason=failed")),
      ...
      assert(
        esc.head.contains("cancelled 通知预算耗尽") && esc.head.contains("承接 / 改接 / abandon"),
  ...
  test(
    "R1-A3: cancel-window threshold → cooldown (merged notice, unmarked); in-cooldown cancels suppressed WITHOUT marking → redeliver after cooldown delivers the backlog; failed window unaffected"
  ) {
    val winMs = 400L
    for
      (store, dn, triggered, escalated, _) <- mkNotify(
        "window",
        cancelledWindowThreshold = 2,
        cancelledWindowMs = winMs,
        cancelledCooldownMs = winMs,
        failedWindowThreshold = 2,
        failedWindowMs = winMs,
        failedCooldownMs = winMs
      )
    ...
    yield
      ...
      assert(
        escMid.head.contains("节点取消通知达 2 次") || escMid.head.contains("取消通知"),
        s"cooldown notice must be the cancel-window one: ${escMid.head}"
      )
      assert(
        firedAfterFailed.exists(_.contains("wf")) && fMid.notifySentAt.isDefined,
      ...
      assert(
        firedFinal.exists(_.contains("w2")) && firedFinal.exists(_.contains("w3")),
  ...
  test(
    "R2/R4/R7-integration: dead-session reap (source=engine) writes result + detaches out→Nebula + prunes downstream in + marks 待承接 + emits nodeUpdated + audits cancelled/barrier-blocked; downstream stays held"
  ) {
    ...
    yield
      ...
      assert(
        cancelledEv.head._3.contains("source=engine"),
        s"R2/R7 audit must carry the source: ${cancelledEv.head._3}"
      )
      assert(
        cancelledEv.head._3.contains("successors awaiting handover: n-b"),
      ...
      assertEquals(
        barrierEv.size,
        1,
        s"R3 immediate barrier alert expected for n-b, got ${audit.map((t, id, _) => (t, id))}"
      )
      assert(
        barrierEv.head._3.contains("pendingSuccession") && barrierEv.head._3.contains("cause=cancelled"),
      ...
      assert(
        texts.head.contains("已被**取消**") && texts.head.contains("不可重激活"),
        s"engine-side trigger must carry the cancelled-only guidance: ${texts.head.take(200)}"
      )
      assert(
        texts.head.contains("下游等待者") && texts.head.contains("B(n-b)"),
      ...
      val bFrame = wsFrames.find(j =>
        j.hcursor.get[String]("type").contains("nodeUpdated")
          && j.hcursor.get[String]("nodeId").contains("n-b")
      )
      assert(
        bFrame.isDefined,
        s"R4 visibility: nodeUpdated frame for the pruned successor expected, got ${wsFrames.size} frames"
      )
      assert(
        bFrame.get.hcursor.get[List[String]]("pendingSuccession").toOption.contains(List("n-a")),
  ...
  test(
    "R3-dedup-b: immediate barrier check obeys the legal-wait exemptions — a still-running sibling upstream (and a dangling ref) must NOT raise the alert, while the R5-方案4 failed re-judge leaves the graph intact"
  ) {
    ...
    for
      ...
      _ <- seed(
        ...
        NodeDef(
          ...
          agent = "general",
          status = NodeLifecycle.Cancelled,
          result = Some("cancelled[source=engine]: reason=x"),
          sessionRef = Some("node-sess-l3b"),
          out = List(OutEdge("n-d"), OutEdge("n-x")),
  ...
  test(
    "R5-方案4: settleFailedHardResume re-judges the L3-cancelled node as FAILED via the failNode chain — out kept (no detach / no pendingSuccession, D5 stop-wait) + exactly one FAILED notification + hard-recovery audit; replay is a state-guarded no-op"
  ) {
    ...
    yield
      ...
      assert(
        u.completedAt.isDefined && u.ttlExpireAt.isEmpty,
        "failed finalize must stamp completedAt and keep the node (no TTL sweep)"
      )
      // ② failed 侧零摘除 / 零标记（D5 零结算停等）——R4 硬约束：不得把摘除扩展到 failed
      assertEquals(
        u.out,
        List(OutEdge("n-d")),
        "failed side must NOT detach the out edge (R4→failed permanently rejected)"
      )
      ...
      assert(
        u.notifySentAt.isDefined,
        "the failed notification must land notifySentAt (chainArchivable entry condition)"
      )
      assertEquals(
        fired1.size,
        1,
        s"exactly one dispatcher notification expected, got ${fired1.size}: ${fired1.map(_.take(80))}"
      )
      val text = fired1.head
      assert(text.contains("reason=failed"), s"notification must be the failed variant: ${text.take(200)}")
      assert(
        text.contains("L3 hard-recovery resume failed"),
      ...
      assert(
        text.contains("下游等待者（failed 零结算停等中"),
        s"D5 stop-wait must be visible in the notification: ${text.take(400)}"
      )
      ...
      assert(
        hr.head._3.contains("L3 resume FAILED") && hr.head._3.contains("re-judged as failed"),
      ...
      assertEquals(
        canc.size,
        0,
        "no NEW cancelled event is written by the fallback (the historical one, if any, is never rewritten)"
      )
      ...
      assertEquals(
        bEv.size,
        1,
        s"the failed-side terminal write point must raise the immediate alert, got ${audit1.map((t, id, _) => (t, id))}"
      )
      ...
      assertEquals(
        audit2.filter { case (t, id, _) => t == "hard-recovery" && id == "n-u" }.size,
        1,
  ...
  test(
    "R5-方案4 dedup: holdTerminalNotify keeps the L3 intermediate Cancelled state out of the 30s redeliver sweep (0 candidates); releaseTerminalNotify hands the marker back so the next real terminal state notifies exactly once"
  ) {
    for
    ...
    yield
      ...
      assert(
        normalHeld,
        "the guard must not disturb the normal L3 intermediate path (still-Cancelled node still gets the hold)"
      )
  }

  test(
    "R5b: settleFailedHardResume no-op legs — node not cancelled, and unknown session (both silent, zero side effects)"
  ) {
    ...
    for
      ...
      _ <- seed(
        ...
        NodeDef(
          ...
          agent = "general",
          status = NodeLifecycle.Running,
          sessionRef = Some("node-sess-live"),
          out = List(OutEdge("n-down")),
          createdAt = now - 60_000L
        )
      )
      _ <- seed(
    ...
    yield
      ...
      assert(
        ghostLines.forall(_._2 == ""),
        s"the orphan audit line must carry an empty nodeId, got ${ghostLines.map(_._2)}"
      )
      assert(
        ghostLines.head._3.contains("no node owns this session"),
  ...
  test(
    "R3-dedup: a barrier alerted immediately is NOT re-emitted by the periodic sweep (single alert per stall episode); an un-alerted stall still emits mount-stalled (not blanket-silenced)"
  ) {
  ...
  test(
    "R3-failed-side + R4-red-line: dead-session convergence to failed raises the immediate barrier alert but performs ZERO detach / ZERO succession marking"
  ) {
    ...
    yield
      ...
      assertEquals(
        bEv.size,
        1,
        s"R3: the failed-side terminal write point must raise the immediate alert, got ${audit.map((t, id, _) => (t, id))}"
      )
      assert(bEv.head._3.contains("cause=failed"), s"alert must carry cause=failed: ${bEv.head._3}")
      ...
      assert(
        fired.head.contains("reason=failed") && fired.head.contains("触发 reactivate 重跑"),
  ...
  test(
    "R4-unlock: NodeEdit with an actual change clears the 待承接 marker (barrier becomes triggerable again); a no-op edit does not"
  ) {
    ...
    for
      ...
      _ <- nebflow.core.tools.NodeEditTool
        .call(
          Json.obj("project" -> Json.fromString("cd-unlock"), "nodename" -> Json.fromString("SUCC")).asObject.get,
          ctx
        )
      ...
      _ <- nebflow.core.tools.NodeEditTool
        .call(
          Json
            .obj(
              "project" -> Json.fromString("cd-unlock"),
              "nodename" -> Json.fromString("SUCC"),
              "task" -> Json.fromString("t-succ-v2")
            )
            .asObject
            .get,
          ctx
