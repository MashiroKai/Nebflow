...
class FailedNotifySuppressionSpec extends FunSuite:
  ...
  private def fiveForms(): IO[Map[String, String]] =
    ...
    val io = for
      ...
      _ <- store.mutate(s => s.copy(nodes = s.nodes ++ Map(
        "n-u-engine" -> NodeDef(id = "n-u-engine", name = "UE", agent = "general",
          status = NodeLifecycle.Cancelled,
          result = Some("cancelled[source=engine]: reason=dead-session reap: status=running but no live execution fiber"),
          out = List(OutEdge.nebula), createdAt = now - 9000L, completedAt = Some(now - 8000L)),
        "n-u-unknown" -> NodeDef(id = "n-u-unknown", name = "UX", agent = "general",
          status = NodeLifecycle.Cancelled,
          result = Some("cancelled by NodeCancel (legacy data, no source prefix)"),
          out = List(OutEdge.nebula), createdAt = now - 9000L, completedAt = Some(now - 8000L)),
        "n-u-user" -> NodeDef(id = "n-u-user", name = "UU", agent = "general",
          status = NodeLifecycle.Cancelled,
          result = Some("cancelled[source=user]: reason=cancelled from panel"),
          out = List(OutEdge.nebula), createdAt = now - 9000L, completedAt = Some(now - 8000L)),
        "n-u-done" -> NodeDef(id = "n-u-done", name = "UD", agent = "general",
          status = NodeLifecycle.Completed, result = Some("done"),
          out = List(OutEdge.nebula), createdAt = now - 9000L, completedAt = Some(now - 8000L))))).void
      forms = List(
        "no-retry" -> NodeDef(id = "n-f1", name = "F1", agent = "general",
          status = NodeLifecycle.Failed, result = Some("boom: scripted failure"),
          gen = 0, createdAt = now - 1000L),
        "capped" -> NodeDef(id = "n-f2", name = "F2", agent = "general",
          status = NodeLifecycle.Failed, result = Some("boom: scripted failure"),
          retry = Some(RetryPolicy("n-u-done", 1)), gen = 1, createdAt = now - 1000L),
        "engine-cancel" -> NodeDef(id = "n-f3", name = "F3", agent = "general",
          status = NodeLifecycle.Failed, result = Some("boom: scripted failure"),
          retry = Some(RetryPolicy("n-u-engine", 3)), gen = 0, createdAt = now - 1000L),
        "unknown-source" -> NodeDef(id = "n-f4", name = "F4", agent = "general",
          status = NodeLifecycle.Failed, result = Some("boom: scripted failure"),
          retry = Some(RetryPolicy("n-u-unknown", 3)), gen = 0, createdAt = now - 1000L),
        "suppressed" -> NodeDef(id = "n-f5", name = "F5", agent = "general",
          status = NodeLifecycle.Failed, result = Some("boom: scripted failure"),
          retry = Some(RetryPolicy("n-u-user", 3)), gen = 0, createdAt = now - 1000L))
      _ <- store.mutate { s =>
        val waiters = forms.zipWithIndex.map { case ((_, f), i) =>
          s"n-w${i + 1}" -> NodeDef(id = s"n-w${i + 1}", name = s"W${i + 1}", agent = "general",
            status = NodeLifecycle.Pending, task = Some("waiting"), in = List(f.id),
            out = List(OutEdge.nebula), createdAt = now - 500L)
        }.toMap
  ...
  test("F2: the four NON-suppressed failed-text forms are byte-identical to the pre-change tree (sha256 lock: no retry / gen>=max RetryCap / engine-cancel upstream / unclassifiable source)") {
    val texts = fiveForms().unsafeRunSync()
    PreChangeSha.foreach { case (form, pre) =>
      val t = texts(form)
      assertEquals(sha256(t), pre,
        s"F2 zero-regression: form '$form' text must be byte-identical to the pre-change reading")
      assert(!t.contains(SuppressedKey),
        s"F2: form '$form' must NOT carry the suppressed-state marker (that is a suppression-only field)")
      assert(t.contains(ReactivateLead),
        s"F2: form '$form' must keep the normal first-choice recipe verbatim")
    }
    // 对照读数（同一夹具）：抑制态改前 sha = PreChangeSuppressedSha（**常态形态**文本，
    // 即被修的缺陷）；改后必须换形态（F1 臂另给正向/负向锚）
    assert(sha256(texts("suppressed")) != PreChangeSuppressedSha,
      "the suppressed fixture must no longer render the pre-change (normal-shape) text")
  }

  // ── F1：抑制态文本改劝（正向锚 + 双重负向锚）─────────────────────────

  test("F1: the suppressed failed text self-describes the suppression and drops the 'first choice = reactivate' advice — it neither copies the cancelled-only absolute sentence nor claims the normal recipe") {
    val texts = fiveForms().unsafeRunSync()
    ...
    assert(t.contains("W5(n-w5)") && t.contains("不会自动续跑"),
    ...
    try
      ...
      val io = for
        ...
        engine = new NodeEngine(
          store, system, resources, _ => IO.unit, ws.toString, rootSid, s"ftproj-$name",
  ...
  private def readAudit(ws: os.Path): IO[List[(String, String, String)]] =
    IO.blocking(os.read(ws / ".nebflow" / FlowMapEventLog.FileName))
      ...
      .map(_.flatMap(l => io.circe.parser.parse(l).toOption.map(j => (
  ...
  private def retryFixture(upstreamResult: String, store: FlowMapStore): IO[Unit] =
    ...
    for
      ...
      _ <- store.mutate(s => s.copy(nodes = s.nodes + (
        "n-b" -> NodeDef(id = "n-b", name = "B", agent = "test-agent", task = Some("FAIL-B work"),
          status = NodeLifecycle.Pending, out = List(OutEdge.nebula),
          retry = Some(RetryPolicy(upstream = "n-u", max = 3)), createdAt = now - 1000L))))
    yield ()

  test("F1/F3 (engine seam): a downstream whose retry.upstream was cancelled by the USER fails ⇒ the dispatcher receives the SUPPRESSED variant (no first-choice reactivation advice) while the retry audit + WARN facts are preserved verbatim") {
    withFixture("user", scriptedLlm("FAIL-B")) { (store, engine, triggered, ws) =>
      ...
      assert(notifyEvents.exists(_._3.contains("triggered: failed → dispatcher")),
      ...
      assert(t.contains("(n-u)") && t.contains(DispatchNotify.UserCancelLead),
        "F1: the injected text must name the user-cancelled upstream")
    }
  }

  test("F2 control (engine seam): an ENGINE-cancelled upstream keeps today's semantics AND today's text (reactivation recipe intact, no suppressed marker)") {
    withFixture("engine", scriptedLlm("FAIL-B")) { (store, engine, triggered, ws) =>
      ...
      assert(texts.forall(t => !t.contains(SuppressedKey)),
        s"F2: an engine-cancel upstream must NOT produce the suppressed variant — got: ${texts.map(_.take(200))}")
      println(s"[spec] F2 control evidence — retry audit: ${retryEvents.map(_._3)}; texts=${texts.size} (suppressed marker absent)")
    }
  }
