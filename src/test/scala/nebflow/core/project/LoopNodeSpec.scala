...
class LoopNodeSpec extends CatsEffectSuite:
  ...
  test("① worker PASS: single round — verify PASS → completed (worker output delivered)") {
    ...
    yield
      ...
      assert(
        payload.hcursor.downField("loop").focus.exists(_.isObject),
        s"buildNodeJson must carry a loop config object, got ${payload.hcursor.downField("loop").focus}"
      )
      assert(
        payload.hcursor.downField("loopRound").focus.exists(j => j.asNumber.exists(_.toInt.contains(1))),
  ...
  test("② FAIL→rework→PASS: round 1 FAIL, worker re-runs on constant rework input, round 2 PASS") {
    ...
    yield
      ...
      assert(
        !reworkTurn.get.contains(WorkerAnswer),
      ...
      assert(
        wFull.exists(_.contains(WorkerAnswer)),
      ...
      assert(
        v2.exists(t =>
          t.contains("== Output under review") && !t.contains("== Original task (acceptance baseline) ==")
        ),
  ...
  test("③ maxRounds reached: K=3, verify always FAIL → loop terminalizes as failed (round-level cap)") {
    ...
    yield
      ...
      assert(
        n.result.exists(_.contains(s"loop reached maxRounds=$K")),
  ...
  test("⑤ dual-session destruction: failed loop (K cap) leaves no node-* sessions in agentRegistry") {
    ...
    for
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield assertEquals(
      registry.keys.filter(_.startsWith("node-")),
      Set.empty[String],
  ...
  test(
    "⑦ destroy window: loop terminal registers BOTH sessions, clears the declaration slots and the sweep reclaims both"
  ) {
                ...
                reg.values.find(_.displayName.exists(_ == "l-win")).map(_.sessionId) match
                  case Some(sid) =>
                    ...
                    NodeReportRegistry.register(
                      (tempRoot / "ws-loop-registry").toString,
                      "loop-spec",
                      "l-win",
                      sid,
                      BlockedFeedback("pass", "residue before loop terminal", "")
                    )
                  case None => IO.unit
              }
        }
    ...
    yield
      ...
      assert(
        eventsScheduled.exists(_.contains("\"node-destroy-scheduled\"")),
      ...
      assertEquals(
        framesInWindow.count(j => j.hcursor.get[String]("type").contains("backgroundTaskUpdate")),
        0,
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "loop-pl",
          "plain",
          "description" -> Json.fromString("plain node"),
          "task" -> Json.fromString("plain-task"),
  ...
  test(
    "⑧ loop gate invariant: bare/single-leg out rejected on create + edit + mirror-append (create & edit paths), empty out exempt, non-loop untouched"
  ) {
    ...
    for
      ...
      rMirror <- nodeEdit(
        nodeInput(
          ...
          "in" -> Json.fromString("n-legacy-loop")
        ),
        ctx
      )
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "down-3",
          "description" -> Json.fromString("downstream"),
          "task" -> Json.fromString("downstream work")
        ),
        ctx
      )
    ...
    yield
      assert(
        rBare.isLeft && rBare.left.exists(_.contains("NODE_LOOP_GATE_INCOMPLETE")),
      ...
      assert(
        rOneLeg.isLeft && rOneLeg.left.exists(_.contains("NODE_LOOP_GATE_INCOMPLETE")),
        s"(b) loop + single-leg explicit gate set must be rejected, got: $rOneLeg"
      )
      assert(
        sRej.nodes.values.forall(n => n.name != "l-bare" && n.name != "l-oneleg"),
      ...
      assert(
        rEdit.isLeft && rEdit.left.exists(_.contains("NODE_LOOP_GATE_INCOMPLETE")),
      ...
      assert(
        rMirror.isLeft && rMirror.left.exists(_.contains("NODE_LOOP_GATE_INCOMPLETE")),
      ...
      assert(
        rEditMirror.isLeft && rEditMirror.left.exists(_.contains("NODE_LOOP_GATE_INCOMPLETE")),
        s"(h) EDIT-side mirror append onto a single-leg loop upstream must be rejected, got: $rEditMirror"
      )
      assertEquals(
        down3After.map(_.in),
        Some(List.empty[String]),
        "(h) zero residue: the downstream in ledger must stay empty"
      )
      assertEquals(
