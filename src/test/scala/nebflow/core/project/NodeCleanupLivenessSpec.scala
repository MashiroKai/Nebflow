...
class NodeCleanupLivenessSpec extends CatsEffectSuite:
  ...
  test(
    "c1+c3 dead running: liveness=false in NodeList; NodeCancel reaps to cancelled (real terminal, not fake success); idempotent after"
  ) {
    ...
    yield
      assertEquals(
        payDead.hcursor.get[Boolean]("liveness").toOption,
        Some(false),
      ...
      assert(
        payAfter.hcursor.get[Boolean]("liveness").toOption.isEmpty,
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          ...
          "task" -> Json.fromString("slow-live"),
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield
      assert(payWiring.hcursor.get[Boolean]("liveness").toOption.isEmpty, "wiring node must NOT carry liveness key")
      assertEquals(engineAlive, true, "live running must be in the engine in-flight map")
      assertEquals(
        payLive.hcursor.get[Boolean]("liveness").toOption,
        Some(true),
        s"live running must report liveness=true, got ${payLive.noSpaces.take(200)}"
      )
      assert(payDone.hcursor.get[Boolean]("liveness").toOption.isEmpty, "terminal node must NOT carry liveness key")
    end for
  }
  ...
  test(
    "kill-protection: abandon on LIVE running refused (node untouched); NodeCancel on live uses the normal signal chain"
  ) {
    ...
    yield
      ...
      assert(
        rAbandon.left.exists(m => m.contains("running") && m.contains("NodeCancel")),
      ...
      assert(
        rCancel.isRight && rCancel.exists(_.contains("cancel signal sent")),
      ...
      assert(
        !audit.exists((t, id) => t == "reaped" && id == liveId),
  ...
  test(
    "c2 abandon dead running: accepted, cancelled, no TTL + audit (2026-09-07 ruling); re-abandon on terminal is idempotent"
  ) {
