...
class FlowMapChainSpec extends CatsEffectSuite:
  ...
  test("T② 跨区并链: combinedNodes 合并集上归档上游与活跃下游同链（D8）；chainIdOf ≥2 才带") {
    ...
    for
      ...
      _ <- store.mutate(s =>
        s.copy(nodes = s.nodes + ("n-b" -> def0("n-b", t0 + 5000, NodeLifecycle.Wiring, in = List("n-arch-a"))))
      )
    ...
    yield
      ...
      assertEquals(
        chains.find(_.id == "chain-n-arch-a").map(_.memberIds),
        Some(List("n-arch-a", "n-b")),
  ...
  test("T③ 悬空名 out 边: 不连边不产生链归属——两端各自成链（D9）") {
    ...
    assertEquals(chainIds(List(x, y)).sorted, List("chain-x", "chain-y"), "dangling name edge must NOT link x to y")
    // 名字形态但目标已删除（同名节点不在集内）= 同款悬空
    val x2 = def0("x2", t0, out = List(OutEdge("name-y")))
    val y2 = def0("y2", t0 + 1000) // name-y2，不叫 name-y
    assertEquals(
      chainIds(List(x2, y2)).sorted,
      List("chain-x2", "chain-y2"),
  ...
  test("T⑩ 跨区续做分量: 归档上游 + 活跃下游完成 → 归档进同链批（id 合并，旧批成员不孤儿化）") {
    ...
    for
      ...
      _ <- store.mutate(s =>
        s.copy(nodes =
          s.nodes +
            ("t10-b" -> def0("t10-b", t0 + 5000, in = List("t10-a"))
              .copy(result = Some("downstream"), completedAt = Some(t0 + 5100)))
        )
      )
  ...
  test("T⑪ 载荷契约: chainId 条件键（≥2 带/孤立不带）+ chains 旁挂（title 三级推导，前端零派生）") {
    ...
    for
      ...
      _ <- store.mutate(s =>
        s.copy(nodes =
          s.nodes ++ Map(
            "p-a" -> def0("p-a", t0, out = List(OutEdge("p-b")))
              .copy(description = Some("修复登录回归"), completedAt = Some(t0 + 1)),
    ...
    yield
      ...
      def nodeJson(id: String): Json =
        nodes.find(_.hcursor.get[String]("id").toOption.contains(id)).getOrElse(fail(s"$id missing"))
      ...
      assertEquals(
        chains.map(_.hcursor.get[String]("id").toOption.getOrElse("")).sorted,
        List("chain-p-a", "chain-p-c", "chain-p-e")
      )
      // title 三级推导（后端下发，前端零派生）
      def titleOf(cid: String): String =
        chains
          .find(_.hcursor.get[String]("id").toOption.contains(cid))
          .flatMap(_.hcursor.get[String]("title").toOption)
          .getOrElse(fail(s"chain $cid missing"))
      ...
      val chainA =
        chains.find(_.hcursor.get[String]("id").toOption.contains("chain-p-a")).getOrElse(fail("chain-p-a missing"))
