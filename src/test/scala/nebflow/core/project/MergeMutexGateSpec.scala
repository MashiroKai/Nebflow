...
class MergeMutexGateSpec extends CatsEffectSuite:
  ...
  private def waitStatus(rt: ProjectRuntime, id: String, statuses: Set[String],
      timeout: FiniteDuration = 60.seconds): IO[Unit] =
  ...
  private def mergeEntry(id: String, name: String, createdAt: Long): NodeDef =
    NodeDef(id = id, name = name, agent = "general", merge = true, task = Some(s"landing task for $name"),
      status = NodeLifecycle.Pending, out = List(OutEdge.nebula), createdAt = createdAt)

  /** task 上游（completed；`completedAt` 决定下游 merge 的 readyAt = 到达时刻）。 */
  private def doneUpstream(id: String, name: String, out: List[OutEdge], completedAt: Long,
      createdAt: Long): NodeDef =
    NodeDef(id = id, name = name, agent = "general", task = Some(s"$name task"),
      status = NodeLifecycle.Completed, result = Some(s"$name artifact"), out = out,
      createdAt = createdAt, completedAt = Some(completedAt))
  ...
  private def verifierUp(id: String, name: String, verdict: Option[String], now: Long): NodeDef =
    NodeDef(id = id, name = name, agent = "general", task = Some(s"$name verify task"),
      ...
      result = Some(s"verdict report for $name"), out = Nil,
  ...
  private def mkGitRepo(dir: os.Path): Unit =
    ...
    os.proc("git", "-C", dir.toString, "commit", "--allow-empty", "-m", "init").call(check = true, stdout = os.Pipe, stderr = os.Pipe)
  ...
  test("KEY (design §3.1): key = realpath(git-common-dir) — a worktree shares ONE key with its main repo, two repos differ, and a workspace with no enclosing repo falls back to its own canonical path") {
    ...
    if !gitAvailable then IO.unit
      ...
      yield
        ...
        assertEquals(kOutside, MergeMutexPolicy.canonical(outside.toString),
          "a workspace with NO enclosing repo falls back to its own canonical path (no false mutex, no false alarm)")
        assert(kOutside != kMain && kOutside != kOther && kOutside != kPlain,
          "the fallback key must not collide with any repo key")
  }

  // ── ① 互斥 + ② FIFO + ③ 释放（完成腿）────────────────────────────

  test("Q1 mutex + FIFO + release(completed): with two same-key merge nodes ready at once EXACTLY ONE runs (the earlier arrival by rank); the other stays pending with a merge-queue hold record, and starts only after the holder's terminal write") {
    ...
    yield
      ...
      assertEquals(b2.status, NodeLifecycle.Completed, "after the holder completed the next one must start (bounded release)")
      assert(b2.startedAt.isDefined, "the released merge must have really run")
      assert(b2.startedAt.get >= a2.completedAt.getOrElse(Long.MaxValue),
        s"release must follow the holder's terminal write (startedAt=${b2.startedAt} vs holder.completedAt=${a2.completedAt})")
  }

  test("Q2 FIFO order: the queue is ranked by ARRIVAL (readyAt) — a node created earlier but arriving later must NOT jump the queue (fixture inverts createdAt vs readyAt, so a createdAt-based implementation goes red)") {
    ...
    for
      ...
      _ <- seed(rt,
        ...
        NodeDef(id = "n-m1", name = "merge-1", agent = "general", merge = true, task = Some("landing m1"),
          status = NodeLifecycle.Pending, in = List("u1"), out = List(OutEdge.nebula), createdAt = now - 90_000L),
        NodeDef(id = "n-m2", name = "merge-2", agent = "general", merge = true, task = Some("landing m2"),
          status = NodeLifecycle.Pending, in = List("u2"), out = List(OutEdge.nebula), createdAt = now - 80_000L),
        NodeDef(id = "n-m3", name = "merge-3", agent = "general", merge = true, task = Some("landing m3"),
          status = NodeLifecycle.Pending, in = List("u3"), out = List(OutEdge.nebula), createdAt = now - 70_000L))
      _ <- rt.engine.settleRunnableSweep()
    ...
    yield
      ...
      assert(s3.startedAt.get < t2.startedAt.get && t2.startedAt.get < f1.startedAt.get,
        s"FIFO start order must be m3 < m2 < m1 by startedAt, got ${List(("n-m3", s3.startedAt), ("n-m2", t2.startedAt), ("n-m1", f1.startedAt))}")
      assert(s3.createdAt > t2.createdAt && t2.createdAt > f1.createdAt,
        "fixture sanity: createdAt order is the INVERSE of the observed start order (so createdAt-based ordering would be red here)")
  }

  // ── ③ 释放：取消腿 / 失败腿 ──────────────────────────────────────

  test("Q3 release(cancelled): a merge node cancelled while holding the critical section releases it — the queued node starts right after (bounded)") {
    ...
    yield
      ...
      assertEquals(b.status, NodeLifecycle.Running, "the cancel of the holder must release the queue (the next one really started)")
      assertEquals(bf.status, NodeLifecycle.Completed, "the released merge ran to completion")
      assert(bf.startedAt.get >= a.completedAt.getOrElse(Long.MaxValue),
        s"release must follow the cancel terminal write (startedAt=${bf.startedAt} vs completedAt=${a.completedAt})")
  }

  test("Q3 release(failed): a merge node that converges to failed while holding the critical section releases it — the queued node starts right after (bounded)") {
    ...
    for
      ...
      _ <- seed(rt,
        NodeDef(id = "n-a", name = "merge-a", agent = "general", merge = true, task = Some("landing a"),
          status = NodeLifecycle.Running, out = List(OutEdge.nebula),
          startedAt = Some(now - 3_600_000L), createdAt = now - 3000L),
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield
      assertEquals(held.status, NodeLifecycle.Pending, "precondition: the later arrival was queued behind the running holder")
      ...
      assert(a.completedAt.isDefined, "the failed terminal write must carry completedAt (release fact is derived from status)")
      assert(b.startedAt.get >= a.completedAt.getOrElse(Long.MaxValue),
        s"release must follow the failed terminal write (startedAt=${b.startedAt} vs completedAt=${a.completedAt})")
  }

  // ── ④ 跨键并行（SEM-5：异键不互斥）───────────────────────────────

  test("Q4 cross-key parallel: two projects on DIFFERENT git dirs run their merge nodes AT THE SAME TIME (no cross-key mutex, zero queue records)") {
    ...
    if !gitAvailable then IO.unit
      ...
      yield
        ...
        assertEquals(y.status, NodeLifecycle.Running, "cross-key: project B's merge must run AT THE SAME TIME (SEM-5: different keys MUST be allowed to run in parallel)")
  ...
  test("Q5 O-1 alarm: two projects resolving to the SAME git dir (main repo + its worktree) raise the visible merge-queue alarm (kind=same-git-dir-multi-project) — and the KNOWN GAP is read out: no cross-project claim is implemented, so both merge nodes run") {
    ...
    if !gitAvailable then IO.unit
      ...
      for
        ...
        _ <- seed(rtA,
          NodeDef(id = "a-merge", name = "merge-a", agent = "general", merge = true, task = Some("landing a"),
            status = NodeLifecycle.Running, out = List(OutEdge.nebula),
            startedAt = Some(now - 60_000L), createdAt = now - 3000L))
        _ <- seed(rtB, mergeEntry("b-merge", "merge-b", now - 1000L))
        _ <- rtB.engine.settleRunnableSweep()
        _ <- waitStatus(rtB, "b-merge", Set(NodeLifecycle.Running))
        alarm <- readAudit(wt)
      ...
      yield
        ...
        val alarms = alarm.filter((t, _, s) => t == FlowMapEventLog.MergeQueueType && s.contains("kind=same-git-dir-multi-project"))
        assertEquals(alarms.size, 1, s"the same-git-dir multi-project situation MUST raise the visible alarm, got $alarms")
        val (_, alarmNode, summary) = alarms.head
        assertEquals(alarmNode, "mm-o1-b", "the alarm's nodeId field carries the reporting project name (dispatcher-wake precedent)")
        ...
        assertEquals(aRunning.status, NodeLifecycle.Running, "precondition: project A's merge node is in the critical section")
        assertEquals(bRunning.status, NodeLifecycle.Running,
  ...
  test("Q6 O-2 deps: a merge node whose VERIFIER is mounted via `deps` (not `in`) is held while lastVerdict != pass, and starts once the verdict is pass (G-1 one-liner, zero legacy impact)") {
    ...
    for
      ...
      _ <- seed(rt,
        ...
        NodeDef(id = "n-merge", name = "merge-deps", agent = "general", merge = true,
          task = Some("landing via deps"), status = NodeLifecycle.Pending,
          deps = List("n-ver"), out = List(OutEdge.nebula), createdAt = now - 100_000L))
      _ <- rt.engine.settleRunnableSweep()
      _ <- settleWindow
      ...
      _ <- system.stopAll.handleErrorWith(_ => IO.unit)
    yield
      assertEquals(held.status, NodeLifecycle.Pending,
      ...
      assertEquals(held2.status, NodeLifecycle.Pending, "the hold is idempotent across sweeps (current value, not a one-shot latch)")
  ...
  test("Q7 merge-only: two NON-merge nodes in the same project run in parallel — the mutex gate leaves every non-merge node untouched") {
    ...
    for
      ...
      _ <- seed(rt,
        NodeDef(id = "n-p1", name = "plain-1", agent = "general", task = Some("work 1"),
          status = NodeLifecycle.Pending, out = List(OutEdge.nebula), createdAt = now - 3000L),
        NodeDef(id = "n-p2", name = "plain-2", agent = "general", task = Some("work 2"),
          status = NodeLifecycle.Pending, out = List(OutEdge.nebula), createdAt = now - 1000L))
      _ <- rt.engine.settleRunnableSweep()
    ...
    yield
      ...
      assertEquals(queueEvents, Nil, s"the gate is merge-only — no merge-queue record may be produced for non-merge nodes, got $queueEvents")
  }
