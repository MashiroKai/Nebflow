...
class OutNullableDeliverySpec extends CatsEffectSuite:
  ...
  test("①D bare \"Nebula\" exit marker: completion delivers NOTHING to root, yet the ledger is stamped") {
    ...
    for
      ...
      r <- nodeEdit(
        nodeInput(
          "outnull-exit",
          "exit-node",
          "description" -> Json.fromString("exit marker node"),
          "task" -> Json.fromString("finish through the marker"),
    ...
    yield
      ...
      assert(
        node.flatMap(_.nebulaDeliveredAt).isDefined,
    ...
    for
      ...
      r <- nodeEdit(
        nodeInput(
          "outnull-notify",
          "notify-node",
          "description" -> Json.fromString("notify node"),
          "task" -> Json.fromString("report to root"),
  ...
  test(
    "③D empty out create is legal: completion delivers nothing, no root notify, result retained (dangling hint in receipt)"
  ) {
    ...
    for
      ...
      r <- nodeEdit(
        nodeInput(
          "outnull-empty",
          "dangling-node",
          "description" -> Json.fromString("dangling node"),
          "task" -> Json.fromString("produce a retained result")
        ),
        ctx
      )
  ...
  test("④D dangling completed node wired by a downstream 'in:' declaration (path B) → retained result delivered") {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "outnull-pathb",
          "A-悬空",
          "description" -> Json.fromString("dangling producer"),
          "task" -> Json.fromString("produce")
        ),
        ctx
      )
      ...
      rB <- nodeEdit(
        nodeInput(
          "outnull-pathb",
          "B-下游",
          "description" -> Json.fromString("consumer"),
    ...
    yield
      ...
      assert(
        ...
        s"B must receive A's retained result (path B), got ${b.map(_.deliveredTo)}"
      )
    end for
  }
  ...
  test("⑦D NodePayload.wiringGap: empty out ⇒ pending/retained key; nodes with an out edge carry no key") {
    ...
    assertEquals(
      wiringEmpty.hcursor.get[String]("wiringGap").toOption,
      Some("pending"),
      "empty out + wiring ⇒ wiringGap=pending"
    )
    assertEquals(
      completedEmpty.hcursor.get[String]("wiringGap").toOption,
      Some("retained"),
  ...
  test(
    "⑧D/A10·N4 terminal-node rewire delivers ONLY the newly-wired target: kept downstream + kept Nebula edge get no second delivery"
  ) {
    ...
    for
      ...
      _ <- nodeEdit(
        nodeInput(
          "outnull-a10",
          "A-源",
          "description" -> Json.fromString("retained producer"),
          "task" -> Json.fromString("produce"),
          "out" -> Json.fromString("(pass,failed)Nebula")
        ),
        ctx
      ...
      rB <- nodeEdit(
        nodeInput(
          "outnull-a10",
          "B-旧目标",
          "description" -> Json.fromString("kept downstream"),
          "task" -> Json.fromString("consume"),
          "in" -> Json.fromString(aId)
        ),
        ctx
      ...
      rC <- nodeEdit(
        nodeInput(
          "outnull-a10",
          "C-新目标",
          "description" -> Json.fromString("newly wired target"),
          "task" -> Json.fromString("assemble the newly wired target")
        ),
        ctx
      )
      ...
      r <- nodeEdit(
        nodeInput("outnull-a10", "A-源", "out" -> Json.fromString(s"(pass,failed)Nebula,(pass)$bId,(pass)$cId")),
        ctx
      )
    ...
    yield
      ...
      assert(
        aBefore.flatMap(_.nebulaDeliveredAt).isEmpty,
        "precondition: A 的完成期投递因 root 缺席被跳过（零记账）⇒ M1 持久锚未置位 = A10 的承重窗口"
      )
      // 正向半：新接线目标收到滞留结果（接线即投递）
      assert(
        cAfter.exists(_.deliveredTo.contains(aId)),
      ...
      assert(
        aAfter.flatMap(_.nebulaDeliveredAt).isEmpty,
