...
class ChainCancelSpec extends CatsEffectSuite:
  ...
  test("chainMembersOf: resolves the weakly-connected component of the merged set, id = chain-<earliest createdAt member>, members ascending by createdAt") {
    withRig("resolve") { rig =>
      for
        _ <- linearChain(rig)
  ...
  test("C1+C3+C4+C5+C6+C7: chain cancel of A→B→C writes 3 cancelled + 1 chain-cancelled audit, ONE injection, budget +1 with window/cooldown untouched, all members notifySentAt set, pruned referrers accounted, partition exact") {
    withRig("c-main") { rig =>
      ...
      for
        ...
        markers <- List("n-a", "n-b", "n-c").traverse(id => rig.rt.store.getNode(id).map(_.exists(_.notifySentAt.isDefined)))
        ...
        assertEquals(chainAfter.map(_.info.memberIds), Some(List("n-a")),
  ...
  test("C2: injection count is 1 for N=1 and for N=20 alike (the N>=5 storm arithmetic is structurally gone)") {
    withRig("c-count") { rig =>
      ...
      for
        ...
        _ <- seed(rig, List(n("n-solo", NodeLifecycle.Pending, 100L),
          n("n-solo2", NodeLifecycle.Pending, 200L, in = List("n-solo"))))
  ...
  test("M10: cancelled set explicitly enumerates running/pending/wiring/blocked/interrupted and never uses the Terminal predicate (which contains blocked)") {
    withRig("m10") { rig =>
      for
        _ <- seed(rig, List(
      ...
      yield
        assertEquals(report.cancelled.map(_.nodeId).sorted,
          List("n-blocked", "n-intr", "n-pend", "n-run", "n-wire"))
        ...
        assert(report.preserved.forall(_.why == "terminal"),
          s"all three preserved members are terminal: ${report.preserved}")
        assertEquals(report.skipped, Nil)
        // 零触碰：终态节点状态与结果逐字保留
        assertEquals(sts, List("cancelled", "cancelled", "cancelled", "cancelled", "cancelled",
  ...
  test("M12: single-member chain and unknown chainId return actionable error codes and NEVER degrade into a single-node cancel") {
    withRig("m12") { rig =>
      for
