...
/**
 * blocked 结构化信号（report_blocked 工具）集成回归（20260909 spec §9.2/§9.3，
 * 真实引擎路径驱动，NodeBlockedReentrySpec 同款基建）：
...
class NodeBlockedToolSignalSpec extends CatsEffectSuite:
  ...
  /** 工具申报状态机 LLM：首 turn 发 report_blocked 工具调用，见到工具结果后输出
    * 无锚定收尾文本——证明 blocked 由工具通道驱动而非文本形态。
          ...
          case Right(blocks) =>
            blocks.exists {
              case ContentBlock.ToolResult(_, content, _) =>
                content.contains("[OK] blocked declaration recorded")
              case _ => false
            }
          case Left(t) => t.contains("[OK] blocked declaration recorded")
      }
    def handle: LlmHandle[IO] = new LlmHandle[IO]:
      def send(req: LlmRequest): IO[LlmResponse] = IO.raiseError(new RuntimeException("send not expected"))
      def sendStream(
  ...
  test("tool declaration: report_blocked call drives blocked terminal (feedback/count/ttl/result head+full text), no text anchor needed, downstream NOT settled") {
  ...
  test("leftover sweep: cancelled session's declaration is removed by cleanup (no cross-session leak)") {
    ...
    for
      _ <- BlockedSignalRegistry.register(sid, BlockedFeedback("other", "残留申报", ""))
      _ <- BlockedSignalRegistry.remove(sid)
      drained <- BlockedSignalRegistry.drain(sid)
      _ <- BlockedSignalRegistry.remove(sid) // 幂等二次
      again <- BlockedSignalRegistry.drain(sid)
    yield
