  ...
  test("spawn 首条消息：nebflow 规模 Flow Map 下不含快照字节（result:/task:/```json/v:1 零命中）且 <30KB") {
    ...
    for
      ...
      _ <- waitUntil(20.seconds)(
        resources.agentRegistry.get.map(!_.keySet.exists(_.startsWith(ProjectActor.DispatcherSessionPrefix)))
      )
      ins <- llm.inputs.get
    ...
    yield
      ...
      assert(
        ...
        s"first message must NOT contain hydrated result JSON field, len=${first.length}"
      )
      assert(
        !first.contains("\"task\":"),
        s"first message must NOT contain hydrated task JSON field, len=${first.length}"
      )
      ...
      assert(
        !first.contains("\"v\":1") && !first.contains("\"updatedAt\":"),
        "first message must NOT carry FlowMapState snapshot signature keys"
      )
      ...
      assert(
        first.contains("Read the Flow Map first (NodeList"),
        "first message must keep the NodeList-first workflow guidance"
      )
      ...
      println(
        s"[ctx-econ 裁定①] nodes=$nodeCount old-style snapshot embed≈$oldStyleBytes bytes → actual first message=$bytes bytes (reduction $pct%)"
      )
      assert(
        bytes < oldStyleBytes / 10,
        s"first message must be <10% of old-style snapshot embed ($bytes vs $oldStyleBytes)"
      )
      val payloadBytes = payload.noSpaces.getBytes("UTF-8").length
      println(
        s"[ctx-econ 裁定②③⑤] NodeList default payload on $nodeCount-node fixture = $payloadBytes bytes (audit real-data baseline ≈86KB)"
      )
    end for
  }
