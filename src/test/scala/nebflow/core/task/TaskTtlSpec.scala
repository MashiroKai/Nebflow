...
class TaskTtlSpec extends CatsEffectSuite:
  ...
  private def seedTask(sid: String, id: String, status: String, createdAt: String, completedAt: String): Unit =
    ...
    os.makeDir.all(dir)
    val json =
      s"""{"id":"$id","subject":"t$id","description":"d","status":"$status","createdAt":"$createdAt","updatedAt":"$createdAt","completedAt":"$completedAt","events":[]}"""
  ...
  test("TTL-1: completed tasks expire after 6h, not before"):
    ...
    val empty = ""
    val freshTask =
      Task(id = "1", subject = "fresh", description = "d", status = TaskStatus.Completed, completedAt = Some(fresh))
    val staleTask =
      Task(id = "2", subject = "stale", description = "d", status = TaskStatus.Failed, completedAt = Some(stale))
  ...
  test("TTL-2: active tasks expire after 2d, not before"):
    ...
    val freshTask =
      Task(id = "1", subject = "f", description = "d", status = TaskStatus.InProgress, createdAt = Some(fresh))
    val staleTask =
      Task(id = "2", subject = "s", description = "d", status = TaskStatus.Pending, createdAt = Some(stale))
  ...
  test("TTL-3: purgeAllExpired sweeps disk data after restart (no in-memory state)"):
    ...
    os.write(
      teamDir / "1.json",
      s"""{"id":"1","subject":"team","description":"d","status":"completed","createdAt":"$oldCompleted","updatedAt":"$oldCompleted","completedAt":"$oldCompleted","events":[]}"""
    )
