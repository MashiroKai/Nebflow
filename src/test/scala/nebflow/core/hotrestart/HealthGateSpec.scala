...
class HealthGateSpec extends munit.CatsEffectSuite:
  ...
  private def mkIntent(probePort: Option[Int], phase: String, failure: Option[String] = None, spawnCmd: String = ""): HotRestartIntent =
    HotRestartIntent(
  ...
  private def atDoor(
    ...
    alive: Boolean = true,
    connectProbe: Option[Int => IO[Boolean]] = Some(p => IO.blocking(nebflow.cli.SingleInstanceGuard.connectProbeAccepted(p)))
  ): IO[HealthReport] =
    HealthCheck.atDoor(
  ...
  test("tier 3+4 green (real socket/HTTP/payload): probe endpoint serves the health payload; version matches") {
    for
      ...
      _ = assertEquals(ProbeEndpoint.Path, "/api/health", "single health path for both the real endpoint and the door probe")
  ...
  test("tier 3 red: announced probe port with nothing listening ⇒ port-serving fails after its own deadline") {
    for
      ...
      _ = assertEquals(report.tierWire(HealthTier.VersionMatch), Some("unverified"), "later tiers are skipped once one fails")
      _ = assert(!report.isHealthy)
    yield ()
  }

  test("legacy successor (no probePort announced) ⇒ tiers 3/4 unverified, and the gate stays healthy (declared, not silent)") {
    for
      ...
      _ = assert(report.isHealthy, "unverified is a third state: it never pretends to be a pass, and never blocks the existing handover path")
  ...
  test("expected version: the install pointer wins; otherwise the package path in the successor's spawn command") {
    ...
    IO {
      assertEquals(HealthCheck.expectedVersion(Some("2026.9.19"), Some("java -jar /x/nebflow-assembly-2026.1.1.jar start")), Some("2026.9.19"))
      assertEquals(HealthCheck.expectedVersion(None, Some("java -jar /x/nebflow-assembly-2026.1.1.jar start")), Some("2026.1.1"))
      assertEquals(HealthCheck.expectedVersion(Some("  "), Some("-jar /x/nebflow-assembly-1.4.1-beta.56.jar")), Some("1.4.1-beta.56"))
  ...
  private def mkInstallDir(prefix: String, current: String, previous: Option[String], versions: List[String]): IO[os.Path] =
    for
      dir <- tmpDir(prefix)
      _ <- IO.blocking {
        versions.foreach { v =>
  ...
  test("install pointer: read / flip / retained-package lookup (and the missing-pointer branches)") {
    for
      ...
      _ = assertEquals(after.versions, List("2026.9.19", "2026.9.20"), "both versions stay on disk (retention keeps two)")
      // 缺指针 / 缺上一版：明确 Left，不猜
  ...
  test("rollback: pointer flips to the previous version and the relaunch swaps ONLY the package path") {
    for
      ...
      _ = assert(seen.exists { case (jar, i) =>
        i.spawnCmd == HotRestart.buildCommand(
          i.form,
          nebflow.core.RestartHelper.resolveJavaBin(),
          Some(jar),
          SuccessorGate.intentPath(dataRoot).toString,
          dataRoot.toString,
          8098).fold(msg => msg, _.mkString(" "))
      ...
      _ = assertEquals(intentRead.map(_.phase), Some("spawned"), "the relaunch reuses the successor protocol (intent write-ahead)")
  ...
  test("rollback red control: with no previous version on disk the pointers stay untouched (no silent downgrade)") {
    for
      ...
      layout <- InstallPointer.layout(dir)
      _ = assertEquals(layout.current, Some("2026.9.20"), "red: nothing changed ⇒ a broken new version would be picked again")
      _ = assert(!os.exists(SuccessorGate.restartDir(dataRoot) / "rollback.json"))
    yield ()
  }
