...
class PluginApprovalSpec extends CatsEffectSuite:
  ...
  private def mkSkillPlugin(
    name: String,
    manifestExtra: String = "",
    envKeys: List[String] = Nil,
    ...
    os.write.over(d / "plugin.json", s"""{"$$schema":"${PluginRegistry.CanonicalSchema}","name":"$name"$extra1}""")
    os.write.over(
      d / "mcp.json",
  ...
  test("§B.3 红标: 无 author / 无 version → flags 命中；凭据类 env 键名、curl|sh 类 command → flags 命中") {
    ...
    assert(
      joined.contains("API_KEY") && joined.contains("credential"),
      s"credential env key must be flagged, got: $joined"
    )
  ...
  test("§B.3 变更摘要: 首审 new-install → approve 后 unchanged → 改文件 changed（added/modified 逐文件）") {
    ...
    val before = PluginRegistry
      .approvalManifest(manifestOf("diffed"))
      .hcursor
      .downField("changeSummary")
      .as[Json]
      .getOrElse(fail("no changeSummary"))
    assertEquals(
      before.hcursor.downField("kind").as[String].getOrElse("?"),
      "new-install",
      "no approval record → new-install"
    )

    PluginRegistry.approve("diffed").unsafeRunSync()
    val after = PluginRegistry
      .approvalManifest(manifestOf("diffed"))
      .hcursor
      .downField("changeSummary")
      .as[Json]
      .getOrElse(fail("no changeSummary"))
    assertEquals(
      after.hcursor.downField("kind").as[String].getOrElse("?"),
      "unchanged",
  ...
  test("§B.3 变更摘要: 旧审批记录（无 files 快照）→ 降级 digest 级比对并注明") {
    ...
    os.write.over(
      tempRoot / "nebflow.json",
      Json
        .obj(
          "plugins" -> Json.obj(
            "trust" -> Json.obj(
              "legacy" -> Json
                .obj("sha256" -> p0.digest.asJson, "approvedAt" -> 1757000000L.asJson, "scope" -> "all".asJson)
            )
          )
        )
        .noSpaces
    ...
    val manifest = PluginRegistry
      .approvalManifest(manifestOf("legacy"))
      .hcursor
      .downField("changeSummary")
      .as[Json]
      .getOrElse(fail("no changeSummary"))
    ...
    assert(
      manifest.hcursor.downField("note").as[String].toOption.exists(_.contains("digest-level")),
      s"legacy record must degrade honestly, got: $manifest"
    )
    assertEquals(
      manifest.hcursor.downField("digestMatches").as[Boolean].getOrElse(true),
      true,
              ...
              case Left(err3) =>
                IO(assert(err3.contains("PLUGIN_BLOCKED"), s"got: $err3")) *>
                  PluginBlockPolicy.unblock("installed-plugin", "spec").flatMap { _ =>
                    PluginRegistry
                      .resolve("installed-plugin")
                      .map(r => assert(r.isRight, "unblocked plugin must resolve again"))
                  }
            }
          }
    }
  ...
  test("§B.3 add: 重名已存在 / 非 plugin 目录 / manifest name 非法 → 拒绝（不覆盖不落盘）") {
    ...
    yield
      assert(
        dup.isLeft && dup.swap.toOption.getOrElse("").contains("already exists"),
        s"duplicate install must be refused: $dup"
      )
      assert(
        bare.isLeft && bare.swap.toOption.getOrElse("").contains("plugin.json"),
        s"non-plugin dir must be refused: $bare"
      )
      assert(
        bad.isLeft && bad.swap.toOption.getOrElse("").contains("§5.5"),
