...
class ReadImageSpec extends CatsEffectSuite:
  ...
  test("Read on PNG file returns image description and extractImages provides Image block"):
    ...
    for result <- ReadTool.call(inputJson(pngPath), ctx)
    yield result match
      case Right(text) =>
  ...
  test("Read on JPG file returns image description"):
    ...
    for result <- ReadTool.call(inputJson(jpgPath), ctx)
    yield result match
      case Right(text) =>
        assert(text.startsWith("[image:"), s"Expected image description, got: $text")
...
<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100">
  <circle cx="50" cy="50" r="40" fill="red"/>
    ...
    for result <- ReadTool.call(inputJson(svgPath), ctx)
    yield result match
      case Right(text) =>
  ...
  test("Read on text file returns text content (not image block)"):
    ...
    for result <- ReadTool.call(inputJson(txtPath), ctx)
    yield result match
      case Right(text) =>
