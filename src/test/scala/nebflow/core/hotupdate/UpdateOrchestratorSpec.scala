...
class UpdateOrchestratorSpec extends CatsEffectSuite:
  ...
  test("① full phase chain: checking→preparing→freezing→updating→restarting→recovering→completed (install called once, restart delegated to the real orchestrator)") {
    for
  ...
  test("① abort branch (freeze): in-flight work past the wait limit → aborted, admission reopened, install never runs") {
    for
      shutdownD <- Deferred[IO, Unit]
      res <- mkResources(shutdownD)
      frames <- mkFrames
  ...
  test("① abort branch (install): install action fails → aborted, admission reopened, legacy failure frame emitted") {
    for
      ...
      outcomes <- legacy.get
      _ = assertEquals(outcomes, List("failed:Install script failed (exit code: 13)"),
        "既有完成帧的失败分支必须照旧发出")
      draining <- HotRestart.isDraining
      _ = assertEquals(draining, false, "abort must reopen the admission gate")
      intentExists <- IO(os.exists(intentFile))
  ...
  test("② idempotency: same key arriving again while in flight → AlreadyInFlight + current phase, install count stays 1") {
    for
      shutdownD <- Deferred[IO, Unit]
      res <- mkResources(shutdownD)
      frames <- mkFrames
  ...
  test("③ concurrency gate: different key while in flight → Busy(phase, source), never queued (no second execution, key never switches)") {
    for
      shutdownD <- Deferred[IO, Unit]
      res <- mkResources(shutdownD)
      frames <- mkFrames
      ...
      _ <- waitUntil("install action started") { counter.get.map(_ >= 1) }
      second <- orch.request(
        settingsReq(key = Some("run-B"), source = UpdateSource.DeviceList))
      _ = assertEquals(
        second,
  ...
  test("⑤ drain never forces: RejectIfBusy on in-flight work → Left, and no node status is flipped (no forced tier)") {
    ...
    for
      ...
      _ = assertEquals(after.nodes("n-run").status, NodeLifecycle.Running,
      ...
      _ = assertEquals(stillRunning.nodes("n-run").status, NodeLifecycle.Running,
  ...
  test("criterion 3: the update scenario default await-idle is 300s (independent from the 600s UI restart command default)") {
    for
  ...
  test("ruling 3②/③ (frozen literals): orchestrator reason enum and source enum literals match the frozen word tables") {
    for
      _ <- IO {
        ...
        assertEquals(
          ...
          List("settings", "device-list", "relay", "website", "cli"))
  ...
  test("ruling 3④ (idempotency window): after a successful terminal phase the key is held for the frozen window, then expires") {
    for
      shutdownD <- Deferred[IO, Unit]
      res <- mkResources(shutdownD)
      frames <- mkFrames
  ...
  test("criterion 4: unified progress frame schema per phase + message keys resolve in both locales (no hardcoded display text)") {
    for
      shutdownD <- Deferred[IO, Unit]
      res <- mkResources(shutdownD)
      frames <- mkFrames
  ...
  test("criterion 4 (negative): an unreachable release pointer is fail-soft — the chain still reaches the install phase") {
    for
      shutdownD <- Deferred[IO, Unit]
      res <- mkResources(shutdownD)
      frames <- mkFrames
