...
class DreamModeSpec extends FunSuite:
  ...
  // ---------------------------------------------------------------
  // 快照先行（dream-agent 批 2026-09-05）：hook 写面同样先过写前快照
  // ---------------------------------------------------------------

  test("updateMemory: 正常合并落盘前留有写前快照（备份 == 写前真身）"):
    val seed = "## Dream Extract\n\n### PATTERN\n\n- 既有模式\n"
    os.write.over(userFile, seed, createFolders = true)
    // 清掉前测可能留下的备份目录，保证快照可定位
    val root = home / "memory-backups"
    if os.exists(root) then os.remove.all(root)
    DreamMode.updateMemory(List("FACT 1: [DECISION] 快照探针裁定")).unsafeRunSync() match
      case DreamMode.MergeResult.Merged(_, _, _) => ()
      case other                                 => fail(s"应正常合并: $other")
    val key = "User.md"
    val mine = os.list(root).filter(os.isDir(_)).filter(d => os.exists(d / key)).sortBy(_.last)
    assert(mine.nonEmpty, "hook 写入必须先产生备份")
    assert(os.read(mine.last / key) == seed, "备份内容 = 写前真身")
    assert(os.read(userFile).contains("快照探针裁定"), "目标 = 写后内容")

  test("updateMemory: 快照失败 → SnapshotBlocked，文件零写入（hook fail-closed）"):
    val seed = "## Dream Extract\n\n### PATTERN\n\n- 原样保留\n"
    os.write.over(userFile, seed, createFolders = true)
    // 快照根位置占成【文件】→ 快照必败
    val root = home / "memory-backups"
    if os.exists(root) then os.remove.all(root)
    os.write.over(root, "not a directory", createFolders = true)
    DreamMode.updateMemory(List("FACT 1: [DECISION] 不该被写入的裁定")).unsafeRunSync() match
      case DreamMode.MergeResult.SnapshotBlocked(_) => ()
      case other                                    => fail(s"期望 SnapshotBlocked，得 $other")
    assertEquals(os.read(userFile), seed, "fail-closed：hook 零写入")
