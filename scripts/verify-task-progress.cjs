// verify-task-progress.cjs — 第七件 (2026-08-30) 任务工具重做简化为进展展示,
// frontend phase 1 (contract-independent parts).
//
//   G1  flat zero-regression: agent tasks WITHOUT team attribution render
//       exactly as before (no subgroup headers, rows direct in section)
//   G2  grouping: team -> member -> tasks hierarchy, first-appearance order,
//       two members in one team, two teams
...
//   G6  todo zone untouched: needs_confirmation still renders in todo section
...
const now = Date.now();
// Recent timestamps only: failed rows are visible same-day (isTodayLocal) —
// an hour-plus offset can cross midnight and silently drop the row.
...
async function renderIn(page, tasks) {
  return page.evaluate(async (list) => {
    const m = await import('/js/taskList.js');
    const container = document.getElementById('task-list');
    m.renderTaskList(list, container, 'sess1');
    ...
    return { directRows, subHeaders, groups, html: ps ? ps.innerHTML.length : 0 };
  }, tasks);
}
...
(async () => {
  ...
  // ── DOM-level groups (normal motion) ──
  {
    ...
    // G1 flat zero-regression
    let r = await renderIn(page, [tk(1, 'pending'), tk(2, 'in_progress'), tk(3, 'failed')]);
    ok('G1 no team info -> flat, zero subgroup headers', r.subHeaders.length === 0 && r.groups.length === 0, r.subHeaders);
    ok('G1 rows direct children of progress section', r.directRows === 3, r.directRows);

    // G2 grouping: teamA(two members) + teamB(one member). The progress zone
    // sorts updatedAt DESC before grouping, and groups appear in that sorted
    // first-appearance order — so teamA tasks carry the fresher timestamps.
    r = await renderIn(page, [
      tk(1, 'in_progress', { team: 'teamA', member: 'backend', updatedAt: new Date(now - 60000).toISOString() }),
      tk(2, 'pending', { team: 'teamA', member: 'frontend', updatedAt: new Date(now - 30000).toISOString() }),
      tk(3, 'in_progress', { team: 'teamA', member: 'backend', updatedAt: new Date(now - 50000).toISOString() }),
      tk(4, 'pending', { team: 'teamB', member: 'qa', updatedAt: new Date(now - 120000).toISOString() }),
    ]);
    ...
    r = await renderIn(page, [tk(1, 'in_progress'), tk(2, 'pending', { team: 'teamA', member: 'backend' })]);
    ...
    // G6 todo zone untouched
    const todo = await page.evaluate(async () => {
      const m = await import('/js/taskList.js');
      const container = document.getElementById('task-list');
      m.renderTaskList([
        { id: 'n1', subject: 'review me', status: 'needs_confirmation', taskKind: 'agent', updatedAt: new Date().toISOString() },
      ], container, 'sess1');
      await new Promise(r => setTimeout(r, 50));
      const row = container.querySelector('.task-section-todo .task-item[data-task-id="n1"]');
      const check = row && row.querySelector('.task-check');
      return { inTodo: !!row, circle: check ? getComputedStyle(check).borderRadius : null };
    });
    ok('G6 needs_confirmation stays in todo zone with circle', todo.inTodo === true && todo.circle === '50%', todo);
